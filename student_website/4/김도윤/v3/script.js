...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function spawnMonstersForZone(zone) {
        ...
        if (zone === 'ZONE1') {
            // 1지역: 늪 슬라임 (체력 30~45, 공격력 5~8, 전리품: 슬라임볼, 쉬움)
            bossHud.classList.add('hidden');
            for (let i = 0; i < 7; i++) {
                monsters.push({
                    type: 'slime',
            ...
            // 2지역: 숲 좀비 & 스켈레톤 (체력 50~65, 공격력 10~18, 보통)
            bossHud.classList.add('hidden');
            for (let i = 0; i < 4; i++) {
                monsters.push({
                    type: 'zombie',
            ...
            // 3지역: 동굴 오크 & 거미 (체력 70~100, 공격력 20~30, 어려움)
            bossHud.classList.add('hidden');
            for (let i = 0; i < 3; i++) {
                monsters.push({
                    type: 'orc',
    ...
    function draw() {
        ...
        monsters.forEach(m => {
            ...
            if (m.type === 'dragon') {
                ...
                ctx.fillRect(m.x - 24, m.y + 10, 24, 50);
                ctx.fillRect(m.x + m.width, m.y + 10, 24, 50);
                // 눈 (빛나는 황금안)
                ctx.fillStyle = '#ffe600';
                ctx.fillRect(m.x + 18, m.y + 24, 14, 14);
                ctx.fillRect(m.x + 64, m.y + 24, 14, 14);
                // 뿔
                ctx.fillStyle = '#222222';
                ctx.fillRect(m.x + 12, m.y - 16, 12, 16);
                ctx.fillRect(m.x + 72, m.y - 16, 12, 16);
                ctx.restore();
            } else {
