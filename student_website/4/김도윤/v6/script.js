// 픽셀 RPG v6 게임 엔진: 개성 넘치는 수제작 NPC 외형 7종 렌더링
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 캔버스 그리기 함수 (맵 테마 + 수제작 NPC 외형 렌더링)
    function draw() {
        ...
        if (currentZone === 'TOWN') {
            ...
            // [3지역 동굴] 어둡지만 영롱한 광물의 빛으로 가득 찬 신비로운 동굴
        ...
        // 2. 마을 구조물 (집, 나무) & 수제작 개성 NPC 7종 그리기
        if (currentZone === 'TOWN') {
            houses.forEach(h => {
            ...
            // 개성 넘치는 수제작 NPC 7종 렌더링
            npcs.forEach(npc => {
                const bob = Math.sin(time * 3 + npc.x) * 2;
                ctx.save();

                if (npc.role === 'elder') {
                    // 1. 촌장 장로 (푸른 마법 로브, 긴 하얀 수염, 에메랄드 지팡이)
                    ctx.fillStyle = '#2980b9'; // 파란 로브
                    ctx.beginPath();
                    ...
                    // 2. 경비대장 론 (은빛 강철 투구, 붉은 깃털 장식, 사자 갑옷, 장검)
                    ctx.fillStyle = '#bdc3c7'; // 은빛 갑옷
                    ctx.beginPath();
                    ...
                    // 3. 약초상 엘리 (화사한 꽃 머리띠, 분홍/초록 에이프런, 약초 바구니)
                    ctx.fillStyle = '#27ae60'; // 초록 치마
                    ctx.fillRect(npc.x + 6, npc.y + 18 + bob, npc.width - 12, npc.height - 18);
                    ctx.fillStyle = '#fd79a8'; // 분홍 에이프런
                    ...
                    // 4. 용병왕 칼드 (안대, 붉은 용병 망토, 어깨 철갑, 등 뒤 쌍검)
                    ctx.fillStyle = '#2d3436'; // 가죽 갑옷
                    ctx.beginPath();
                    ...
                    // 5. 무기상 대장장이 (붉은 두건, 가죽 작업복, 쇠망치, 모루)
                    ctx.fillStyle = '#795548'; // 가죽 앞치마
                    ctx.beginPath();
                    ...
                    // 7. 사냥꾼 카일 (초록 후드 망토, 깃털 사냥모자, 활과 화살통)
                    ctx.fillStyle = '#218c74'; // 사냥꾼 망토
                    ctx.beginPath();
                    if (ctx.roundRect) ctx.roundRect(npc.x + 4, npc.y + 10 + bob, npc.width - 8, npc.height - 10, 6);
                    else ctx.rect(npc.x + 4, npc.y + 10 + bob, npc.width - 8, npc.height - 10);
                    ctx.fill();
