// 픽셀 RPG v11 게임 엔진: BGM ON/OFF 음소거 스위치 (버튼 클릭 & M 단축키 지원) 구현
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 🎵 BGM ON/OFF 음소거 토글 함수 (버튼 클릭 및 M키 지원)
    function toggleBGM() {
        sound.init();
        isBgmMuted = !isBgmMuted;

        if (isBgmMuted) {
            btnBgmToggle.textContent = '🔇 BGM OFF (M키)';
            btnBgmToggle.classList.add('muted');
            sound.stopBGM();
            addFloatingText('🔇 배경음악(BGM)이 꺼졌습니다.', player.x - 20, player.y - 30, '#ff6666');
        } else {
            btnBgmToggle.textContent = '🔊 BGM ON (M키)';
            btnBgmToggle.classList.remove('muted');
            sound.playZoneBGM(currentZone);
            addFloatingText('🎵 배경음악(BGM)이 켜졌습니다.', player.x - 20, player.y - 30, '#00ffcc');
        }
    }
