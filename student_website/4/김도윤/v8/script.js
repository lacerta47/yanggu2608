// 픽셀 RPG v8 게임 엔진: Web Audio API 기반 지역별 5색 맞춤 테마 BGM 구현
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 사운드 & 지역별 절차적 맞춤 BGM 합성기 (Web Audio API)
    class SoundSynth {
        constructor() {
            this.ctx = null;
        ...
        playZoneBGM(zone) {
            ...
            if (this.currentBgmZone === zone) return; // 이미 같은 지역 BGM 연주 중
            ...
            if (zone === 'TOWN') {
                // 1. 🏡 마을: 평화롭고 아늑한 마장조 멜로디 (C4, E4, G4, B4, C5)
                ...
                // 2. 🌫️ 1지역 늪: 축축하고 어두운 물방울 서스펜스 (저음 베이스 + 물방울 드롭)
                ...
                // 3. ✨ 2지역 숲: 신비롭고 몽환적인 요정 반딧불이 아르페지오 (E minor / High Triangle Chimes)
                ...
                // 4. 💎 3지역 동굴: 어둡지만 광물의 빛이 영롱하게 울리는 오블리비언 울림 (Low Obsidian Bass + Crystal Rings)
                ...
                // 5. 🔥 드래곤의 둥지: 뜨겁고 긴박한 16비트 보스전 격투 비트 (D minor Fast Sawtooth Bass)
        ...
        // 2. BGM을 방해하지 않는 아주 은은하고 극히 작은 칼날 바람 소리
        playSwordSlash() {
            if (!this.ctx) return;
            const now = this.ctx.currentTime;
