// 픽셀 RPG v10 게임 엔진: 아늑하고 다양한 4가지 지붕/벽체 시골집 양식 및 굴뚝 연기 구현
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // 사운드 & 절차적 BGM 및 효과음 합성기 (Web Audio API)
    class SoundSynth {
        constructor() {
            this.ctx = null;
    ...
    // 마을 배치 생성 (다양하지만 화려하지 않고 아늑한 4가지 시골집 양식)
    function generateTownObjects() {
        houses = [];
        trees = [];

        // 4가지 다채롭고 아늑한 색감 & 양식 디자인
        const houseStyles = [
            { type: 'A', width: 90, height: 80, roofColor: '#a74a38', wallColor: '#e8d8c8', doorColor: '#4a2511', name: '기와 오두막' },
            { type: 'B', width: 84, height: 90, roofColor: '#4a6572', wallColor: '#8b6d5c', doorColor: '#2b2d42', name: '첨탑 집' },
            { type: 'C', width: 104, height: 82, roofColor: '#3d4856', wallColor: '#9c6644', doorColor: '#3e2723', name: '이층 돌담집' },
            { type: 'D', width: 92, height: 76, roofColor: '#6b8e23', wallColor: '#d4b896', doorColor: '#5d4037', name: '이끼 풀밭집' },
            { type: 'A', width: 88, height: 78, roofColor: '#8c533e', wallColor: '#f2e9e1', doorColor: '#4e342e', name: '황토 오두막' },
            { type: 'B', width: 86, height: 88, roofColor: '#556b2f', wallColor: '#706860', doorColor: '#37474f', name: '숲 오두막' },
            { type: 'C', width: 98, height: 84, roofColor: '#795548', wallColor: '#c8b6a6', doorColor: '#3e2723', name: '통나무집' },
            { type: 'D', width: 90, height: 80, roofColor: '#b85b35', wallColor: '#e5d4c0', doorColor: '#4a2511', name: '시골 오두막' }
        ];
    ...
    function draw() {
        ...
        // 2. 마을 구조물 (다양하고 아늑한 4가지 양식 시골집) & 수제작 개성 NPC 7종 그리기
        if (currentZone === 'TOWN') {
            houses.forEach(h => {
                ctx.save();
                ...
                // 집 양식별 지붕 및 창문 디테일 렌더링
                if (h.type === 'A') {
                    // Type A: 삼각 2-pitch 기와 지붕 + 박스 창문
                    ctx.fillStyle = h.roofColor;
                    ctx.beginPath();
                    ctx.moveTo(h.x - 8, h.y);
                    ctx.lineTo(h.x + h.width / 2, h.y - 28);
                    ctx.lineTo(h.x + h.width + 8, h.y);
                    ctx.fill();

                    // 격자 창문 & 처마
                    ctx.fillStyle = '#ffffff';
                    ctx.fillRect(h.x + 12, h.y + 16, 18, 18);
                    ctx.strokeStyle = '#4a2511';
                    ctx.lineWidth = 1.5;
                    ctx.strokeRect(h.x + 12, h.y + 16, 18, 18);

                } else if (h.type === 'B') {
                    // Type B: 뾰족 첨탑 지붕 + 동그라미 다락방 창문 + 돌 기단
                    ctx.fillStyle = '#636e72'; // 석재 기단
                    ctx.fillRect(h.x, h.y + h.height - 14, h.width, 14);

                    ctx.fillStyle = h.roofColor;
                    ctx.beginPath();
                    ctx.moveTo(h.x - 6, h.y);
                    ctx.lineTo(h.x + h.width / 2, h.y - 38);
                    ...
                    ctx.beginPath();
                    ctx.arc(h.x + h.width / 2, h.y + 22, 10, 0, Math.PI * 2);
                    ctx.fill();
                    ctx.strokeStyle = '#4a2511';
                    ctx.lineWidth = 1.5;
                    ctx.stroke();
                    ...
                    // Type C: 2층 낭만 돌담집 + 굴뚝 연기 모션
                    ctx.fillStyle = h.roofColor;
                    ctx.fillRect(h.x - 4, h.y - 12, h.width + 8, 12);

                    // 굴뚝 & 피어오르는 부드러운 연기
                    ctx.fillStyle = '#5c4033';
                    ctx.fillRect(h.x + h.width - 22, h.y - 30, 12, 18);

                    const smokeY = (time * 18) % 25;
                    ctx.fillStyle = 'rgba(220, 220, 220, 0.45)';
                    ctx.beginPath();
                    ctx.arc(h.x + h.width - 16 + Math.sin(time * 2) * 4, h.y - 34 - smokeY, 5 + smokeY * 0.2, 0, Math.PI * 2);
                    ctx.fill();

                    // 쌍창문
                    ctx.fillStyle = '#ffffff';
                    ctx.fillRect(h.x + 14, h.y + 14, 16, 16);
                    ctx.fillRect(h.x + h.width - 30, h.y + 14, 16, 16);

                } else if (h.type === 'D') {
                    // Type D: 초가/이끼 둥근 아치 지붕 + 야간 등불
                    ctx.fillStyle = h.roofColor;
                    ctx.beginPath();
                    ctx.ellipse(h.x + h.width / 2, h.y, h.width / 2 + 8, 18, 0, Math.PI, 0);
                    ctx.fill();
                    ...
                    ctx.beginPath();
                    ctx.arc(h.x + 16, h.y + h.height - 35, 4, 0, Math.PI * 2);
                    ctx.fill();
                }

                // 공통 아치 문구조
                ctx.fillStyle = h.doorColor;
                ctx.fillRect(h.x + h.width / 2 - 10, h.y + h.height - 28, 20, 28);
                ctx.fillStyle = '#ffd700'; // 손잡이
                ctx.fillRect(h.x + h.width / 2 + 3, h.y + h.height - 16, 3, 3);
