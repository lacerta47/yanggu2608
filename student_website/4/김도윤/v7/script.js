// 픽셀 RPG v7 게임 엔진: 느긋한 슬라임 또잉 모션 및 칼을 든 모험가 플레이어 구현
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    function updatePlayer() {
        ...
        if (currentZone === 'TOWN') {
            ...
            // 다른 사냥터 지역에서 마을로 복귀 (출구 어느 가장자리로 가도 안전 복귀)
    ...
    // 캔버스 그리기 함수 (맵 테마 + 수제작 NPC + 느긋한 슬라임 + 칼을 든 플레이어 영웅)
    function draw() {
        ...
        // 3. 몬스터 5종 및 보스 드래곤 렌더링 (슬라임 또잉속도 천천히 조정!)
        monsters.forEach(m => {
            if (m.hp <= 0) return;
            ...
            if (m.type === 'dragon') {
                ...
                // 1. 슬라임 (느긋하게 또~잉~ 또~잉~ 출렁이는 푸른 젤리 슬라임)
                const slowTime = Date.now() * 0.002; // 또잉 속도 천천히 조정!
        ...
        ctx.fillStyle = '#e74c3c'; // 붉은 모험가 머리띠
        ...
        // 🗡️ 손에 든 빛나는 묵직한 강철 칼 (항상 칼을 들고 있음!)
        ctx.save();
