...
shootBtn.addEventListener('touchstart', (e) => { e.preventDefault(); handleUserInteraction(); triggerManualShoot(); });
shootBtn.addEventListener('mousedown', (e) => { handleUserInteraction(); triggerManualShoot(); });
...
// 🔫 수동 레이저 총알 발사 (선인장 & 날아오는 익룡까지 완벽 타격!)
function triggerManualShoot() {
    if (gameState === 'RUNNING' && hasGun) {
        const startY = dino.isDucking ? GROUND_Y - 45 : GROUND_Y - 65;
        
        bullets.push({
            x: dino.x + dino.width + 10,
            y: startY,
            width: 22,
            height: 48, // 높은 범위까지 타격하여 익룡도 격추!
            speed: 18,
            isGodBullet: isGodHandActive // 신의 손 상태면 황금 무지개 총알!
        });

        const shootText = isGodHandActive ? '✨ 갓 블래스터 발사!' : '💥 탕! 익룡격추!';
        const shootColor = isGodHandActive ? '#f59e0b' : '#ef4444';
        addFloatingText(dino.x + 40, dino.y - 15, shootText, shootColor);
        createParticles(dino.x + dino.width + 10, startY + 24, shootColor, 15);
    }
}
...
// 아이템 스폰 (🖐️✨ 신의손, 🥚 황금알, 🛡️ 보호막, 🥛 무적우유, 🔫 총)
function createItem() {
    if (frameCount % 160 === 0 && Math.random() < 0.75) {
...
function collectItem(type) {
    if (type === 'godhand') {
        ...
        // 🖐️✨★ 핵심: 신의 손 획득 시 낮은 확률(약 25%)로 전설의 총이 솟아나옵니다!
        const luckyGunDrop = Math.random() < 0.25;
        if (luckyGunDrop) {
            hasGun = true;
            gunHud.classList.remove('hidden');
            if (gunText) gunText.textContent = "✨ 신의 손에서 전설의 갓 블래스터 등장! ('ㅜ' 키로 발사!)";
            addFloatingText(dino.x, dino.y - 40, '✨ 럭키! 신의 손에서 전설의 총 등장! (25% 대박!)', '#facc15');
            createParticles(dino.x + 20, dino.y + 20, '#facc15', 50);
        } else {
        ...
        hasGun = true;
        gunHud.classList.remove('hidden');
        if (gunText) gunText.textContent = "레이저 총 장착 중! ('ㅜ' / N 키로 발사!)";
        addFloatingText(dino.x, dino.y - 20, "🔫 레이저 총 장착! ('ㅜ'키로 수동 발사!)", '#ef4444');
        createParticles(dino.x + 20, dino.y + 20, '#ef4444', 35);
    } else if (type === 'egg') {
        if (!isGodHandActive) {
...
function drawDino() {
    ...
    if (isGodHandActive) dinoColor = '#f59e0b';
    else if (isDimensionActive) dinoColor = '#c084fc';
    else if (hasGun) dinoColor = '#38bdf8';
    ...
    if (dino.isDucking) {
        ...
        if (hasGun) {
            ctx.fillStyle = isGodHandActive ? '#facc15' : '#ef4444';
            ctx.fillRect(x + w + 2, y + 6, 14, 5);
            ctx.fillStyle = dinoColor;
        }
        ...
        if (hasGun) {
            ctx.fillStyle = isGodHandActive ? '#facc15' : '#ef4444';
            ctx.fillRect(x + 28, y + 24, 18, 6);
            ctx.fillRect(x + 24, y + 26, 6, 8);
            ctx.fillStyle = dinoColor;
        } else {
    ...
    // 🥛/🖐️✨/🌀 3초 남았을 때 공룡 몸 위에 3, 2, 1 카운트다운 숫자
...
// 💥 레이저 총알 그리기 (신의 손 발동 시 황금 무지개 레이저 빔!)
function drawBullets() {
    ctx.save();
    bullets.forEach(b => {
        const laserGrad = ctx.createLinearGradient(b.x, b.y, b.x + b.width, b.y);
        if (b.isGodBullet) {
            laserGrad.addColorStop(0, '#facc15');
            laserGrad.addColorStop(0.5, '#ef4444');
            laserGrad.addColorStop(1, '#38bdf8');
            ctx.shadowColor = '#facc15';
        } else {
            laserGrad.addColorStop(0, '#f59e0b');
            laserGrad.addColorStop(1, '#ef4444');
        ...
        ctx.fillRect(b.x - 4, b.y + b.height - 6, b.width, 6);
...
function update() {
    ...
    if (isGodHandActive) {
        ...
        // 신의 손 차원 여행 중 차원 균열에서 낮은 확률(20%)로 황금 총알 지원 생성!
        if (frameCount % 120 === 0 && Math.random() < 0.2 && !hasGun) {
            hasGun = true;
            gunHud.classList.remove('hidden');
            if (gunText) gunText.textContent = "✨ 차원 균열에서 갓 블래스터 획득! ('ㅜ' 키로 발사!)";
            addFloatingText(dino.x, dino.y - 30, '✨ 차원 균열에서 전설의 총 획득!', '#facc15');
    ...
    // 💥 수동 레이저 총알 이동 및 장애물(선인장 & 날아오는 익룡) 격추 파괴!
    for (let bIndex = bullets.length - 1; bIndex >= 0; bIndex--) {
        ...
        for (let obsIndex = obstacles.length - 1; obsIndex >= 0; obsIndex--) {
            ...
            if (checkCollision(bullet, obs)) {
                const text = obs.type === 'pterodactyl' ? '🦅💥 익룡 격추 파괴!' : '🌵💥 선인장 산산조각!';
                addFloatingText(obs.x, obs.y - 10, text, '#ef4444');
                createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, bullet.isGodBullet ? '#facc15' : '#ef4444', 25);
                score += bullet.isGodBullet ? 250 : 150;
