...
// 배열 요소
let obstacles = [];
let items = []; // 🥚 황금알, 🛡️ 보호막, 🥛 무적우유
let clouds = [];
let particles = [];
let floatingTexts = [];
let spaceStars = []; // 이차원 세계 우주 별빛들
...
// 아이템 획득 처리 (황금알 🥚 -> 이차원 세계 포탈 이동!)
function collectItem(type) {
    if (type === 'egg') {
        // 🌀 이차원 세계 차원 포탈 오픈!
        isDimensionActive = true;
        dimensionTimer = 450; // ~7.5초 동안 이차원 세계 진입!
        dimensionHud.classList.remove('hidden');

        addFloatingText(dino.x, dino.y - 20, '🌀 웜홀 포탈! 이차원 세계 진입 (3배 폭발!)', '#c084fc');
...
function drawDino() {
    ...
    if (isDimensionActive) dinoColor = '#c084fc'; // 이차원 보라색 공룡
    ...
    // 🥛 무적 우유 3초 남으면 공룡 몸에 3, 2, 1 카운트다운 숫자를 새김!
    if (isMilkActive && milkTimer <= 180) {
