...
function collectItem(type) {
    if (type === 'gun') {
        ...
        addFloatingText(dino.x, dino.y - 20, '🔫 무한 레이저 총 장착! (무한 파괴 연사!)', '#ef4444');
...
function drawDino() {
    ...
    else if (hasGun) dinoColor = '#38bdf8'; // 총을 가졌을 때 레이저 블루 공룡
    ...
    if (dino.isDucking) {
        ...
        if (hasGun) {
            ...
            ctx.fillRect(x + w + 2, y + 6, 14, 5); // 총열
            ctx.fillStyle = dinoColor;
        }
        ...
        if (hasGun) {
            ctx.fillStyle = '#ef4444';
            ctx.fillRect(x + 28, y + 24, 18, 6); // 총열
            ctx.fillRect(x + 24, y + 26, 6, 8);  // 손잡이
            ctx.fillStyle = dinoColor;
        } else {
...
function update() {
    ...
    // 🔫 레이저 총 무한 자동 발사 연사 처리! (14프레임마다 총알 발사)
    if (hasGun) {
        bulletShootTimer++;
        if (bulletShootTimer >= 14) {
            bulletShootTimer = 0;
            const bulletY = dino.isDucking ? dino.y + 8 : dino.y + 24;
            bullets.push({
