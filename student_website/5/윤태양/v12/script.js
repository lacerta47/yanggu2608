...
// 🔫 수동 레이저 총알 발사 (선인장 & 높이 날아오는 익룡까지 완벽 타격하는 듀얼 와이드 빔!)
function triggerManualShoot() {
    if (gameState === 'RUNNING' && hasGun) {
        // 공중 익룡과 지상 선인장을 모두 맞추는 와이드 레이저 빔 발사!
        ...
        addFloatingText(dino.x + 40, dino.y - 15, '💥 탕! 익룡격추!', '#ef4444');
        createParticles(dino.x + dino.width + 10, startY + 24, '#ef4444', 15);
    }
}
...
// 아이템 스폰 (🔫 레이저총, 🖐️✨ 신의손, 🥚 황금알, 🛡️ 보호막, 🥛 무적우유)
function createItem() {
...
function collectItem(type) {
    if (type === 'gun') {
        ...
        addFloatingText(dino.x, dino.y - 20, "🔫 레이저 총 장착! (익룡&선인장 전멸 발사!)", '#ef4444');
...
function drawBullets() {
    ...
    bullets.forEach(b => {
        ...
        // 듀얼 트윈 빔그리기 (위쪽 빔: 익룡용, 아래쪽 빔: 선인장용)
    ...
    for (let bIndex = bullets.length - 1; bIndex >= 0; bIndex--) {
        ...
        for (let obsIndex = obstacles.length - 1; obsIndex >= 0; obsIndex--) {
            ...
            if (checkCollision(bullet, obs)) {
                ...
                createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, '#ef4444', 25);
                score += 150; // 격추 성공 보너스 점수!
