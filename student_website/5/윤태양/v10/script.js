...
// 시간 멈춤 HUD 요소
const timeFreezeHud = document.getElementById('timeFreezeHud');
const timeFreezeTimerBar = document.getElementById('timeFreezeTimerBar');
...
// ⏳ 'N' 키 시간 멈춤 (Time Freeze) 변수
let isTimeFreezeActive = false;
let timeFreezeTimer = 0;
const TIME_FREEZE_MAX_DURATION = 360;
...
freezeBtn.addEventListener('touchstart', (e) => { e.preventDefault(); handleUserInteraction(); triggerTimeFreeze(); });
freezeBtn.addEventListener('mousedown', (e) => { handleUserInteraction(); triggerTimeFreeze(); });
...
// ⏸️ N키 / 모바일 버튼 누를 때 시간 멈춤 발동!
function triggerTimeFreeze() {
    if (gameState === 'RUNNING' && !isTimeFreezeActive) {
        isTimeFreezeActive = true;
        timeFreezeTimer = TIME_FREEZE_MAX_DURATION;
        timeFreezeHud.classList.remove('hidden');

        addFloatingText(dino.x, dino.y - 20, '⏸️ N키 시간 멈춤 파워 발동!', '#38bdf8');
        createParticles(dino.x + 20, dino.y + 20, '#38bdf8', 35);
    }
}
...
function startGame() {
    ...
    startOverlay.style.display = 'none'; // 확실하게 가리기
}
...
function resetGame() {
    ...
    timeFreezeHud.classList.add('hidden');
...
// 아이템 스폰 (⏳ 시간의구, 🖐️✨ 신의손, 🥚 황금알, 🛡️ 보호막, 🥛 무적우유)
function createItem() {
...
function handleGodHandBackground() {
    ...
    if (godHandTimer > 360) {
        ...
        spaceStars.forEach(star => {
            ...
            if (!isTimeFreezeActive) star.x -= star.speed * 3;
...
function handleDimensionBackground() {
    ...
    spaceStars.forEach(star => {
        ctx.fillStyle = star.color;
        ctx.fillRect(star.x, star.y, star.size, star.size);
        if (!isTimeFreezeActive) star.x -= star.speed * 2;
        if (star.x < 0) star.x = CANVAS_WIDTH;
    });
...
function drawDino() {
    ...
    if (isTimeFreezeActive) {
        ctx.beginPath();
        ctx.arc(x + w / 2, y + h / 2, w / 2 + 15, 0, Math.PI * 2);
    ...
    // 보호막 링
    if (shieldsCount > 0 && !isMilkActive && !isDimensionActive && !isGodHandActive && !isTimeFreezeActive) {
        ctx.beginPath();
    ...
    if (isTimeFreezeActive) dinoColor = '#38bdf8';
    ...
    // ⏳★ 3초 남았을 때 공룡 몸 위에 3, 2, 1 카운트다운 숫자가 새겨집니다!
    if (isTimeFreezeActive && timeFreezeTimer <= 180) {
        const remainingSeconds = Math.ceil(timeFreezeTimer / 60);
        ...
        ctx.fillStyle = (frameCount % 10 < 5) ? '#38bdf8' : '#ffffff';
        ctx.fillText(remainingSeconds.toString(), x + w / 2, y + h / 2);
    } else if (isGodHandActive && godHandTimer <= 180) {
...
function drawObstacle(obs) {
    ...
    if (obs.type === 'pterodactyl') {
        ctx.fillStyle = isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : (isTimeFreezeActive ? '#38bdf8' : '#f59e0b'));
        ...
        if (wingUp && !isTimeFreezeActive) {
            ctx.fillRect(obs.x + 14, obs.y - 10, 12, 18);
        } else {
            ctx.fillRect(obs.x + 14, obs.y + 16, 12, 14);
        }
    } else {
        ctx.fillStyle = isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : (isTimeFreezeActive ? '#0284c7' : '#22c55e'));
...
function drawGround(currentSpeed) {
    ctx.save();
    ctx.strokeStyle = isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : (isTimeFreezeActive ? '#38bdf8' : '#475569'));
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(0, GROUND_Y);
    ctx.lineTo(CANVAS_WIDTH, GROUND_Y);
    ...
    ctx.fillStyle = isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : (isTimeFreezeActive ? '#0284c7' : '#64748b'));
...
function update() {
    ...
    if (isTimeFreezeActive) {
        timeFreezeTimer--;
        const pct = (timeFreezeTimer / TIME_FREEZE_MAX_DURATION) * 100;
        timeFreezeTimerBar.style.width = `${pct}%`;

        if (timeFreezeTimer <= 0) {
            isTimeFreezeActive = false;
            timeFreezeHud.classList.add('hidden');
            addFloatingText(dino.x, dino.y - 15, '시간 멈춤 해제! 시간이 흐릅니다!', '#38bdf8');
    ...
    const currentSpeed = isTimeFreezeActive ? 0 : (baseSpeed + Math.floor(score / 250) * 0.5);
    ...
    // 구름 이동
    if (!isDimensionActive && !isGodHandActive && !isTimeFreezeActive) createCloud();
    for (let i = clouds.length - 1; i >= 0; i--) {
        clouds[i].x -= isTimeFreezeActive ? 0 : clouds[i].speed;
        if (clouds[i].x + clouds[i].width < 0) clouds.splice(i, 1);
    }

    // 아이템 이동 및 획득
    if (!isTimeFreezeActive) createItem();
    ...
    // 장애물 이동 및 충돌 처리
    if (!isTimeFreezeActive) createObstacle();
