...
function startBGM() {
    ...
    bgmIntervalId = setInterval(() => {
        ...
        if (isDragonActive) {
            // 🐉🖤 블랙 드래곤 조련 BGM: 웅장하고 용맹한 아포칼립스 락 베이스
...
function startDuck() {
    if (gameState === 'RUNNING' && !dino.isDucking && !dino.isJumping) {
        ...
        dino.y = (isDragonActive ? GROUND_Y - 110 : GROUND_Y - currentH);
    }
}
...
function stopDuck() {
    if (dino.isDucking) {
        ...
        dino.height = normalH;
        dino.y = (isDragonActive ? GROUND_Y - 110 : GROUND_Y - normalH);
    }
}
...
// 아이템 스폰 (🐉🖤 블랙드래곤, 👑 킹공룡, ⚡ 번개, 🕳️ 블랙홀 확률 대폭 업!)
function createItem() {
    if (frameCount % 130 === 0 && Math.random() < 0.85) {
        const rand = Math.random();
        let type = 'shield';

        if (rand < 0.2) {
            type = 'dragon'; // 🐉🖤 블랙 드래곤 조련 아이템!
        } else if (rand < 0.4) {
...
function collectItem(type) {
    if (type === 'dragon') {
        ...
        dino.y = GROUND_Y - 110; // 공중 비행 높이
        dragonHud.classList.remove('hidden');

        addFloatingText(dino.x, dino.y - 25, '🐉🖤 블랙 드래곤 조련 비행! (다크 브레스 발사!)', '#a855f7');
        createParticles(dino.x + 40, dino.y + 20, '#a855f7', 50);
    } else if (type === 'giant') {
        isGiantActive = true;
...
// 🐉🔥 드래곤 불꽃 브레스 그리기
function drawDragonFlames() {
    ctx.save();
    dragonFlames.forEach(f => {
        const flameGrad = ctx.createLinearGradient(f.x, f.y, f.x + f.width, f.y);
...
function drawDino() {
    ...
    // 🐉🖤 검은 블랙 드래곤에 조련 탑승한 웅장한 모습!
    if (isDragonActive) {
        const dragX = x - 25;
        const dragY = y + 15;

        // 드래곤 다크 퍼플 날개짓 애니메이션
        const wingYOffset = Math.sin(frameCount * 0.2) * 12;

        ctx.fillStyle = '#1e1b4b'; // 칠흑 드래곤 몸체
        ctx.beginPath();
        ctx.ellipse(dragX + 45, dragY + 25, 42, 22, 0, 0, Math.PI * 2);
        ctx.fill();

        // 날개
        ctx.fillStyle = '#581c87';
        ctx.beginPath();
        ctx.moveTo(dragX + 35, dragY + 15);
        ctx.lineTo(dragX + 10, dragY - 20 + wingYOffset);
        ctx.lineTo(dragX + 55, dragY + 10);
        ctx.fill();

        // 드래곤 머리 & 입(브레스 뿜는 모습)
        ctx.fillStyle = '#1e1b4b';
        ctx.fillRect(dragX + 75, dragY + 5, 25, 18);
        ctx.fillStyle = '#ef4444';
        ctx.fillRect(dragX + 90, dragY + 9, 5, 5); // 붉은 드래곤 눈빛

        // 드래곤 오라
        ctx.beginPath();
        ctx.arc(dragX + 50, dragY + 20, 55, 0, Math.PI * 2);
    ...
    // 보호막 링
    if (shieldsCount > 0 && !isMilkActive && !isDimensionActive && !isGodHandActive && !isGiantActive && !isDragonActive) {
        ctx.beginPath();
    ...
    if (isDragonActive) dinoColor = '#c084fc'; // 드래곤 기수 보라빛 공룡
...
function drawObstacle(obs) {
    ...
    if (obs.type === 'pterodactyl') {
        ctx.fillStyle = isDragonActive ? '#a855f7' : (isGiantActive ? '#f59e0b' : (isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : '#f59e0b')));
        ...
        ctx.fillStyle = isDragonActive ? '#a855f7' : (isGiantActive ? '#eab308' : (isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : '#22c55e')));
...
function drawGround(currentSpeed) {
    ctx.save();
    ctx.strokeStyle = isDragonActive ? '#a855f7' : (isGiantActive ? '#f59e0b' : (isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : '#475569')));
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(0, GROUND_Y);
    ctx.lineTo(CANVAS_WIDTH, GROUND_Y);
    ...
    ctx.fillStyle = isDragonActive ? '#a855f7' : (isGiantActive ? '#eab308' : (isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : '#64748b')));
...
function update() {
    ...
    if (isDragonActive) {
        ...
        if (dragonTimer <= 0) {
            ...
            dragonHud.classList.add('hidden');
            dragonFlames = [];
            addFloatingText(dino.x, dino.y - 15, '드래곤 조련 종료! 착지!', '#a855f7');
            createParticles(dino.x + 20, dino.y + 20, '#a855f7', 30);
        }
    }
    ...
    // 공룡 점프 및 중력 (드래곤 조련 중이 아닐 때만 점프 연산)
    if (dino.isJumping && !isDragonActive) {
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (checkCollision(dino, obs)) {
            if (isDragonActive) {
                addFloatingText(obs.x, obs.y - 15, '🐉🔥 드래곤 짓밟기 +200!', '#a855f7');
                createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, '#a855f7', 35);
                score += 200;
                obstacles.splice(i, 1);
                continue;
...
function draw() {
    ...
    drawDragonFlames();    // 🐉🔥 드래곤 불꽃 브레스 그리기
    drawDino();
    handleEffects();
}
