...
// 🌀✨ 종결 아이템: 최고의 소용돌이 HUD
const vortexHud = document.getElementById('vortexHud');
const vortexTimerBar = document.getElementById('vortexTimerBar');
...
const vortexSecretBtn = document.getElementById('vortexSecretBtn');
...
// 🌀✨ 종결 아이템: 최고의 소용돌이 (120초 순백 무한 차원 무적)
let isVortexActive = false;
let vortexTimer = 0;
const VORTEX_MAX_DURATION = 120 * 60; // 120초 (7,200 프레임)
...
if (vortexSecretBtn) {
    vortexSecretBtn.addEventListener('click', () => {
        handleUserInteraction();
        triggerUltimateVortex();
    });
}
...
// 레트로 8비트 & 천상의 신스 BGM 루프
function startBGM() {
    if (bgmIntervalId) clearInterval(bgmIntervalId);

    const normalMelody = [261.63, 293.66, 329.63, 392.00, 440.00, 392.00, 329.63, 293.66];
    const divineChords = [523.25, 659.25, 783.99, 1046.50, 880.00, 659.25, 587.33, 783.99];

    bgmIntervalId = setInterval(() => {
        if (gameState !== 'RUNNING' || isSoundMuted || !audioCtx) return;

        if (isVortexActive) {
            // 🌀✨ 120초 최고의 소용돌이: 웅장한 천상의 신디사이저 성가 코드!
            const note = divineChords[bgmNoteIndex % divineChords.length];
            playTone(note, 'sine', 0.45, 0.12);
            playTone(note * 0.5, 'triangle', 0.5, 0.15); // 깊은 베이스 오르간
...
function resetGame() {
    ...
    vortexHud.classList.add('hidden');
...
// 🌀✨ 궁극의 종결 아이템: 최고의 소용돌이 발동 함수! (120초 순백 무한 차원 무적)
function triggerUltimateVortex() {
    isVortexActive = true;
    vortexTimer = VORTEX_MAX_DURATION;
    vortexHud.classList.remove('hidden');

    addFloatingText(dino.x, dino.y - 45, '🌀✨ 120초 최고의 소용돌이 발동! 순백의 무한 차원 무적!', '#ffffff');
    createParticles(dino.x + 40, dino.y + 20, '#ffffff', 80);
    createParticles(dino.x + 40, dino.y + 20, '#facc15', 60);

    // 웅장한 승리의 팡파르
    playTone(523.25, 'sine', 0.3, 0.2);
    setTimeout(() => playTone(659.25, 'sine', 0.3, 0.2), 120);
    setTimeout(() => playTone(783.99, 'sine', 0.3, 0.2), 240);
    setTimeout(() => playTone(1046.50, 'sine', 0.6, 0.25), 360);
}

// 아이템 스폰 (극악의 확률 룰 적용!)
function createItem() {
    // 🌀✨ 최고의 소용돌이 기적의 극악 확률: 1e-108
    if (Math.random() < 1e-108) {
        items.push({ x: CANVAS_WIDTH, y: GROUND_Y - 70, width: 36, height: 36, type: 'vortex' });
        return;
    }

    // 히든 아이템 3종의 극악 확률: 1e-31
    if (Math.random() < 1e-31) {
        const hRand = Math.random();
        let hType = hRand < 0.33 ? 'giant' : (hRand < 0.66 ? 'thunder' : 'blackhole');
        items.push({ x: CANVAS_WIDTH, y: GROUND_Y - 60, width: 32, height: 32, type: hType });
        return;
    }

    // 기본 아이템 스폰
    if (frameCount % 160 === 0 && Math.random() < 0.8) {
...
function collectItem(type) {
    if (type === 'vortex') {
        ...
        if (!isGodHandActive && !isVortexActive) {
...
// 🤍 120초 순백의 신 배경 연출
function handleWhiteVoidBackground() {
    ctx.save();
    // 눈부신 순백의 신성한 그라데이션
    const whiteGrad = ctx.createLinearGradient(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);
    whiteGrad.addColorStop(0, '#ffffff');
    whiteGrad.addColorStop(0.5, '#fef9c3');
    whiteGrad.addColorStop(1, '#fef08a');
    ctx.fillStyle = whiteGrad;
    ctx.fillRect(0, 0, CANVAS_WIDTH, CANVAS_HEIGHT);

    // 성스러운 황금 광선
    holyRays.forEach(ray => {
        ctx.fillStyle = `rgba(255, 255, 255, ${ray.opacity})`;
        ctx.fillRect(ray.x, 0, ray.width, CANVAS_HEIGHT);
        ray.x -= ray.speed * 40;
        if (ray.x + ray.width < 0) ray.x = CANVAS_WIDTH;
    });
...
// 공룡 그리기 (순백의 무한 차원 후광 및 3초 카운트다운)
function drawDino() {
    ...
    if (isVortexActive) {
        ctx.beginPath();
        ctx.arc(x + w / 2, y + h / 2, w / 2 + 25, 0, Math.PI * 2);
    ...
    // 이차원 잔상
    if (isDimensionActive && !isGodHandActive && !isVortexActive) {
        ctx.beginPath();
    ...
    // 보호막 링
    if (shieldsCount > 0 && !isMilkActive && !isDimensionActive && !isGodHandActive && !isGiantActive && !isVortexActive) {
        ctx.beginPath();
    ...
    if (isVortexActive) dinoColor = '#d97706'; // 순백 배경 대비 황금 브라운
    ...
    if (isVortexActive && vortexTimer <= 180) { activeTimer = vortexTimer; timerColor = '#d97706'; }
...
function drawObstacle(obs) {
    ...
    if (obs.type === 'pterodactyl') {
        ctx.fillStyle = isVortexActive ? '#d97706' : (isGiantActive ? '#f59e0b' : (isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : '#f59e0b')));
        ...
        ctx.fillStyle = isVortexActive ? '#d97706' : (isGiantActive ? '#eab308' : (isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : '#22c55e')));
...
function drawGround(currentSpeed) {
    ctx.save();
    ctx.strokeStyle = isVortexActive ? '#d97706' : (isGiantActive ? '#f59e0b' : (isGodHandActive ? '#f59e0b' : (isDimensionActive ? '#c084fc' : '#475569')));
    ctx.lineWidth = 3;
    ctx.beginPath();
    ctx.moveTo(0, GROUND_Y);
    ctx.lineTo(CANVAS_WIDTH, GROUND_Y);
    ...
    ctx.fillStyle = isVortexActive ? '#f59e0b' : (isGiantActive ? '#eab308' : (isGodHandActive ? '#eab308' : (isDimensionActive ? '#38bdf8' : '#64748b')));
...
function update() {
    ...
    if (isVortexActive) {
        vortexTimer--;
        const pct = (vortexTimer / VORTEX_MAX_DURATION) * 100;
        vortexTimerBar.style.width = `${pct}%`;

        if (vortexTimer <= 0) {
            isVortexActive = false;
            vortexHud.classList.add('hidden');
            addFloatingText(dino.x, dino.y - 15, '순백의 무한 차원 종료!', '#f59e0b');
            createParticles(dino.x + 20, dino.y + 20, '#f59e0b', 40);
        }
    }
    ...
    let scoreAdd = 1;
    if (isVortexActive) scoreAdd = 10; // 🌀✨ 최고의 소용돌이: 점수 10배 폭발!
    else if (isGodHandActive) scoreAdd = 5;
    ...
    // 구름 이동
    if (!isDimensionActive && !isGodHandActive && !isVortexActive) createCloud();
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        // 🌀✨ 최고의 소용돌이 120초: 모든 장애물을 성스러운 빛으로 승화 소멸!
        if (isVortexActive && obs.x < CANVAS_WIDTH - 20) {
            addFloatingText(obs.x, obs.y - 15, '✨ 신의 정화 +500!', '#facc15');
            createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, '#ffffff', 25);
            score += 500;
    ...
    for (let i = obstacles.length - 1; i >= 0; i--) {
        ...
        if (checkCollision(dino, obs)) {
            if (isVortexActive) {
                addFloatingText(obs.x, obs.y - 15, '✨ 순백 소멸 +500!', '#ffffff');
                createParticles(obs.x + obs.width / 2, obs.y + obs.height / 2, '#ffffff', 40);
                score += 500;
                obstacles.splice(i, 1);
                continue;
            } else if (isGiantActive) {
...
function draw() {
    ...
    // 🤍 120초 최고의 소용돌이 순백의 배경
    if (isVortexActive) {
        handleWhiteVoidBackground();
    } else if (isGodHandActive) {
