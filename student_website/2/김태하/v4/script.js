...
// 우주 장애물 클래스 (바닥 운석 / UFO / 하늘 공중 운석 추가!)
class Obstacle {
  constructor(canvasWidth, canvasHeight, type) {
    this.type = type; // 'meteor'(바닥), 'low-ufo', 'high-ufo', 'sky-meteor'(상공 날아다니는 운석)
    ...
    if (this.type === 'meteor') {
      ...
      this.height = 28;
      this.x = canvasWidth + 10;
      this.y = canvasHeight - 30 - 45 - 28;
      this.color = '#ff9900';
      this.speedMult = 1.0;
    } else {
      // 하늘 공중 운석 (상공에서 빠르게 날아옴!)
      this.width = 38;
      this.height = 30;
      this.x = canvasWidth + 20;
      this.y = Math.floor(Math.random() * 90 + 60); // 상공 60~150 위치
      this.color = '#ff3300';
      this.speedMult = 1.35; // 일반 장애물보다 빠르게 슝- 날아감!
    }
  }
...
// 배경을 통과하는 배경용 혜성/운석 (데미지 없는 배경 연출)
...
class SpaceDinoGame {
  ...
  spawnObstacle() {
    ...
    if (this.obstacleTimer >= this.nextObstacleDistance) {
      ...
      if (rand > 0.75) {
        type = 'sky-meteor'; // 상공 날아가는 공중 운석!
      } else if (rand > 0.5) {
        type = 'high-ufo';   // 머리 위치 UFO (슬라이딩 필수)
      } else if (rand > 0.25) {
        type = 'low-ufo';    // 중간 위치 UFO
      }
