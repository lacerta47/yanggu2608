...
// 장애물 클래스 (운석 = 슬라이딩 불가, 무조건 점프! / UFO = 슬라이딩 가능)
class Obstacle {
  constructor(canvasWidth, canvasHeight, type) {
    this.type = type; // 'meteor'(운석: 높이가 커서 슬라이딩으로도 절대 못 피함!), 'high-ufo'(UFO: 슬라이딩으로 피함)
    this.canvasWidth = canvasWidth;
    this.canvasHeight = canvasHeight;

    if (this.type === 'meteor') {
      // ☄️ 운석: 바닥부터 공룡 키 높이까지 커다란 불꽃 운석으로 기어오거나 날아옴!
      // 높이를 42px로 크게 잡아 슬라이딩(높이 24px) 상태에서도 무조건 부딪히도록 구현!
      this.width = 40;
      this.height = 42;
      this.x = canvasWidth + 10;
      this.y = canvasHeight - 30 - this.height; // 바닥 밀착
      this.color = '#ff0055';
    } else {
      // 🛸 UFO: 공중에 수평으로 떠 있어서 슬라이딩(숙이기)으로만 쏙 지나갈 수 있음!
      this.width = 48;
      this.height = 26;
      this.x = canvasWidth + 10;
...
class SpaceDinoGame {
  constructor() {
    ...
    this.score = 0;
    this.highScore = localStorage.getItem('space_dino_v9_high_score') || 0;
    this.baseSpeed = 4.8;
    ...
    this.warningTimer = 0;
    this.warningType = null; // 'JUMP' (운석 - 점프 필수!), 'SLIDE' (UFO - 슬라이딩)
  ...
  resetGame() {
    ...
    this.gameTitle.textContent = '🚀 SPACE DINO RUNNER v9 🚀';
  ...
  spawnObstacle() {
    ...
    if (this.obstacleTimer === this.nextObstacleDistance - 18) {
      ...
      if (rand > 0.45) {
        this.warningType = 'JUMP';  // ☄️ 운석 등장! -> 무조건 점프! (슬라이딩 불가!)
      } else {
        this.warningType = 'SLIDE'; // 🛸 UFO 등장! -> 슬라이딩 숙이기!
      }
    ...
    if (this.obstacleTimer >= this.nextObstacleDistance) {
      ...
      const minGap = Math.max(48, 110 - Math.floor((this.gameSpeed - this.baseSpeed) * 8));
  ...
  handleGameOver() {
    ...
    if (this.score > this.highScore) {
      ...
      localStorage.setItem('space_dino_v9_high_score', this.highScore);
    }
