...
class SpaceDinoGame {
  constructor() {
    ...
    this.score = 0;
    this.highScore = localStorage.getItem('space_dino_v7_high_score') || 0;
    this.baseSpeed = 4.8; // 부드럽고 쉬운 초반 시작 속도!
  ...
  updateUI() {
    ...
    if (currentScore < 200) {
      ...
      this.levelText.textContent = '💥 EXTREME';
  ...
  spawnObstacle() {
    ...
    if (this.obstacleTimer >= this.nextObstacleDistance) {
      ...
      // 속도가 빨라질수록 장애물 간격이 점진적으로 좁아짐
      const minGap = Math.max(50, 110 - Math.floor((this.gameSpeed - this.baseSpeed) * 8));
  ...
  handleGameOver() {
    ...
    if (this.score > this.highScore) {
      ...
      localStorage.setItem('space_dino_v7_high_score', this.highScore);
    }
  ...
  gameLoop() {
    ...
    this.score += 0.15;
    // 부드러운 난이도 곡선: 점수가 높아질수록 속도가 매끄럽게 상승!
