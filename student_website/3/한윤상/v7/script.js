// --------------------------------------------------
// 백룸 3D 공포 생존 게임 (v7 Engine - 입체 실감 오디오 시스템)
// --------------------------------------------------
...
// --------------------------------------------------
// 2. Web Audio API 실감 오디오 합성 엔진 (v7 신규 연출)
// --------------------------------------------------
let audioCtx = null;
...
// ⚡ 1. 형광등 깜박이는 전기 스파크 소리 (Flicker Zap)
function playFlickerZapSound() {
    ...
    try {
        ...
        filter.frequency.setValueAtTime(3000 + Math.random() * 2000, audioCtx.currentTime);
...
// 👣 2. 레벨 1 카펫/딱딱한 바닥 걷는 소리 (Carpet Footstep)
...
// 💦 3. 레벨 2 수영장 무릎 높이 물을 걷는 초현실적 3중 유체 사운드 (Ultra-Realistic Water Splash)
...
function playLevel2WaterFootstep() {
    ...
    try {
        ...
        const pitchVar = (Math.random() - 0.5) * 120; // 발을 딛는 미세한 무게 피치 변화

        // 1️⃣ [수중 묵직한 물 저항 펀치] 무릎으로 물을 쳐내며 가르는 묵직한 저음 (Sub-Water Punch)
        ...
        // 2️⃣ [철퍽! 물표면 타격 & 튀는 찰나의 순간] (Surface Hydro-Splash Noise)
        ...
        // 3️⃣ [물방울 튀어서 떨어지고 물살이 출렁이는 잔향] (Water Droplet & Slosh Ripple Echo)
...
// 📌 레벨 1: 백룸 (발광 황금 열쇠 + 노란 벽 일체형 EXIT 문)
function loadLevel1() {
    clearCurrentScene();
    initVisitedGrid();
    currentLevel = 1;
...
function updateBacteriaAI(delta) {
    ...
    // ⚡ 레벨 1 형광등 깜빡임 연출 (소리 없이 시각 효과만 유지)
...
function updateMovement(delta) {
    ...
    // 👣 레벨별 입체 발소리 리듬 연출 (레벨1 걷기 / 레벨2 물 첨벙 소리)
    if (isMoving) {
        ...
        if (player.stepTimer >= 1.0) {
            player.stepTimer = 0;
            if (currentLevel === 1) {
                playLevel1Footstep(); // 👣 레벨 1 카펫 걷는 소리
            } else if (currentLevel === 2) {
                playLevel2WaterFootstep(); // 💦 레벨 2 수영장 물 걷는 소리
            }
        }
    } else {
