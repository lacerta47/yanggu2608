// --------------------------------------------------
// 백룸 3D 공포 생존 게임 (v5 Engine - 오류 완전 해결 & 안정화)
// --------------------------------------------------
...
function loadLevel1() {
    ...
    currentLevel = 1;
    hasKey = false;
    if (missionText) missionText.textContent = '🔑 백룸 열쇠 찾기 (0/1)';
    ...
    // 🔑 황금빛으로 강하게 발광하는 무작위 1개 황금 열쇠 스폰
    ...
    // 🚪 노란 벽지 벽면에 일체형으로 설치되는 EXIT 빛나는 탈출문
...
// 📌 레벨 2: 풀룸 수영장 (하얀 타일 + 출렁이는 물 + 파이프 밸브 4개 + 미끄럼틀)
function loadLevel2() {
    clearCurrentScene();
    initVisitedGrid();
    currentLevel = 2;
    valvesTurned = 0;
    if (missionText) missionText.textContent = `🔧 파이프 밸브 돌리기 (0/${TOTAL_VALVES})`;
    if (fpsDisplay) fpsDisplay.textContent = '⚡ 60 FPS | 📍 레벨 2 풀룸 수영장';
    if (statusMessage) statusMessage.textContent = '수영장 미로에 흩어진 4개의 파이프 밸브를 돌리세요!';
    ...
    if (currentLevel === 2) {
        for (let v of valves) {
            if (!v.isTurned && player.position.distanceTo(v.pos) < 2.0) {
                ...
                valvesTurned++;
                if (missionText) missionText.textContent = `🔧 파이프 밸브 돌리기 (${valvesTurned}/${TOTAL_VALVES})`;
...
// 🧭 탐험 안개(Fog of War) 실시간 2D 미니맵 안전 렌더링
...
function drawMinimap() {
    ...
    if (currentLevel === 1 && keyMesh) {
        ...
        if (visitedGrid[kx] && visitedGrid[kx][kz]) {
            minimapCtx.fillStyle = '#ffd166';
            minimapCtx.beginPath();
            minimapCtx.arc(kx * cellSize + cellSize / 2, kz * cellSize + cellSize / 2, cellSize * 0.8, 0, Math.PI * 2);
            minimapCtx.fill();
        }
    }
    ...
    if (doorMeshObj) {
        ...
        if (visitedGrid[dx] && visitedGrid[dx][dz]) {
