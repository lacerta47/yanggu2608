/* ==========================================================================
   1인칭 500km RPG (v8) Script - Real 3D Gun Mesh Engine & High Quality Overhaul
   - 화면 글씨/DOM 총을 완전히 제거하고 Three.js 입체 3D 총기 메쉬 적용!
   - 일반 권총(일반 총) / 대구경 저격소총 / 🚀 RPG 폭탄 로켓포 3D 메쉬 동적 변경!
   - 1인칭 3D 반동 애니메이션, 총구 3D 화염 및 탄환 궤적 레이저 이펙트!
   - 집 안 보물상자 뽑기 시스템 (🚀 RPG 30% / 🎯 저격소총 50% / 💣 꽝 20%)
...
// 2. 무기 DB (0: 일반 총 - 권총 / 1: 저격소총 / 2: RPG 폭탄 로켓포)
const weaponsDB = [
...
// 3. Web Audio Synthesizer (사격음 / RPG 폭격을 / 발소리 / 피격음)
let audioCtx = null;
...
function playRPGBoomSound() {
    ...
    try {
        ...
        osc1.frequency.exponentialRampToValueAtTime(20, ctx.currentTime + 0.65);

        osc2.frequency.setValueAtTime(160, ctx.currentTime);
        osc2.frequency.exponentialRampToValueAtTime(30, ctx.currentTime + 0.65);
...
// 4. Three.js 리얼 3D 총기 메쉬 엔진 & 3D 뷰포트
...
function init3DEngine() {
    ...
    // 카메라 전면에 붙일 1인칭 리얼 3D 총기 메쉬 그룹 생성! (Three.js 3D 지오메트리!)
    init3DGunMesh();
...
// 카메라에 1인칭 리얼 3D 총기 지오메트리 연결! (요청 완전 반영!)
function init3DGunMesh() {
    fpsGunGroup = new THREE.Group();

    // 3D 총기 조립 (Body, Barrel, Handle, Trigger, Scope)
    update3DGunModel();
...
// 무기 종류별 리얼 3D 모델 메쉬 동적 생성 (일반 총 / 저격소총 / 🚀 RPG 로켓포)
...
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (리얼 3D 사격 / 폭발 / 우오)' : '🔇 효과음 OFF';
    });
...
// 📦 집 안 보물상자 가챠 (🚀 RPG 30% / 🎯 저격소총 50% / 💣 꽝 20%)
function openHouseChestGacha() {
    ...
    resultBox.innerHTML = `<div class="roulette-spin">✨ 집 안 보물상자를 개봉하는 중... ✨</div>`;

    setTimeout(() => {
        const rand = Math.random();

        if (rand < 0.30) {
            // 30% 확률: 🚀 RPG 로켓포 (DMG 1000)
            gameState.player.weaponIdx = 2;
            resultBox.innerHTML = `
                <div class="gacha-item-title rpg">🎉 [대박! S급 3D 무기] 🚀 RPG 폭탄 로켓포! 🎉</div>
                <p style="color:#f87171; font-weight:700; margin-top:0.5rem;">공격력: 1000 | 15m 광역 폭탄 대폭발로 몬스터 몰살!!</p>
            `;
        ...
        update3DGunModel(); // 3D 총기 모델 실시간 변경!
...
// 6. 메인 렌더링 Loop (3D 총기 반동 & 이동 흔들림 애니메이션)
function animate() {
    ...
    if (p.isJumping) {
        ...
        if (camera.position.y <= eyeBaseY) {
            ...
            if (currentFloorY > 0 && !p.isOnRoof) {
                p.isOnRoof = true;
                showToast(`🏠 [지붕 무적 안전지대] ${landedHouseName} 지붕 위 착지! 몬스터 피격 0%!`);
            }
        }
    } else {
        if (camera.position.y > eyeBaseY) {
...
// 7. 1인칭 리얼 3D 총기 사격! (반동 & 3D 화염 & 3D 궤적)
function performRifleShooting() {
    const currentWep = weaponsDB[gameState.player.weaponIdx];

    // 3D 총기 피지컬 반동 애니메이션 (Three.js 3D Mesh Recoil)
    if (fpsGunGroup) {
...
function attackNearestMonster(dmg, isAoE = false) {
    ...
    if (hitCount === 0) {
        showToast(`🎯 3D 총기 사격이 허공을 갈랐습니다! (몬스터 조준하세요)`);
    } else {
        if (isAoE) {
            showToast(`🚀 [3D RPG 폭탄] 15m 광역 폭탄 대폭발!! (${hitCount}마리 몬스터 몰살!)`);
        }
        updateUI();
    }
}
