/* ==========================================================================
   1인칭 500km RPG (v11) Script
   - 🎯 무기 데미지 밸런스 조정: 저격소총 50 DMG / 🚀 RPG 로켓 80 DMG!
   - ⏱️ 시간이 지날수록 몬스터가 점차 강력해지는 실시간 난이도 강화 시스템 (HP / ATK 증가)!
   - 🚀 날아가는 3D RPG 탄두 물리 비행 & 15m 광역 화염구 대폭발!
   - 👹 고퀄리티 3D 몬스터 (고블린/독거미/해골/골렘/흑룡) & 보행 애니메이션!
...
const gameState = {
    ...
    gameTime: 0,              // ⏱️ 게임 플레이 시간 (초 단위)
    difficultyWaveLevel: 1,   // 👿 몬스터 강화 단계 (15초마다 +1 레벨업!)
...
// 2. 무기 DB (요청 반영: 저격소총 50 DMG / RPG 80 DMG 로 밸런스 조정!)
const weaponsDB = [
...
const monstersDB = [
    { name: "숲속 아기 고블린", avatar: "👹", color: 0x22c55e },
    { name: "무서운 숲 독거미", avatar: "🕷️", color: 0x8b5cf6 },
    { name: "던전 해골 전사", avatar: "💀", color: 0xe2e8f0 },
    { name: "용암 지역 화염 골렘", avatar: "🗿", color: 0xef4444 },
    { name: "지옥의 흑룡 드래곤", avatar: "🐉", color: 0xf59e0b }
];
...
// 4. Three.js 리얼 3D 그래픽 엔진 (밝고 화사한 하늘 & 태양광 조명)
...
function init3DEngine() {
    ...
    // 🌌 짙은 남색과 검은색이 섞인 고품질 밤하늘 배경 (요청 반영!)
    ...
    // ⏱️ 실시간 타이머 및 몬스터 강화 루프 (1초마다 플레이 시간 갱신 / 15초마다 몬스터 스펙 레벨업!)
...
// ⏱️ 실시간 몬스터 스펙 강화 엔진 (시간이 지날수록 HP와 ATK 증가!)
function updateGameTimerAndMonsterScaling() {
    ...
    // 15초마다 몬스터 난이도 분노 단계 상승 (+1 Level)
    ...
    if (newWaveLevel > gameState.difficultyWaveLevel) {
        ...
        showToast(`👿 [몬스터 분노 중첩!] 시간 경과로 몬스터의 능력치가 상승했습니다! (HP +${extraHp} / 공격력 +${extraAtk})`);

        // 현재 맵에 존재하는 몬스터 스펙도 실시간 강화!
    ...
    document.getElementById('monster-scaling-text').textContent = `😈 강화 단계: Lv.${gameState.difficultyWaveLevel} (HP ${currentHp} / ATK ${currentAtk})`;
...
function spawnEnemyMonstersGroup(count = 5) {
    ...
    showToast(`👾 [3D 몬스터 습격] 강화된 적 5마리가 나타났습니다! (총 ${gameState.monsters.length}마리)`);
    updateUI();
}
...
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (RPG 비행 / 대폭발 / 우오)' : '🔇 효과음 OFF';
    });
...
// 📦 벽돌집 보물상자 가챠 (요청 반영: 🎯 저격 50 DMG / 🚀 RPG 80 DMG!)
...
function animate() {
    ...
    // 🚀 1. 날아가는 3D RPG 로켓 탄두 물리 시뮬레이션 (80 DMG 광역 폭발)
...
function updateUI() {
    ...
    if (gameState.monsters.length > 0) {
        ...
        document.getElementById('monster-tier').textContent = `[강화 ${gameState.difficultyWaveLevel}단계] HP ${m.hp} / ATK ${m.atk} (적:${gameState.monsters.length}마리 / 내소환수:${gameState.friendlyMinions.length}마리)`;
