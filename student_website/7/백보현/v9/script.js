/* ==========================================================================
   1인칭 500km RPG (v9) Script - 3D Grass Map & Red Brick Houses Overhaul
   - 맵 주변에 바람에 흔들리는 3D 잔디밭 & 풀숲 군락지 배치!
   - 마을 집 3채를 붉은 벽돌집(벽돌 외벽, 굴뚝, 연기, 창문, 나무문)으로 전면 리모델링!
   - 1인칭 Three.js 리얼 3D 총기 메쉬 (일반 총 / 저격소총 / 🚀 RPG 로켓포)
   - 집 안 보물상자 가챠 (🚀 RPG 30% / 🎯 저격소총 50% / 💣 꽝 20%)
   - 붉은 벽돌집 지붕 무적 안전지대 (p.isOnRoof 피격 0%)
...
// 4. Three.js 리얼 3D 그래픽 엔진 (3D 잔디밭 & 붉은 벽돌집)
...
function init3DEngine() {
    ...
    scene.fog = new THREE.FogExp2(0x0f172a, 0.032);
...
// 🌿 3D 잔디밭 & 🏡 붉은 벽돌집 3채 맵 구축 (요청 완전 반영!)
function createTownWorld() {
    townGroup = new THREE.Group();

    // 1. 잔디 지면 바닥 (푸른 잔디 텍스처 컬러)
    const groundGeo = new THREE.PlaneGeometry(40, 40);
    const groundMat = new THREE.MeshStandardMaterial({ color: 0x15803d, roughness: 0.6 });
    ...
    // 2. 맵 주변주변 3D 잔디밭 & 풀숲 군락지 생성! (요청 반영: 맵에 풀 생성!)
    const grassBladeGeo = new THREE.ConeGeometry(0.08, 0.65, 4);
    const grassMat1 = new THREE.MeshStandardMaterial({ color: 0x22c55e, roughness: 0.5 });
    const grassMat2 = new THREE.MeshStandardMaterial({ color: 0x16a34a, roughness: 0.5 });
    const grassMat3 = new THREE.MeshStandardMaterial({ color: 0x4ade80, roughness: 0.4 });
    ...
    // 3. 붉은 벽돌집 3채 리모델링 (요청 반영: 벽돌집으로 변경!)
    const brickWallMat = new THREE.MeshStandardMaterial({ color: 0xb91c1c, roughness: 0.7 }); // 붉은 벽돌 테라코타
    const stoneFoundationMat = new THREE.MeshStandardMaterial({ color: 0x475569, roughness: 0.6 }); // 하단 석조 기단
    const slateRoofMat = new THREE.MeshStandardMaterial({ color: 0x1e293b, roughness: 0.4 }); // 어두운 슬레이트 지붕 기와
    const woodDoorMat = new THREE.MeshStandardMaterial({ color: 0x78350f, roughness: 0.5 }); // 원목 문
    const glassWindowMat = new THREE.MeshStandardMaterial({ color: 0x93c5fd, roughness: 0.1, metalness: 0.5 }); // 유리창
    const chimneyMat = new THREE.MeshStandardMaterial({ color: 0x991b1b, roughness: 0.8 }); // 붉은 벽돌 굴뚝
    const chestMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, roughness: 0.1, metalness: 0.8 }); // 보물상자
    ...
    townHouses.forEach(h => {
        ...
        // 지붕 옥상 탑승 바닥판 (높이 3.0m)
        const roofFloorGeo = new THREE.BoxGeometry(4.4, 0.2, 4.4);
        const roofFloorMat = new THREE.MeshStandardMaterial({ color: 0x334155 });
...
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (3D 사격 / 폭발 / 우오)' : '🔇 효과음 OFF';
    });
...
// 📦 벽돌집 보물상자 가챠 (🚀 RPG 30% / 🎯 저격소총 50% / 💣 꽝 20%)
...
// 6. 메인 렌더링 Loop (잔디밭 살랑임 & 벽돌집 지붕 무적 판정)
function animate() {
    ...
    if (gameState.grassBlades && gameState.grassBlades.length > 0) {
        const wind = Math.sin(now * 0.003) * 0.08;
