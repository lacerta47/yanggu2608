/* ==========================================================================
   1인칭 500km 건슈팅 RPG (v4) Script
   - 시작 무기: 🔫 초보자의 권총 (시작 데미지 20! 요청 완전 반영)
   - 권총 총기 발사음 (빵!) & 반동 애니메이션 & 머즐플래시 이펙트
   - 상대 몬스터: 10초마다 5마리씩 대량 습격 (HP 100 / 공격력 25)
   - 네크로멘서 아군 소환수: HP 25 / 공격력 10 (3.0초 느린 묵직 공격)
...
const gameState = {
    player: {
        ...
        hp: 100,
        maxHp: 100,
        weaponIdx: 0, // 시작 무기: 0번 index (초보자의 권총!)
        armorIdx: 0,
        jobClass: "💀 네크로멘서",
        monstersKilled: 0,
        currentZone: 'town',
...
// 2. 무기 및 갑옷 DB (시작 무기: 🔫 초보자의 권총 - 시작 데미지 20!)
const weaponsDB = [
    { name: "초보자의 권총", icon: "🔫", dmg: 20, reqLv: 1, ultName: "연사 탄막 폭격", ultDmg: 70 }, // 요청 반영: 시작 데미지 20!
    { name: "녹슨 라이플", icon: "🔫", dmg: 35, reqLv: 5, ultName: "관통 정밀 조준사격", ultDmg: 110 },
    { name: "은빛 샷건", icon: "💥", dmg: 65, reqLv: 10, ultName: "산탄 대폭사", ultDmg: 200 },
    { name: "불꽃 미니건", icon: "🔥", dmg: 120, reqLv: 15, ultName: "화염 제동 화력발사", ultDmg: 380 },
    { name: "고대 룬의 레이저포", icon: "🔮", dmg: 200, reqLv: 20, ultName: "룬 플라즈마 폭격", ultDmg: 600 },
    { name: "용암 플라즈마 캐논", icon: "🌋", dmg: 300, reqLv: 25, ultName: "용암 핵융합 대폭발", ultDmg: 950 },
    { name: "전설의 가우스 라이플", icon: "⚡", dmg: 450, reqLv: 30, ultName: "천공의 뇌전 레일건", ultDmg: 1400 },
    { name: "빛의 신성 광선포", icon: "✨", dmg: 650, reqLv: 35, ultName: "성스러운 성광 레이저", ultDmg: 2100 },
    { name: "신화의 둠스데이 바주카", icon: "🌌", dmg: 900, reqLv: 40, ultName: "암흑 둠스데이 조준포", ultDmg: 3000 },
    { name: "신들의 제왕 신화 총기 (최종 무기)", icon: "👑", dmg: 1300, reqLv: 50, ultName: "신들의 심판 오비탈 캐논", ultDmg: 5000 }
];
...
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (총소리 / 발소리 / 우오)' : '🔇 효과음 OFF';
    });
    ...
    document.getElementById('mobile-attack-btn').addEventListener('click', performGunShooting);
...
// 7. 총기 발사 슈팅 공격! (시작 데미지 20!)
function performGunShooting() {
    ...
    setTimeout(() => muzzle.classList.add('hidden'), 150);

    const currentWep = weaponsDB[gameState.player.weaponIdx];
    let dmg = currentWep.dmg; // 시작 권총 DMG 20!
...
function openBuildingModal(type) {
    ...
    if (type === 'blacksmith') {
        ...
        body.innerHTML = `
            ...
            <p>현재 총기 공격력: <strong>${weaponsDB[gameState.player.weaponIdx].dmg}</strong></p>
