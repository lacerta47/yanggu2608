/* ==========================================================================
   1인칭 500km RPG (v7) Script - RPG Rocket Launcher & Chest Gacha Version
   - 집 안 보물상자 뽑기 시스템 (F키 / 상자 버튼):
     🚀 RPG 로켓포 (30% 확률) - 1000 DMG 15m 광역 폭탄 대폭발!
     🎯 대구경 저격소총 (50% 확률) - 500 DMG 한 방 저격!
     💣 꽝! (20% 확률)
   - 고퀄리티 전술 돌격소총 비주얼 (조준경 + 전술 총열 + 레이저 조준선)
   - 마을 집 3채 지붕 무적 안전지대 (p.isOnRoof 피격 0%)
   - WASD 시야 기준 4방향 정밀 이동 수식
...
// 마을 집 3채 좌표 및 지붕 탑승 바운딩 박스
const townHouses = [
    { x: -7, z: -8, width: 4.8, depth: 4.8, height: 3.0, name: "대장장이의 집" },
    { x: 7, z: -8, width: 4.8, depth: 4.8, height: 3.0, name: "포션 상인의 집" },
    { x: 0, z: -14, width: 4.8, depth: 4.8, height: 3.0, name: "전직 길드 회관" }
];

// 2. 무기 DB (🚀 RPG 로켓포 / 🎯 저격소총 / 🎯 고퀄리티 돌격소총)
const weaponsDB = [
    { name: "고퀄리티 전술 돌격소총", icon: "🎯", dmg: 20, isAoE: false, reqLv: 1, ultName: "연사 폭격 탄막", ultDmg: 80 },
    { name: "대구경 저격소총", icon: "🎯", dmg: 500, isAoE: false, reqLv: 1, ultName: "관통 장거리 정밀사격", ultDmg: 900 },
    { name: "🚀 RPG 폭탄 로켓포", icon: "🚀", dmg: 1000, isAoE: true, reqLv: 1, ultName: "광역 융합 핵폭발", ultDmg: 2000 },
    { name: "불꽃 파쇄 개틀링", icon: "🔥", dmg: 130, isAoE: false, reqLv: 15, ultName: "화염 난사 연타", ultDmg: 420 },
    { name: "고대 룬의 플라즈마 소총", icon: "🔮", dmg: 220, isAoE: false, reqLv: 20, ultName: "룬 플라즈마 주사", ultDmg: 650 },
    { name: "용암 플라즈마 돌격포", icon: "🌋", dmg: 330, isAoE: false, reqLv: 25, ultName: "용암 대분출 사격", ultDmg: 1050 },
    { name: "전설의 가우스 돌격소총", icon: "⚡", dmg: 500, isAoE: false, reqLv: 30, ultName: "천공의 레일건 난사", ultDmg: 1550 },
    { name: "빛의 신성 성광 라이플", icon: "✨", dmg: 720, isAoE: false, reqLv: 35, ultName: "성스러운 광휘 빔 사격", ultDmg: 2300 },
    { name: "신화의 둠스데이 중소총", icon: "🌌", dmg: 980, isAoE: false, reqLv: 40, ultName: "암흑 소멸 포탄 발사", ultDmg: 3300 },
    { name: "신들의 제왕 신화 소총", icon: "👑", dmg: 1400, isAoE: true, reqLv: 50, ultName: "신들의 종말 오비탈 캐논", ultDmg: 5500 }
];
...
// 3. Web Audio Synth (RPG 폭격음 & 소총 연사음)
let audioCtx = null;
...
function playRPGBoomSound() {
    ...
    try {
        ...
        osc2.frequency.exponentialRampToValueAtTime(30, ctx.currentTime + 0.6);
...
function createTownWorld() {
    ...
    const houseMat = new THREE.MeshStandardMaterial({ color: 0xb45309, roughness: 0.4 });
    const roofMat = new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.2 });
    const chestMat = new THREE.MeshStandardMaterial({ color: 0xf59e0b, roughness: 0.1 });
    ...
    townHouses.forEach(h => {
        ...
        const roofFloorMat = new THREE.MeshStandardMaterial({ color: 0x92400e });
        const roofFloor = new THREE.Mesh(roofFloorGeo, roofFloorMat);
        roofFloor.position.set(h.x, 3.0, h.z);
        townGroup.add(roofFloor);

        const roof = new THREE.Mesh(new THREE.ConeGeometry(3.2, 1.6, 4), roofMat);
        ...
        chest.position.set(h.x, 0.4, h.z + 2.2);
        townGroup.add(chest);
    });
...
// 5. 키보드 입력 수신 엔진 (F키: 집 안 보물상자 개봉 추가!)
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (RPG 폭격 / 저격소총 / 우오)' : '🔇 효과음 OFF';
    });
...
// 📦 집 안 보물상자 뽑기 룰렛 (요청 완전 반영!)
// RPG 로켓포: 30% 확률 | 저격소총: 50% 확률 | 꽝: 20% 확률
function openHouseChestGacha() {
    ...
    resultBox.innerHTML = `<div class="roulette-spin">✨ 집 안 보물상자를 뽑는 중입니다... ✨</div>`;

    setTimeout(() => {
        const rand = Math.random(); // 0.0 ~ 1.0

        if (rand < 0.30) {
            // 30% 확률: 🚀 RPG 로켓포 (DMG 1000, 15m 광역 대폭발!)
            gameState.player.weaponIdx = 2; // RPG 로켓포
            resultBox.innerHTML = `
                <div class="gacha-item-title rpg">🎉 [대박! S급 획득] 🚀 RPG 폭탄 로켓포! 🎉</div>
                <p style="color:#f87171; font-weight:700; margin-top:0.5rem;">공격력: 1000 | 15m 광역 폭탄 폭발로 몬스터 한 번에 전멸!!</p>
            `;
            showToast('🚀 [S급 획득!] 30% 확률의 RPG 폭탄 로켓포 장착!');
        } else if (rand < 0.80) {
            // 50% 확률: 🎯 대구경 저격소총 (DMG 500)
            gameState.player.weaponIdx = 1; // 저격소총
            resultBox.innerHTML = `
                <div class="gacha-item-title sniper">✨ [A급 획득] 🎯 대구경 저격소총! ✨</div>
                <p style="color:#38bdf8; font-weight:700; margin-top:0.5rem;">공격력: 500 | 강력한 장거리 저격 사격!</p>
            `;
            showToast('🎯 [A급 획득!] 50% 확률의 대구경 저격소총 장착!');
        } else {
            // 20% 확률: 💣 꽝!
            resultBox.innerHTML = `
                <div class="gacha-item-title dud">💨 [꽝!] 상자 안이 비어있습니다! 💣</div>
                <p style="color:#94a3b8; font-weight:700; margin-top:0.5rem;">상자에서 먼지만 폴폴 피어오릅니다... 다음 기회에!</p>
            `;
...
function animate() {
    ...
    if (p.isJumping) {
        ...
        if (camera.position.y <= eyeBaseY) {
            ...
            if (currentFloorY > 0 && !p.isOnRoof) {
                p.isOnRoof = true;
                showToast(`🏠 [지붕 무적 안전지대] ${landedHouseName} 지붕 위 착지! 몬스터 공격 100% 무효화!`);
            }
        }
    } else {
        if (camera.position.y > eyeBaseY) {
        ...
        if (currentFloorY === 0 && p.isOnRoof) {
            p.isOnRoof = false;
            showToast('🍃 지붕에서 지면으로 내려왔습니다.');
        }
    }

    // WASD 이동 (W:앞/S:뒤/A:왼쪽/D:오른쪽)
    ...
    const viewmodelRifle = document.getElementById('viewmodel-rifle');
    if (isMoving && !p.isJumping) {
        playFootstepSound();
        const bob = Math.sin(Date.now() * 0.015) * 8;
        if (viewmodelRifle) viewmodelRifle.style.transform = `translateY(${bob}px)`;
    } else if (!isMoving) {
        if (viewmodelRifle) viewmodelRifle.style.transform = `translateY(0px)`;
    }

    const now = Date.now();

    // 적 몬스터 AI (지붕 위 무적 안전지대 판정 포함)
...
// 7. 총기 사격 (🚀 RPG 로켓포일 경우 1000 DMG 15m 광역 대폭발사격!)
function performRifleShooting() {
    const weaponView = document.getElementById('viewmodel-rifle');
    const muzzle = document.getElementById('muzzle-flash');

    if (weaponView) {
        weaponView.classList.remove('shootRecoil');
        void weaponView.offsetWidth;
        weaponView.classList.add('shootRecoil');
    }

    const currentWep = weaponsDB[gameState.player.weaponIdx];

    if (currentWep.icon === "🚀") {
        playRPGBoomSound(); // RPG 폭격음
        if (muzzle) {
            muzzle.textContent = "💥 🚀 RPG 콰쾅!!";
            muzzle.classList.remove('hidden');
            setTimeout(() => muzzle.classList.add('hidden'), 200);
        }
    } else {
        playRifleShotSound();
        if (muzzle) {
            muzzle.textContent = "💥 연사!";
            muzzle.classList.remove('hidden');
            setTimeout(() => muzzle.classList.add('hidden'), 100);
        }
    }
    ...
    attackNearestMonster(dmg, currentWep.isAoE);
}
...
// 몬스터 공격 (🚀 RPG 로켓포는 15m 광역 대폭발로 몬스터 한 번에 전멸!)
function attackNearestMonster(dmg, isAoE = false) {
    ...
    let hitCount = 0;
    const range = isAoE ? 15 : 9; // RPG 로켓포 15m 대폭발!
    ...
    for (let i = gameState.monsters.length - 1; i >= 0; i--) {
        ...
        if (dist <= range) {
            ...
            if (!isAoE) break; // 단일 사격 시 1마리만 공격
        }
    }

    if (hitCount === 0) {
        showToast(`🎯 탄환이 허공을 갈랐습니다! (몬스터에게 조준하세요)`);
    } else {
        if (isAoE) {
            showToast(`🚀 [RPG 폭탄 로켓포] 15m 광역 폭탄 대폭발!! (${hitCount}마리 몬스터 몰살!)`);
        }
        updateUI();
    }
}
...
function openBuildingModal(type) {
    ...
    if (type === 'blacksmith') {
        ...
        body.innerHTML = `
            ...
            <p>현재 무기 화력: <strong>${weaponsDB[gameState.player.weaponIdx].dmg}</strong></p>
            <button id="upgrade-weapon-btn" class="shop-item-btn">🔨 총기 화력 추가 강화 (+10 공격력)</button>
        `;
        document.getElementById('upgrade-weapon-btn').addEventListener('click', () => {
            weaponsDB[gameState.player.weaponIdx].dmg += 10;
            showToast('✨ 총기 공격력이 +10 강화되었습니다!');
            updateUI();
...
function updateUI() {
    ...
    const vmIcon = document.getElementById('vm-icon');
    const vmName = document.getElementById('vm-name');
    if (vmIcon) vmIcon.textContent = weapon.icon;
    if (vmName) vmName.textContent = weapon.name;
