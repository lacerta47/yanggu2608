/* ==========================================================================
   👹 몬스터 아레나 (Monster Arena) v12 Script
   - 👹 무시무시하고 흉측한 3D 몬스터 업그레이드 (핏빛 눈동자/날카로운 엄니/가시 무기/기괴한 보행)!
   - 🎮 게임 이름 "몬스터 아레나 (Monster Arena)" 정식 적용!
   - 🌌 짙은 남색 + 검은색 딥 네이비 배경 (0x0b1329) & 화사한 메인 조명!
   - 🎯 밸런스 조정: 저격소총 50 DMG / 🚀 RPG 로켓 80 DMG (3D 비행 & 15m 광역 대폭발)!
   - ⏱️ 실시간 몬스터 스펙 레벨업 난이도 강화 엔진!
...
// 2. 무기 DB (저격소총 50 DMG / RPG 80 DMG)
const weaponsDB = [
    { name: "리얼 3D 권총 (일반 총)", icon: "🔫", dmg: 20, type: "pistol", isAoE: false, reqLv: 1, ultName: "연사 폭격 탄막", ultDmg: 60 },
    { name: "대구경 저격소총", icon: "🎯", dmg: 50, type: "sniper", isAoE: false, reqLv: 1, ultName: "관통 장거리 정밀사격", ultDmg: 120 },
    { name: "🚀 RPG 폭탄 로켓포", icon: "🚀", dmg: 80, type: "rpg", isAoE: true, reqLv: 1, ultName: "광역 융합 핵폭발", ultDmg: 200 },
    { name: "불꽃 파쇄 개틀링", icon: "🔥", dmg: 95, type: "pistol", isAoE: false, reqLv: 15, ultName: "화염 난사 연타", ultDmg: 250 },
    { name: "고대 룬의 플라즈마 소총", icon: "🔮", dmg: 120, type: "sniper", isAoE: false, reqLv: 20, ultName: "룬 플라즈마 주사", ultDmg: 320 },
    { name: "용암 플라즈마 돌격포", icon: "🌋", dmg: 150, type: "rpg", isAoE: true, reqLv: 25, ultName: "용암 대분출 사격", ultDmg: 400 },
    { name: "전설의 가우스 돌격소총", icon: "⚡", dmg: 200, type: "sniper", isAoE: false, reqLv: 30, ultName: "천공의 레일건 난사", ultDmg: 550 },
    { name: "빛의 신성 성광 라이플", icon: "✨", dmg: 280, type: "sniper", isAoE: false, reqLv: 35, ultName: "성스러운 광휘 빔 사격", ultDmg: 700 },
    { name: "신화의 둠스데이 중소총", icon: "🌌", dmg: 380, type: "rpg", isAoE: true, reqLv: 40, ultName: "암흑 소멸 포탄 발사", ultDmg: 950 },
    { name: "신들의 제왕 신화 소총", icon: "👑", dmg: 500, type: "rpg", isAoE: true, reqLv: 50, ultName: "신들의 종말 오비탈 캐논", ultDmg: 1500 }
];
...
// 무시무시한 괴수 몬스터 DB (요청 반영: 몬스터 퀄리티 및 테러 분위기 대폭 강화!)
const monstersDB = [
    { name: "피에 굶주린 오크 대장", avatar: "👹", color: 0xdc2626 },
    { name: "심해 독거미 마왕", avatar: "🕷️", color: 0x7e22ce },
    { name: "저주받은 아포칼립스 해골 처형자", avatar: "💀", color: 0x64748b },
    { name: "심연 마그마 골렘", avatar: "🗿", color: 0xb91c1c },
    { name: "지옥의 둠스데이 흑룡", avatar: "🐉", color: 0x111827 }
];
...
// 4. Three.js 리얼 3D 그래픽 엔진 (짙은 남색 + 검은색 0x0b1329 배경 적용!)
...
function init3DEngine() {
    ...
    // 🌌 요청 반영: 뒤 배경을 완전히 남색과 검은색이 섞인 0x0b1329 딥 네이비로 설정!
    scene.background = new THREE.Color(0x0b1329);
    scene.fog = new THREE.Fog(0x0b1329, 30, 95);
    ...
    hemiLight = new THREE.HemisphereLight(0x38bdf8, 0x16a34a, 1.1);
    scene.add(hemiLight);
    ...
    setInterval(updateGameTimerAndMonsterScaling, 1000);
...
// ⏱️ 실시간 몬스터 강화 타이머
function updateGameTimerAndMonsterScaling() {
    gameState.gameTime++;

    const minutes = Math.floor(gameState.gameTime / 60);
    const seconds = gameState.gameTime % 60;
    const timeStr = `⏱️ ${String(minutes).padStart(2, '0')}:${String(seconds).padStart(2, '0')}`;
    
    document.getElementById('game-timer-text').textContent = timeStr;

    const newWaveLevel = 1 + Math.floor(gameState.gameTime / 15);
    if (newWaveLevel > gameState.difficultyWaveLevel) {
        gameState.difficultyWaveLevel = newWaveLevel;
        
        const extraHp = (newWaveLevel - 1) * 20;
        const extraAtk = (newWaveLevel - 1) * 5;

        showToast(`👹 [몬스터 광기 발작!] 몬스터 군단이 더욱 무시무시해졌습니다! (HP +${extraHp} / 공격력 +${extraAtk})`);

        gameState.monsters.forEach(m => {
            m.maxHp = 100 + extraHp;
            m.hp = Math.min(m.hp + 20, m.maxHp);
            m.atk = 25 + extraAtk;
        });
    }

    const currentHp = 100 + (gameState.difficultyWaveLevel - 1) * 20;
    const currentAtk = 25 + (gameState.difficultyWaveLevel - 1) * 5;
    document.getElementById('monster-scaling-text').textContent = `👹 무시무시한 몬스터: Lv.${gameState.difficultyWaveLevel} (HP ${currentHp} / ATK ${currentAtk})`;
...
function createTownWorld() {
    ...
    const brickWallMat = new THREE.MeshStandardMaterial({ color: 0xdc2626, roughness: 0.6 });
...
function spawnEnemyMonstersGroup(count = 5) {
    ...
    showToast(`👹 [몬스터 아레나] 무시무시한 3D 괴수 5마리가 나타났습니다! (총 ${gameState.monsters.length}마리)`);
    updateUI();
}

// 👹 무시무시하고 흉측한 3D 몬스터 조립 생성기 (요청 반영!)
function createHighQualityMonsterModel(tier) {
    const group = new THREE.Group();

    if (tier === 0) {
        // 👹 1단계: 피에 굶주린 오크 버서커 (붉은 핏빛 눈동자, 뾰족한 밑엄니, 가시 오크 망치)
        const skinMat = new THREE.MeshStandardMaterial({ color: 0x15803d, roughness: 0.3 });
        const bloodClothMat = new THREE.MeshStandardMaterial({ color: 0x991b1b });
        const redEyeMat = new THREE.MeshBasicMaterial({ color: 0xff0000 });
        const fangMat = new THREE.MeshStandardMaterial({ color: 0xfffbebe, roughness: 0.2 });
        const steelSpikeMat = new THREE.MeshStandardMaterial({ color: 0x334155, metalness: 0.9 });

        const head = new THREE.Mesh(new THREE.SphereGeometry(0.6, 12, 12), skinMat);
        head.position.y = 1.35;
        head.name = "monsterBody";
        group.add(head);

        // 무서운 밑엄니 (위로 솟구친 날카로운 이빨)
        const fang1 = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.35, 4), fangMat);
        fang1.rotation.x = Math.PI;
        fang1.position.set(-0.2, 1.25, 0.52);
        const fang2 = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.35, 4), fangMat);
        fang2.rotation.x = Math.PI;
        fang2.position.set(0.2, 1.25, 0.52);
        group.add(fang1);
        group.add(fang2);

        // 붉게 이글거리는 핏빛 눈
        const eye1 = new THREE.Mesh(new THREE.SphereGeometry(0.12, 8, 8), redEyeMat);
        eye1.position.set(-0.22, 1.42, 0.52);
        const eye2 = new THREE.Mesh(new THREE.SphereGeometry(0.12, 8, 8), redEyeMat);
        eye2.position.set(0.22, 1.42, 0.52);
        group.add(eye1);
        group.add(eye2);

        // 근육질 몸통 & 가시 어깨 갑옷
        const torso = new THREE.Mesh(new THREE.CylinderGeometry(0.45, 0.5, 0.8, 8), bloodClothMat);
        torso.position.y = 0.75;
        group.add(torso);

        const shoulderSpike = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.5, 4), steelSpikeMat);
        shoulderSpike.position.set(-0.55, 1.1, 0);
        shoulderSpike.rotation.z = Math.PI / 3;
        group.add(shoulderSpike);

        // 가시 오크 망치 무기
        const hammerHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.06, 0.06, 1.2, 8), steelSpikeMat);
        hammerHandle.position.set(0.65, 0.9, 0.3);
        hammerHandle.rotation.x = Math.PI / 3;
        const hammerHead = new THREE.Mesh(new THREE.BoxGeometry(0.35, 0.35, 0.5), steelSpikeMat);
        hammerHead.position.set(0.65, 1.3, 0.6);
        group.add(hammerHandle);
        group.add(hammerHead);

        const leg1 = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.5), skinMat);
        leg1.position.set(-0.22, 0.25, 0);
        const leg2 = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.12, 0.5), skinMat);
        leg2.position.set(0.22, 0.25, 0);
        group.add(leg1);
        group.add(leg2);
        group.userData = { type: 'biped', leg1, leg2 };
    } else if (tier === 1) {
        // 🕷️ 2단계: 심해 독거미 마왕 (검붉은 기괴한 몸통, 8개 징그러운 갈고리 다리)
        const spiderMat = new THREE.MeshStandardMaterial({ color: 0x4c1d95, roughness: 0.2 });
        const darkBodyMat = new THREE.MeshStandardMaterial({ color: 0x0f172a });
        const redEyeMat = new THREE.MeshBasicMaterial({ color: 0xef4444 });
        const fangMat = new THREE.MeshStandardMaterial({ color: 0xd97706, roughness: 0.1 });

        const body = new THREE.Mesh(new THREE.SphereGeometry(0.6, 12, 12), spiderMat);
        body.position.set(0, 0.6, 0.4);
        body.name = "monsterBody";
        group.add(body);

        const head = new THREE.Mesh(new THREE.SphereGeometry(0.4, 8, 8), darkBodyMat);
        head.position.set(0, 0.45, -0.45);
        group.add(head);

        // 징그러운 독 집게 턱
        const fang1 = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.4, 4), fangMat);
        fang1.rotation.x = Math.PI / 2;
        fang1.position.set(-0.15, 0.3, -0.8);
        const fang2 = new THREE.Mesh(new THREE.ConeGeometry(0.08, 0.4, 4), fangMat);
        fang2.rotation.x = Math.PI / 2;
        fang2.position.set(0.15, 0.3, -0.8);
        group.add(fang1);
        group.add(fang2);
        ...
        for (let e = -3; e <= 3; e++) {
            if (e === 0) continue;
            const eye = new THREE.Mesh(new THREE.SphereGeometry(0.06, 6, 6), redEyeMat);
            eye.position.set(e * 0.08, 0.55, -0.75);
            group.add(eye);
        }
        ...
        for (let l = 0; l < 8; l++) {
            ...
            const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.04, 0.02, 1.1), darkBodyMat);
            leg.position.set(side * 0.55, 0.4, (idx - 1.5) * 0.35);
        ...
        // 💀 3단계: 아포칼립스 해골 처형자 (보랏빛 이글거리는 두개골, 거대한 곡선 처형 낫)
        const boneMat = new THREE.MeshStandardMaterial({ color: 0xf1f5f9, roughness: 0.4 });
        const darkSteelMat = new THREE.MeshStandardMaterial({ color: 0x0f172a, metalness: 0.9 });
        const purpleEyeMat = new THREE.MeshBasicMaterial({ color: 0xa855f7 });

        const skull = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.6, 0.55), boneMat);
        skull.position.y = 1.4;
        skull.name = "monsterBody";
        group.add(skull);

        const eye1 = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, 0.1), purpleEyeMat);
        eye1.position.set(-0.15, 1.45, 0.29);
        const eye2 = new THREE.Mesh(new THREE.BoxGeometry(0.14, 0.14, 0.1), purpleEyeMat);
        eye2.position.set(0.15, 1.45, 0.29);
        group.add(eye1);
        group.add(eye2);

        const rib = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.7, 0.4), boneMat);
        rib.position.y = 0.75;
        group.add(rib);

        // 거대한 사신의 처형 낫
        const scytheHandle = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.05, 1.6), darkSteelMat);
        scytheHandle.position.set(0.55, 1.0, 0.1);
        scytheHandle.rotation.z = -Math.PI / 8;
        const scytheBlade = new THREE.Mesh(new THREE.ConeGeometry(0.3, 0.9, 4), darkSteelMat);
        scytheBlade.position.set(0.85, 1.7, 0.1);
        scytheBlade.rotation.z = Math.PI / 2;
        group.add(scytheHandle);
        group.add(scytheBlade);

        const leg1 = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.5), boneMat);
        leg1.position.set(-0.2, 0.25, 0);
        const leg2 = new THREE.Mesh(new THREE.CylinderGeometry(0.08, 0.08, 0.5), boneMat);
        leg2.position.set(0.2, 0.25, 0);
        group.add(leg1);
        group.add(leg2);
        group.userData = { type: 'biped', leg1, leg2 };
    } else {
        // 🔥/🐉 4~5단계: 심연 마그마 골렘 / 둠스데이 흑룡 보스
        const magmaMat = new THREE.MeshStandardMaterial({ color: 0xef4444, emissive: 0x991b1b, roughness: 0.1 });
        const blackRockMat = new THREE.MeshStandardMaterial({ color: 0x090d16, roughness: 0.9 });
        const redEyeMat = new THREE.MeshBasicMaterial({ color: 0xff0000 });

        const core = new THREE.Mesh(new THREE.DodecahedronGeometry(0.75), magmaMat);
        core.position.y = 1.3;
        core.name = "monsterBody";
        group.add(core);

        const head = new THREE.Mesh(new THREE.BoxGeometry(0.55, 0.45, 0.65), blackRockMat);
        head.position.set(0, 1.9, 0.25);
        group.add(head);

        const eye1 = new THREE.Mesh(new THREE.SphereGeometry(0.1, 6, 6), redEyeMat);
        eye1.position.set(-0.18, 1.95, 0.58);
        const eye2 = new THREE.Mesh(new THREE.SphereGeometry(0.1, 6, 6), redEyeMat);
        eye2.position.set(0.18, 1.95, 0.58);
        group.add(eye1);
        group.add(eye2);

        const horn1 = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.6, 4), magmaMat);
        horn1.position.set(-0.3, 2.2, 0.1);
        horn1.rotation.z = Math.PI / 5;
        const horn2 = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.6, 4), magmaMat);
        horn2.position.set(0.3, 2.2, 0.1);
        horn2.rotation.z = -Math.PI / 5;
        group.add(horn1);
        group.add(horn2);

        const wing1 = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.06, 0.7), magmaMat);
        wing1.position.set(-1.1, 1.5, 0);
        const wing2 = new THREE.Mesh(new THREE.BoxGeometry(1.4, 0.06, 0.7), magmaMat);
        wing2.position.set(1.1, 1.5, 0);
        group.add(wing1);
        group.add(wing2);
...
function spawnSingleEnemyMonster() {
    ...
    const sizeScale = 1.0 + (gameState.difficultyWaveLevel - 1) * 0.05;
    ...
    const baseHp = 100 + (gameState.difficultyWaveLevel - 1) * 20;
    const baseAtk = 25 + (gameState.difficultyWaveLevel - 1) * 5;
...
function initControls() {
    ...
    document.getElementById('sound-btn').addEventListener('click', () => {
        ...
        btn.textContent = gameState.soundEnabled ? '🔊 효과음 ON (괴수 포효 / 로켓 비행 / 대폭발)' : '🔇 효과음 OFF';
    });
...
function openHouseChestGacha() {
    ...
    setTimeout(() => {
        ...
        if (rand < 0.30) {
            ...
            resultBox.innerHTML = `
                <div class="gacha-item-title rpg">🎉 [S급 획득] 🚀 RPG 폭탄 로켓포! 🎉</div>
                <p style="color:#f87171; font-weight:700; margin-top:0.5rem;">공격력: 80 | 3D 비행 로켓 탄두 & 15m 광역 폭발!</p>
            `;
            showToast('🚀 [S급 획득!] 30% 확률의 🚀 RPG 폭탄 로켓포 (80 DMG) 장착!');
        } else if (rand < 0.80) {
            gameState.player.weaponIdx = 1; // 🎯 저격소총
            resultBox.innerHTML = `
                <div class="gacha-item-title sniper">✨ [A급 획득] 🎯 대구경 저격소총! ✨</div>
                <p style="color:#38bdf8; font-weight:700; margin-top:0.5rem;">공격력: 50 | 3D 레이저 조준선 단일 저격 사격!</p>
            `;
            showToast('🎯 [A급 획득!] 50% 확률의 🎯 대구경 저격소총 (50 DMG) 장착!');
        } else {
            resultBox.innerHTML = `
    ...
    for (let i = gameState.activeProjectiles.length - 1; i >= 0; i--) {
        ...
        for (let m of gameState.monsters) {
            if (proj.mesh.position.distanceTo(m.mesh.position) < 1.6) {
                trigger3DRPGExplosion(proj.mesh.position.x, proj.mesh.position.y, proj.mesh.position.z, 80);
                exploded = true;
                break;
            }
        }
        ...
        if (!exploded && (proj.mesh.position.y <= 0.4 || proj.life <= 0)) {
            trigger3DRPGExplosion(proj.mesh.position.x, 0.4, proj.mesh.position.z, 80);
            exploded = true;
        }
    ...
    gameState.monsters.forEach(m => {
        ...
        if (minDistance <= 2.2 && (now - m.lastAttackTime > 1500)) {
            ...
            if (targetPos.isPlayer) {
                ...
                showToast(`👹 [${m.name}] 이(가) 내 소환수를 물어뜯었습니다! (-${m.atk} HP)`);
...
function spawnFlyingRPGRocket() {
    ...
    showToast('🚀 [RPG 발사!] 로켓 탄두가 날아가며 80 광역 피해를 줍니다!');
}

function trigger3DRPGExplosion(x, y, z, dmg = 80) {
    playRPGBoomSound();
    ...
    for (let i = gameState.monsters.length - 1; i >= 0; i--) {
        ...
        if (dist <= 15) {
            ...
            if (m.hp <= 0) {
                ...
                gameState.player.monstersKilled++;
                showToast(`💥 무시무시한 몬스터 처치! 경험치 +100 EXP 획득!`);
                addExp(100);
    ...
    showToast(`💥 [RPG 대폭발!] 15m 범위 내 ${hitCount}마리 몬스터에 ${dmg} 광역 피격!`);
    updateUI();
}
...
function onPlayerHit(damage, monsterName) {
    ...
    showToast(`👹 [${monsterName}]의 흉측한 공격에 피격되었습니다! (-${finalDmg} HP) [크아악!!]`);
...
function onLevelUp(newLevel) {
    showToast(`🎉 레벨업! 아레나 챔피언 레벨 ${newLevel} 달성!`);
...
function setZone(zone) {
    ...
    if (zone === 'town') {
        ...
        badge.textContent = '🌋 몬스터 아레나 괴수 소굴';
        badge.className = 'zone-badge danger';
        btn.textContent = '🏡 평화로운 벽돌집 마을 귀환';
        miniTxt.textContent = '몬스터 아레나';
        townGroup.visible = false;
        dangerGroup.visible = true;
        scene.background = new THREE.Color(0x180505);
        showToast('🌋 몬스터 아레나 괴수 소굴에 진입했습니다!');
    }
}
...
function updateUI() {
    ...
    if (gameState.monsters.length > 0) {
        ...
        document.getElementById('monster-tier').textContent = `[아레나 ${gameState.difficultyWaveLevel}단계] HP ${m.hp} / ATK ${m.atk} (적:${gameState.monsters.length}마리 / 내소환수:${gameState.friendlyMinions.length}마리)`;
        ...
        document.getElementById('monster-name').textContent = '아레나 괴수 탐색 중...';
...
document.getElementById('restart-btn').addEventListener('click', () => {
    ...
    gameState.difficultyWaveLevel = 1;
