/* ==========================================================================
   1인칭 500km RPG (v6) Script
   - 지붕 무적 안전지대: 집 옥상 지붕(p.isOnRoof)에 올라가면 아래 몬스터가 절대로 때릴 수 없음!
   - 1인칭 전술 돌격소총 뷰모델 & 사격 반동 연출 (시작 데미지 20)
   - WASD 키 수신 보장 (W:앞/S:뒤/A:왼쪽/D:오른쪽)
...
const gameState = {
    player: {
        ...
        hp: 100,
        maxHp: 100,
        weaponIdx: 0, // 시작 무기: 초보자의 돌격소총 (공격력 20)
        armorIdx: 0,
        jobClass: "💀 네크로멘서",
        monstersKilled: 0,
        currentZone: 'town',
...
const weaponsDB = [
    { name: "초보자의 돌격소총", icon: "🎯", dmg: 20, reqLv: 1, ultName: "연사 폭격 탄막", ultDmg: 80 },
...
// 6. 메인 렌더링 Loop (지붕 무적 안전지대 판정 및 이동)
function animate() {
    ...
    if (p.currentZone === 'town') {
        townHouses.forEach(h => {
            ...
            if (dx <= h.width / 2 && dz <= h.depth / 2) {
                currentFloorY = h.height; // 지붕 높이 3.0m!
    ...
    if (p.isJumping) {
        ...
        if (camera.position.y <= eyeBaseY) {
            ...
            if (currentFloorY > 0 && !p.isOnRoof) {
                p.isOnRoof = true;
                showToast(`🏠 [지붕 무적 안전지대 성공!] ${landedHouseName} 지붕 위에 올라탔습니다! 몬스터 공격 100% 무효화!`);
            }
        }
    } else {
        if (camera.position.y > eyeBaseY) {
        ...
        if (currentFloorY === 0 && p.isOnRoof) {
            p.isOnRoof = false;
            showToast('🍃 지붕에서 지면으로 내려왔습니다. (몬스터 공격 주의!)');
        }
    }

    // 시야 방향 벡터 기반 WASD 이동 (W:앞/S:뒤/A:왼쪽/D:오른쪽)
    ...
    // 적 몬스터 AI (플레이어가 지붕 위에 있으면 몬스터가 플레이어를 절대로 때릴 수 없음!) (요청 반영)
...
function performRifleShooting() {
    const weapon = document.getElementById('viewmodel-rifle');
    ...
    let dmg = currentWep.dmg; // 시작 소총 데미지 20!
...
function attackNearestMonster(dmg, isAoE = false) {
    ...
    for (let i = gameState.monsters.length - 1; i >= 0; i--) {
        ...
        const range = isAoE ? 10 : 9; // 소총 사거리 9m
        if (dist <= range) {
            m.hp -= dmg;
            hitCount++;
    ...
    if (hitCount === 0) {
        showToast(`🎯 돌격소총 탄환이 허공을 갈랐습니다! (몬스터에게 조준하세요)`);
    } else {
        updateUI();
    }
}
