...
const monologues = [
    ...
    "정성스레 빚어낸 온기가 누군가의 고단한 하루에 작은 위로가 되길 바란다. 오늘도 감사함으로 하루를 마무리한다."
];
...
function loadGameData() {
    const saved = localStorage.getItem('season_workshop_save_v10');
    if (saved) {
        try {
...
function saveGameData() {
    localStorage.setItem('season_workshop_save_v10', JSON.stringify({ day: gameState.day, warmth: gameState.warmth }));
}
...
// ----------------------------------------------------
// THREE.JS 그림자 위 100% 선명한 3D 사람 인형 렌더링
// ----------------------------------------------------
let scene, camera, renderer;
let artisanGroup, leftArm, rightArm, leftLeg, rightLeg, headMesh;
let customerGroup, customerBodyMesh;
let candleLight, kilnFireLight;
let is3DInit = false;

const player3D = { x: 0, z: 2, speed: 0.12 };
const stations3D = [
    { id: 'door', name: '🚪 주문 손님 인형과 대화하기', x: -6, z: -2, radius: 2.2 },
...
function initv10Workshop() {
    ...
    const hemiLight = new THREE.HemisphereLight(0xffffff, 0x553388, 1.3);
    scene.add(hemiLight);
    ...
    // 1. 만드는 사람 (그림자 위 100% 또렷한 3D 장인 인형)
    buildArtisan3DDollOnShadow();

    // 2. 주문하는 사람 (그림자 위 100% 또렷한 3D 손님 인형)
    buildCustomer3DDollOnShadow();
    ...
    if (promptEl) {
        promptEl.addEventListener('click', triggerStationInteract3D);
    }
...
// 🧸 그림자 판 바로 위에 100% 선명하게 서 있는 3D 입체 장인 피규어 인형
function buildArtisan3DDollOnShadow() {
    artisanGroup = new THREE.Group();
    ...
    artisanGroup.add(shadowMesh);

    // 2. 그림자 바로 위에 피트되어 서 있는 3D 인형 캐릭터 (Artisan Figurine)
    // 머리
    ...
    hat.position.y = 1.95;
    artisanGroup.add(hat);

    // 눈 두 개
    const eyeGeo = new THREE.SphereGeometry(0.065, 8, 8);
    const eyeMat = new THREE.MeshBasicMaterial({ color: 0x1e1035 });
    const eyeL = new THREE.Mesh(eyeGeo, eyeMat); eyeL.position.set(-0.16, 1.65, 0.45);
    const eyeR = new THREE.Mesh(eyeGeo, eyeMat); eyeR.position.set(0.16, 1.65, 0.45);
    artisanGroup.add(eyeL); artisanGroup.add(eyeR);

    // 몸통 & 앞치마
    const bodyGeo = new THREE.CylinderGeometry(0.4, 0.45, 0.8, 16);
    const bodyMat = new THREE.MeshStandardMaterial({ color: 0xc084fc, roughness: 0.5 });
    const body = new THREE.Mesh(bodyGeo, bodyMat);
    body.position.y = 0.95;
    artisanGroup.add(body);

    const apronGeo = new THREE.BoxGeometry(0.52, 0.62, 0.12);
    const apronMat = new THREE.MeshStandardMaterial({ color: 0xfaf5ff });
    const apron = new THREE.Mesh(apronGeo, apronMat);
    apron.position.set(0, 0.9, 0.25);
    artisanGroup.add(apron);

    // 관절 팔
    const armGeo = new THREE.CylinderGeometry(0.1, 0.1, 0.5, 8);
    const armMat = new THREE.MeshStandardMaterial({ color: 0xc084fc });
    leftArm = new THREE.Mesh(armGeo, armMat); leftArm.position.set(-0.55, 1.05, 0);
    rightArm = new THREE.Mesh(armGeo, armMat); rightArm.position.set(0.55, 1.05, 0);
    artisanGroup.add(leftArm); artisanGroup.add(rightArm);

    // 관절 다리 (발끝이 그림자 판 바로 위 y:0.02 에 위치!)
    const legGeo = new THREE.CylinderGeometry(0.12, 0.12, 0.5, 8);
    const legMat = new THREE.MeshStandardMaterial({ color: 0x4c1d95 });
    leftLeg = new THREE.Mesh(legGeo, legMat); leftLeg.position.set(-0.2, 0.25, 0);
    rightLeg = new THREE.Mesh(legGeo, legMat); rightLeg.position.set(0.2, 0.25, 0);
    artisanGroup.add(leftLeg); artisanGroup.add(rightLeg);

    artisanGroup.position.set(player3D.x, 0, player3D.z);
    scene.add(artisanGroup);
}

// 🧸 그림자 판 바로 위에 서 있는 3D 손님 인형
function buildCustomer3DDollOnShadow() {
    ...
    shadowMesh.rotation.x = -Math.PI / 2;
    shadowMesh.position.set(0, 0.01, 0);
    customerGroup.add(shadowMesh);

    // 2. 3D 손님 피규어 인형
    const headGeo = new THREE.SphereGeometry(0.45, 24, 24);
    const headMat = new THREE.MeshStandardMaterial({ color: 0xfed7aa });
    const cHead = new THREE.Mesh(headGeo, headMat);
    cHead.position.y = 1.5;
    customerGroup.add(cHead);

    // 손님 빨간 코트 몸통
    customerBodyMesh = new THREE.Mesh(new THREE.CylinderGeometry(0.35, 0.4, 0.8, 16), new THREE.MeshStandardMaterial({ color: 0xe11d48 }));
    customerBodyMesh.position.y = 0.9;
    customerGroup.add(customerBodyMesh);

    // 손님 다리
    const legGeo = new THREE.CylinderGeometry(0.1, 0.1, 0.5, 8);
    const legMat = new THREE.MeshStandardMaterial({ color: 0x4c1d95 });
    const cLegL = new THREE.Mesh(legGeo, legMat); cLegL.position.set(-0.18, 0.25, 0);
    const cLegR = new THREE.Mesh(legGeo, legMat); cLegR.position.set(0.18, 0.25, 0);
    customerGroup.add(cLegL); customerGroup.add(cLegR);

    customerGroup.position.set(-6, 0, -1.8);
    scene.add(customerGroup);
}

function updateCustomerAppearance() {
    if (!customerBodyMesh || !gameState.currentOrder) return;
    const colors = [0xe11d48, 0x0284c7, 0x16a34a, 0x9333ea];
    const dayIdx = (gameState.day - 1) % colors.length;
    customerBodyMesh.material.color.setHex(colors[dayIdx]);
}
...
function animate3DLoop() {
    ...
    // 손님 3D 인형 사뿐 쉼 연출
    if (customerGroup) {
        customerGroup.position.y = Math.sin(Date.now() * 0.003) * 0.03;
    }
    ...
    if (isWalking) {
        ...
        artisanGroup.rotation.y = targetAngle;

        // 3D 인형 걸음 애니메이션 (팔다리 짝짝 움직임)
        animTime += 0.2;
...
function triggerStationInteract3D() {
    ...
    if (nearStation3D.id === 'door') {
        ...
        else if (gameState.stepState === 'IDLE') alert('먼저 문 앞[🚪] 손님 인형에게 걸어가 새로운 주문을 받아주세요!');
