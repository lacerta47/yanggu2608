...
function loadGameData() {
    const saved = localStorage.getItem('season_workshop_save_v5');
    if (saved) {
        try {
...
function saveGameData() {
    localStorage.setItem('season_workshop_save_v5', JSON.stringify({ day: gameState.day, warmth: gameState.warmth }));
}
...
// ----------------------------------------------------
// THREE.JS 실사 인물 사진 기반 포토 리얼리스틱 3D 공방
// ----------------------------------------------------
let scene, camera, renderer;
let realHumanMesh, shadowMesh;
...
function initPhotoRealistic3DWorkshop() {
    if (is3DInit) return;
    is3DInit = true;
    ...
    // 4. 실사 인물 사진 기반 3D 캐릭터 렌더링
    buildPhotoRealisticHumanSprite();
...
// 실사 인물 사진 기반 3D 캐릭터 빌보드 렌더링
function buildPhotoRealisticHumanSprite() {
    ...
    realHumanMesh = new THREE.Mesh(planeGeo, planeMat);
    realHumanMesh.position.set(player3D.x, 1.2, player3D.z);
    realHumanMesh.castShadow = true;
    scene.add(realHumanMesh);
    ...
    shadowMesh.position.set(player3D.x, 0.02, player3D.z);
...
function animate3DLoop() {
    ...
    if (isWalking) {
        ...
        // 실사 캐릭터 걸음걸이 위아래 바운싱 & 걷기 모션
        animTime += 0.2;
        if (realHumanMesh) {
            realHumanMesh.position.y = 1.2 + Math.abs(Math.sin(animTime * 2)) * 0.08;
            realHumanMesh.rotation.z = Math.sin(animTime) * 0.05;
        }

        if (Math.sin(animTime) > 0.9) sounds.playStep();
    } else {
        if (realHumanMesh) {
            realHumanMesh.position.y = 1.2;
            realHumanMesh.rotation.z = 0;
        }
    }

    if (realHumanMesh) {
        realHumanMesh.position.x = player3D.x;
        realHumanMesh.position.z = player3D.z;
    }

    if (shadowMesh) {
        shadowMesh.position.x = player3D.x;
        shadowMesh.position.z = player3D.z;
    }
...
function openDialogueModal() {
    ...
    document.getElementById('acceptOrderBtn').onclick = () => {
        ...
        alert('주문을 받았습니다! 실사 사람 캐릭터와 함께 물레 작업대[🏺]로 걸어가 보세요.');
    };
}
