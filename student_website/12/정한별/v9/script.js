...
function loadGameData() {
    const saved = localStorage.getItem('season_workshop_save_v9');
    if (saved) {
        try {
...
function saveGameData() {
    localStorage.setItem('season_workshop_save_v9', JSON.stringify({ day: gameState.day, warmth: gameState.warmth }));
}
...
// ----------------------------------------------------
// THREE.JS 그림자와 사람이 일체형으로 은은하게 서 있는 공방
// ----------------------------------------------------
let scene, camera, renderer;
let artisanGroup, realArtisanMesh, artisanShadowMesh;
let customerGroup, realCustomerMesh, customerShadowMesh;
...
function initv9Workshop() {
    ...
    // 1. 만드는 사람 (그림자 판 바로 위에 발이 딱 피트된 일체형 장인 그룹)
    buildIntegratedArtisanMaker();

    // 2. 주문하는 사람 (그림자 판 바로 위에 발이 딱 피트된 일체형 손님 그룹)
    buildIntegratedCustomerNpc();
...
// 👣 그림자와 실사 장인이 일체형으로 은은하게 연결된 통합 렌더링
function buildIntegratedArtisanMaker() {
    ...
    artisanShadowMesh.position.set(0, 0.01, 0);
    artisanGroup.add(artisanShadowMesh);

    // 2. 발밑이 그림자 판 바로 위에 착 피트되도록 피벗 오프셋 설정 (지오메트리 피벗 이동)
    const photoTexture = textureLoader.load('../assets/images/real_artisan.jpg');
    const planeGeo = new THREE.PlaneGeometry(2.2, 2.4);
    planeGeo.translate(0, 1.2, 0); // 밑면(발끝)을 원점 0에 일치시킴!
    ...
    realArtisanMesh.position.set(0, 0.02, 0); // 그림자 판 0.01 바로 위 0.02에 일체형 접지!
    artisanGroup.add(realArtisanMesh);

    artisanGroup.position.set(player3D.x, 0, player3D.z);
    scene.add(artisanGroup);
}

// 👣 그림자와 실사 손님이 일체형으로 은은하게 연결된 통합 렌더링
function buildIntegratedCustomerNpc() {
    ...
    customerShadowMesh.position.set(0, 0.01, 0);
    customerGroup.add(customerShadowMesh);

    // 2. 발밑이 그림자 판 바로 위에 피트된 실사 손님
    ...
    planeGeo.translate(0, 1.1, 0); // 밑면(발끝) 원점 일치!
    ...
    realCustomerMesh.position.set(0, 0.02, 0);
    customerGroup.add(realCustomerMesh);
...
function animate3DLoop() {
    ...
    // 손님 잔잔한 은은 연출
    if (realCustomerMesh) {
        realCustomerMesh.position.y = 0.02 + Math.sin(Date.now() * 0.003) * 0.03;
    }
    ...
    if (isWalking) {
        ...
        animTime += 0.2;
        if (realArtisanMesh) {
            realArtisanMesh.position.y = 0.02 + Math.abs(Math.sin(animTime * 2)) * 0.06;
            realArtisanMesh.rotation.z = Math.sin(animTime) * 0.04;
        }

        if (Math.sin(animTime) > 0.9) sounds.playStep();
    } else {
        if (realArtisanMesh) {
            realArtisanMesh.position.y = 0.02;
            realArtisanMesh.rotation.z = 0;
        }
    }

    // 👣 그림자와 실사 장인이 완벽히 결합되어 함께 바닥을 사뿐 이동!
    if (artisanGroup) {
        artisanGroup.position.set(player3D.x, 0, player3D.z);
    }
