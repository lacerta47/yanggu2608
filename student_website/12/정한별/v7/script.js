...
// 4명의 각기 다른 진짜 사람 실사 이미지 데이터베이스
const orderDatabase = [
    {
        npcName: "달님 할머니 👵",
        npcAvatar: "../assets/images/real_grandma.jpg",
        itemTitle: "따스한 둥근 차잔",
    ...
    {
        npcName: "어린 화가 민우 🎨",
        npcAvatar: "../assets/images/real_painter.jpg",
        itemTitle: "우아한 붓꽃병",
    ...
    {
        npcName: "숲속 정원사 수아 🌿",
        npcAvatar: "../assets/images/real_gardener.jpg",
        itemTitle: "아담한 흙화분",
    ...
    {
        npcName: "시인 하은 📜",
        npcAvatar: "../assets/images/real_poet.jpg",
        itemTitle: "새벽빛 머그잔",
...
function loadGameData() {
    const saved = localStorage.getItem('season_workshop_save_v7');
    if (saved) {
        try {
...
function saveGameData() {
    localStorage.setItem('season_workshop_save_v7', JSON.stringify({ day: gameState.day, warmth: gameState.warmth }));
}
...
// ----------------------------------------------------
// THREE.JS 맑고 선명한 3D 공방 & 4명의 고유 실사 사람들
// ----------------------------------------------------
...
function initBrightv7Workshop() {
    ...
    scene.background = new THREE.Color(0x130a2a); // 맑고 아늑한 은은한 배경
    ...
    // 1. 만드는 사람 (도예 장인 실사 캐릭터)
    buildArtisanMakerSprite();

    // 2. 주문하는 사람 (오늘의 손님 실사 캐릭터)
...
// 선명하고 밝은 도예 장인 실사 캐릭터 (Self-Emissive로 그림자 어두움 현상 완벽 해결)
function buildArtisanMakerSprite() {
    ...
    const planeMat = new THREE.MeshStandardMaterial({
        ...
        transparent: true,
        emissive: 0xffffff,
        emissiveIntensity: 0.35, // 밝고 선명한 피부 조명
...
// 4명의 고유한 실사 손님 3D 캐릭터 (오늘의 손님 이미지 동적 로딩)
...
function openDiaryScreen() {
    ...
    document.getElementById('nextDayBtn').onclick = () => {
        ...
        // 🌟 다음 날 새로운 손님의 고유 실사 사진으로 동적 업데이트!
