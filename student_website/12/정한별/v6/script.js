...
const orderDatabase = [
    {
        npcName: "달님 할머니 👵", npcAvatar: "../assets/images/real_grandma.jpg", itemTitle: "따스한 둥근 차잔",
    ...
    {
        npcName: "어린 화가 민우 🎨", npcAvatar: "../assets/images/real_grandma.jpg", itemTitle: "우아한 꽃병",
    ...
    {
        npcName: "숲속 정원사 수아 🌿", npcAvatar: "../assets/images/real_grandma.jpg", itemTitle: "아담한 흙화분",
    ...
    {
        npcName: "시인 하은 📜", npcAvatar: "../assets/images/real_grandma.jpg", itemTitle: "새벽빛 머그잔",
...
function loadGameData() {
    const saved = localStorage.getItem('season_workshop_save_v6');
    if (saved) {
        try {
...
function saveGameData() {
    localStorage.setItem('season_workshop_save_v6', JSON.stringify({ day: gameState.day, warmth: gameState.warmth }));
}
...
// ----------------------------------------------------
// THREE.JS 손님(주문한 사람) & 도예 장인(만드는 사람) 공방
// ----------------------------------------------------
...
function initv6Workshop() {
    ...
    // 조명
    const ambientLight = new THREE.AmbientLight(0xc084fc, 0.7);
    ...
    const floorMat = new THREE.MeshStandardMaterial({ color: 0x1f1435, roughness: 0.7 });
    ...
    // 디테일 3D 공방 구조물
    buildRealisticWorkshopElements();

    // 1. 만드는 사람 (도예 장인 - 플레이어) 렌더링
    buildArtisanMakerSprite();

    // 2. 주문한 사람 (손님 NPC - 달님 할머니) 렌더링
    buildCustomerNpcSprite();
...
function buildRealisticWorkshopElements() {
    // 문
    ...
    // 완성 도자기 진열 선반
    const shelf = new THREE.Mesh(new THREE.BoxGeometry(4.0, 0.15, 0.6), new THREE.MeshStandardMaterial({ color: 0x581c87 }));
    shelf.position.set(0, 2.8, -7.5);
    scene.add(shelf);

    // 선반 위 항아리들
    for (let i = -1.5; i <= 1.5; i += 1.0) {
        const pot = new THREE.Mesh(new THREE.CylinderGeometry(0.2, 0.25, 0.45, 12), new THREE.MeshStandardMaterial({ color: 0xd8b4fe }));
        pot.position.set(i, 3.1, -7.5);
        scene.add(pot);
    }

    // 물레 작업대
    const wheelTable = new THREE.Mesh(new THREE.BoxGeometry(3.0, 1.2, 2.4), new THREE.MeshStandardMaterial({ color: 0x581c87, roughness: 0.6 }));
    ...
    // 가마
    const kilnBody = new THREE.Mesh(new THREE.BoxGeometry(2.6, 2.4, 2.6), new THREE.MeshStandardMaterial({ color: 0x9a3412, roughness: 0.8 }));
    kilnBody.position.set(6, 1.2, 4);
    scene.add(kilnBody);

    const chimney = new THREE.Mesh(new THREE.CylinderGeometry(0.3, 0.3, 1.8, 16), new THREE.MeshStandardMaterial({ color: 0x9a3412, roughness: 0.8 }));
...
// 만드는 사람 (도예 장인) 렌더링
function buildArtisanMakerSprite() {
    ...
    const planeMat = new THREE.MeshStandardMaterial({ map: photoTexture, transparent: true, side: THREE.DoubleSide });

    realArtisanMesh = new THREE.Mesh(planeGeo, planeMat);
    realArtisanMesh.position.set(player3D.x, 1.2, player3D.z);
    realArtisanMesh.castShadow = true;
    ...
    artisanShadowMesh.rotation.x = -Math.PI / 2;
    artisanShadowMesh.position.set(player3D.x, 0.02, player3D.z);
    scene.add(artisanShadowMesh);
}

// 주문한 사람 (손님 NPC - 달님 할머니) 렌더링
function buildCustomerNpcSprite() {
    const textureLoader = new THREE.TextureLoader();
    const grandmaTexture = textureLoader.load('../assets/images/real_grandma.jpg');

    const planeGeo = new THREE.PlaneGeometry(2.0, 2.2);
    const planeMat = new THREE.MeshStandardMaterial({ map: grandmaTexture, transparent: true, side: THREE.DoubleSide });

    realCustomerMesh = new THREE.Mesh(planeGeo, planeMat);
    realCustomerMesh.position.set(-6, 1.1, -1.8);
    realCustomerMesh.castShadow = true;
...
function triggerStationInteract3D() {
    ...
    if (nearStation3D.id === 'door') {
        ...
        else if (gameState.stepState === 'IDLE') alert('먼저 문 앞[🚪] 손님에게 걸어가 주문을 받아주세요!');
...
function openDialogueModal() {
    ...
    document.getElementById('acceptOrderBtn').onclick = () => {
        ...
        alert('주문을 받았습니다! 물레 작업대[🏺]로 걸어가 도자기를 만들어주세요.');
    };
}
