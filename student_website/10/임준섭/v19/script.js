...
// 👑 ADMIN SECRET SHORTCUT WARP TRIGGER 👑
function triggerAdminWarp() {
    ...
    setTimeout(() => adminBadge.classList.add('hidden'), 2500);
...
// 5. 7 RICH ATMOSPHERIC BIOME THEMES
...
const biomeThemes = {
    ...
    3: {
        name: "🌌 신비로운 코스믹 자수정 은하수 (Stages 17-24)",
...
// Keyboard Event Handlers (With Secret Admin Shortcut Hotkey: Shift + P or KeyK!)
document.addEventListener('keydown', (e) => {
    if (gameState !== 'playing') return;

    // 👑 ADMIN SECRET SHORTCUT CHEAT HOTKEY: Shift + P OR KeyK!
    if ((e.shiftKey && e.code === 'KeyP') || e.code === 'KeyK') {
        triggerAdminWarp();
        return;
    }
...
// 8. Hard Level Builder with Hidden Secret Admin Warp Portal
function clearSceneObjects() {
    ...
    if (adminPortalMesh) scene.remove(adminPortalMesh);
...
// 👑 CREATES GLOWING GOLDEN SECRET ADMIN PORTAL BEHIND SPAWN! 👑
function createAdminSecretPortal(x, y, z) {
    // Secret platform behind starting spawn
    createPlatform(x, y - 1, z, 6, 2, 6, { platBody: 0xf59e0b, roughness: 0.1, metalness: 0.9 });

    const portalGeo = new THREE.CylinderGeometry(1.5, 1.5, 8, 32);
    ...
    adminPortalMesh = new THREE.Mesh(portalGeo, portalMat);
    adminPortalMesh.position.set(x, y + 4, z);

    const innerBeamGeo = new THREE.CylinderGeometry(0.8, 0.8, 12, 16);
    ...
    adminPortalMesh.add(innerBeam);

    scene.add(adminPortalMesh);
}

function checkAdminPortalCollision() {
    if (adminPortalMesh) {
        const portalPos = adminPortalMesh.position;
...
// Stage Builder for 50 Massive Stages + Admin Secret Portal
function loadStage(stage) {
    ...
    // 👑 HIDDEN ADMIN SECRET PORTAL BEHIND SPAWN PLATFORM (at Z = +8.0)!
    createAdminSecretPortal(0, 1, 8.0);
...
function loadSpecialStage() {
    ...
    createAdminSecretPortal(0, 1, 9.0);
    ...
    if (currentStage !== 'special') {
        if (currentStage > highestStageCleared) {
            ...
            if (currentStage === 50) {
                ...
                unlockNoticeBanner.textContent = `🎉 스테이지 ${currentStage} 클리어! 드레스룸에서 새로 해금된 이펙트 스킨을 확인하세요!`;
            }
        } else {
        ...
        if (!isRainbowUnlocked) {
            ...
            unlockNoticeBanner.classList.remove('hidden');
            unlockNoticeBanner.textContent = '🎉 ⭐ 특별 한정판 레인보우 히어로 이펙트 해금 성공!';
        } else {
            unlockNoticeBanner.classList.add('hidden');
        }
    }
...
// 13. Main Game Loop (Pure 1st Person FPS Parkour with Admin Secret Portal Check)
...
function animate() {
    ...
    if (adminPortalMesh) {
        adminPortalMesh.rotation.y += 0.02;
    }
