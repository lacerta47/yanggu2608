...
camera.rotation.order = 'YXZ'; // Lock Euler order to prevent gimbal rotation distortion
...
// 5. Game State & Physics Variables
let currentStage = 1;
let startTime = 0;
let elapsedTime = 0;
...
// Stage Map Data Construction for 20 Stages
function loadStage(stage) {
    ...
    createPlatform(0, -1, 0, 12, 2, 12, theme); // Start Platform
    ...
    if (stage === 1) {
        ...
        // Redesigned Stage 2: Introductory Wall Run + Zigzag Wall Jump Tower!
        createWall(6, 8, -22, 1, 16, 20, theme);
        createPlatform(0, 8, -38, 8, 2, 8, theme);
        createCheckpointPlatform(0, 9, -38, theme);

        createWall(-6, 14, -54, 1, 16, 20, theme); // Left Wall
        createWall(6, 20, -70, 1, 16, 20, theme);  // Right Wall
        createPlatform(0, 24, -86, 10, 2, 10, theme);
        createGoal(0, 26, -108);
    } else {
        // Stages 3 to 20: Adjusted First Platform Jump Spacing for comfortable jumps!
        let currentZ = -14; // Start close to spawn platform z=0!
        let currentY = 1;
        let currentX = 0;

        const numSteps = 5 + Math.floor(stage * 0.5);

        for (let i = 0; i < numSteps; i++) {
            createPlatform(currentX, currentY, currentZ, 8, 2, 8, theme);

            // Add Wall Run Challenges
            if (i % 2 === 1) {
                const wallSideX = currentX + (Math.random() > 0.5 ? 6 : -6);
                createWall(wallSideX, currentY + 6, currentZ - 8, 1, 16, 18, theme);
            }
            ...
            // Advance to next platform step with reasonable gap
            currentZ -= 14 + Math.random() * 4;
            currentY += 2 + Math.random() * 2;
            currentX += (Math.random() > 0.5 ? 1 : -1) * (3 + Math.random() * 4);
        }

        // Final Goal Platform for Stages 3 to 20
        createGoal(currentX, currentY + 3, currentZ - 14);
    }
...
function checkWallRun() {
    ...
    camera.rotation.z = 0; // Lock roll angle for rock-solid stability
