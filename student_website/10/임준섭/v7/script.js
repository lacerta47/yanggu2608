...
const specialStageBtn = document.getElementById('specialStageBtn');

// 2. Three.js Scene, Camera, Renderer Setup
const scene = new THREE.Scene();
const camera = new THREE.PerspectiveCamera(75, window.innerWidth / window.innerHeight, 0.1, 1000);
camera.rotation.order = 'YXZ'; // Lock Euler order to prevent gimbal roll distortion
...
document.body.addEventListener('click', (e) => {
    if (e.target === specialStageBtn) return;
...
function createBoosterPad(x, y, z) {
    ...
    const wireGeo = new THREE.WireframeGeometry(geo);
    const wireMat = new THREE.LineBasicMaterial({ color: 0xf43f5e, linewidth: 2 });
    const wireframe = new THREE.LineSegments(wireGeo, wireMat);
    mesh.add(wireframe);
...
function loadStage(stage) {
    ...
    if (stage === 1) {
        ...
        // FIXED Stage 5 & Procedural Stages: Goal platform is now placed close to the final step!
        let currentZ = -14;
        let currentY = 1;
        let currentX = 0;
        ...
        // Final Goal Platform placed comfortably close to the last step!
...
function loadSpecialStage() {
    ...
    createPlatform(0, 2, -20, 8, 2, 8, theme);
    createBoosterPad(0, 2, -20);

    createPlatform(0, 18, -44, 8, 2, 8, theme);
    createCheckpointPlatform(0, 19, -44, theme);

    createWall(-7, 24, -62, 1, 20, 24, theme);
    createBoosterPad(-7, 24, -62);

    createPlatform(0, 38, -88, 10, 2, 10, theme);
    createBoosterPad(0, 38, -88);
...
function stageComplete() {
    ...
    if (currentStage === 'special') {
        ...
        victoryMessage.textContent = '대단합니다! 무지개 스페셜 스테이지를 멋지게 클리어하셨습니다!';
        nextStageBtn.textContent = '1단계로 돌아가기 ▶';
...
specialStageBtn.addEventListener('click', () => {
    victoryOverlay.classList.add('hidden');
    startTime = performance.now();
    isTimerRunning = true;
    loadSpecialStage();
