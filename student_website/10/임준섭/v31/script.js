...
// 👑 DYNAMIC ADMIN TITLE STATE 👑
let isAdminShortcutActive = false;
let adminWarpTimer = null;
...
// 👑 DYNAMIC TITLE CALCULATOR (SHOWS ADMIN TITLE ONLY WHEN SHORTCUT/ADMIN MODE IS ON!) 👑
function getUserTitle(highestCleared, isSpecialCleared) {
    if (isAdminShortcutActive || isFlying) {
        return "👑 관리자";
    }
    if (isSpecialCleared) {
...
// 👑 SECRET ADMIN FLYING MODE & WARP STATE 👑
let isFlying = false;
...
function triggerAdminWarp() {
    ...
    // Show Admin Title ONLY while using shortcut warp!
    isAdminShortcutActive = true;
    updateTitleDisplays();

    if (adminWarpTimer) clearTimeout(adminWarpTimer);
    adminWarpTimer = setTimeout(() => {
        isAdminShortcutActive = false;
        updateTitleDisplays();
    }, 4000);
...
// 13. Main Game Loop (Pure 1st Person FPS Parkour & Dynamic Void Fall Shake Engine)
...
function animate() {
    ...
    if (controls.isLocked && gameState === 'playing') {
        ...
        // 🌌 DYNAMIC VOID FALL SCREEN SHAKE ENGINE (WHEN FALLING INTO ABYSS!) 🌌
        if (!isFlying && playerPos.y < -5.0) {
            // As player drops deeper into the void (-5.0 down to -25.0), shake intensifies violently!
            const depth = Math.abs(playerPos.y - (-5.0));
            const voidShakeIntensity = Math.min(0.85, depth * 0.038);

            camera.position.x += (Math.random() - 0.5) * voidShakeIntensity;
            camera.position.y += (Math.random() - 0.5) * voidShakeIntensity;
            camera.position.z += (Math.random() - 0.5) * voidShakeIntensity;
            camera.rotation.z = (Math.random() - 0.5) * (voidShakeIntensity * 0.2);

            spawnSkinParticles('boost', 2);
        }

        // Screen Shake Jitter from Impacts / Respawn
        if (shakeTime > 0) {
            shakeTime -= delta;
            camera.position.x += (Math.random() - 0.5) * 0.45;
            camera.position.y += (Math.random() - 0.5) * 0.45;
            camera.position.z += (Math.random() - 0.5) * 0.45;
        }

        if (!isFlying) {
            resolveWallCollisions();

            // Respawns when plunging deep into the Void!
            if (playerPos.y < -25) {
                triggerScreenShake();
                respawnPlayer();
            } else {
