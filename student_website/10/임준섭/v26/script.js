...
// 👑 ULTRA EXTENDED EXTREME HARD SPECIAL STAGE GENERATOR (Z = -1,200+ & 55+ EXTREME STEPS!) 👑
function loadSpecialStage() {
    ...
    for (let i = 0; i < totalSpecialSteps; i++) {
        // Spaced out Checkpoints (only 3 checkpoints across the entire 1200+ mega distance!)
        const isCheckpoint = (i === 15 || i === 30 || i === 45);

        if (isCheckpoint) {
            createCheckpointPlatform(currentX, currentY, currentZ, theme);
        } else {
            // Extreme Micro Precision Platforms
            const platW = 2.4 + Math.random() * 0.8; // Ultra narrow 2.4~3.2 width!
            const platD = 2.4 + Math.random() * 0.8;

            if (i % 4 === 1) {
                // High-Speed Oscillating Moving Platform
            ...
            // Extreme Obstacle Traps & Wall Chains
            if (i % 5 === 2) {
                // Dual Tight Wall Running Channels!
                const wallX = currentX + (i % 2 === 0 ? 5.5 : -5.5);
                createWall(wallX, currentY + 8, currentZ - 8, 1, 20, 26, theme);
            } else if (i % 6 === 4) {
                // Super Jump Booster Pad
                createBoosterPad(currentX, currentY, currentZ);
            }
        }

        // Steeper Distance & Vertical Elevation Climb up to sky Y = 140+!
        currentZ -= 20 + Math.random() * 6;
        currentY += 3.2 + Math.random() * 2.5;
        currentX += (Math.sin(i * 0.7) * 7.5); // Oscillating S-Curve path!
    }
