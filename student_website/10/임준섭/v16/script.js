...
// 4. PointerLock Controls & Perspective State Manager ('1st' vs '3rd')
...
function togglePerspective() {
    cameraMode = (cameraMode === '1st') ? '3rd' : '1st';
    if (cameraMode === '3rd') {
        viewToggleBtn.textContent = '🎥 1인칭 카메라 (키: 3)';
        perspectiveBadge.textContent = '🎥 제자리 3인칭 안드로이드 로봇 시점!';
    } else {
        viewToggleBtn.textContent = '🎥 3인칭 카메라 (키: 3)';
        perspectiveBadge.textContent = '🎥 1인칭 FPS 시점!';
    }
    perspectiveBadge.classList.remove('hidden');
    setTimeout(() => perspectiveBadge.classList.add('hidden'), 1800);
}

viewToggleBtn.addEventListener('click', togglePerspective);
...
// 🤖 3D ICONIC ANDROID ROBOT AVATAR MESH GENERATOR! 🤖
let robotGroup = null;
let androidBodyMat = null;
let androidEyeMat = null;

function create3DAndroidAvatar() {
    robotGroup = new THREE.Group();

    androidBodyMat = new THREE.MeshStandardMaterial({
        color: 0x8ec07c,
        roughness: 0.3,
        metalness: 0.4
    });

    androidEyeMat = new THREE.MeshBasicMaterial({ color: 0xffffff });

    const headGeo = new THREE.SphereGeometry(0.55, 32, 16, 0, Math.PI * 2, 0, Math.PI / 2);
    const head = new THREE.Mesh(headGeo, androidBodyMat);
    head.position.set(0, 1.85, 0);
    robotGroup.add(head);

    const antennaGeo = new THREE.CylinderGeometry(0.04, 0.04, 0.35, 16);

    const leftAntenna = new THREE.Mesh(antennaGeo, androidBodyMat);
    leftAntenna.position.set(-0.24, 2.4, 0);
    leftAntenna.rotation.z = Math.PI / 6;
    robotGroup.add(leftAntenna);

    const rightAntenna = new THREE.Mesh(antennaGeo, androidBodyMat);
    rightAntenna.position.set(0.24, 2.4, 0);
    rightAntenna.rotation.z = -Math.PI / 6;
    robotGroup.add(rightAntenna);

    const eyeGeo = new THREE.SphereGeometry(0.07, 16, 16);

    const leftEye = new THREE.Mesh(eyeGeo, androidEyeMat);
    leftEye.position.set(-0.2, 2.08, 0.45);
    robotGroup.add(leftEye);

    const rightEye = new THREE.Mesh(eyeGeo, androidEyeMat);
    rightEye.position.set(0.2, 2.08, 0.45);
    robotGroup.add(rightEye);

    const torsoGeo = new THREE.CylinderGeometry(0.52, 0.52, 1.1, 32);
    const torso = new THREE.Mesh(torsoGeo, androidBodyMat);
    torso.position.set(0, 1.15, 0);
    robotGroup.add(torso);

    const armGeo = new THREE.CylinderGeometry(0.16, 0.16, 0.85, 16);

    const leftArm = new THREE.Mesh(armGeo, androidBodyMat);
    leftArm.position.set(-0.75, 1.15, 0);
    robotGroup.add(leftArm);

    const rightArm = new THREE.Mesh(armGeo, androidBodyMat);
    rightArm.position.set(0.75, 1.15, 0);
    robotGroup.add(rightArm);

    const legGeo = new THREE.CylinderGeometry(0.18, 0.18, 0.55, 16);

    const leftLeg = new THREE.Mesh(legGeo, androidBodyMat);
    leftLeg.position.set(-0.25, 0.32, 0);
    robotGroup.add(leftLeg);

    const rightLeg = new THREE.Mesh(legGeo, androidBodyMat);
...
function updateRobotSkinColor() {
    if (!robotGroup) return;

    let bodyColor = 0x8ec07c;
    ...
    androidBodyMat.color.setHex(bodyColor);
    androidEyeMat.color.setHex(eyeColor);
}

create3DAndroidAvatar();

// 🌟 REAL ELEMENT 3D PARTICLE GENERATOR 🌟
let skinParticles = [];
...
function spawnSkinParticles(actionType, count = 2) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (equippedSkin === 'thunder') {
            ...
            mat = new THREE.MeshStandardMaterial({
                color: Math.random() > 0.5 ? 0x38bdf8 : 0x0284c7,
                roughness: 0.1,
                metalness: 0.8,
                transparent: true,
                opacity: 0.85
...
function loadStage(stage) {
    ...
    const numSteps = 16 + Math.floor(stage * 1.2);
    ...
    for (let i = 0; i < numSteps; i++) {
        ...
        if (isCheckpointStep) {
            ...
            const platWidth = Math.max(3.8, 5.5 - stage * 0.08);

            if (i % 3 === 1 && i > 2) {
                createMovingPlatform(currentX, currentY, currentZ, platWidth, 2, platWidth, theme, 4.5 + Math.random() * 2.0, 2.2);
            } else {
...
function loadSpecialStage() {
    if (highestStageCleared < 20) {
        alert("🔒 스페셜 무지개 파쿠르 코스는 20단계를 완파하셔야 해금됩니다!");
        return;
    }
    ...
    createMovingPlatform(0, 2, -22, 5, 2, 5, theme, 5.0, 2.5);
    ...
    createPlatform(0, 58, -324, 6, 2, 6, theme);
    createBoosterPad(0, 58, -324);

    createCheckpointPlatform(0, 64, -348, theme);
    createPlatform(0, 66, -368, 9, 2, 9, theme);
...
// Special Stage Lock Manager
function updateSpecialStageLockStatus() {
    const isSpecialUnlocked = highestStageCleared >= 20;
    ...
    if (isSpecialUnlocked) {
        ...
        specialStageHomeBtn.textContent = '🔒 ⭐ 스페셜 무지개 코스 (20단계 클리어 시 해금)';
        specialStageHudBtn.disabled = true;
        specialStageHudBtn.textContent = '🔒 스페셜 (20단계 해금)';
    }
}
...
function updateClosetUI() {
    ...
    thunderStatusTag.textContent = isThunderUnlocked ? '✅ 해금 완료! (화염 안드로이드)' : '🔒 5단계 클리어 시 해금 (화염 안드로이드)';
    ...
    emeraldStatusTag.textContent = isEmeraldUnlocked ? '✅ 해금 완료! (그린 안드로이드)' : '🔒 10단계 클리어 시 해금 (그린 안드로이드)';
    ...
    diamondStatusTag.textContent = isDiamondUnlocked ? '✅ 해금 완료! (다이아 안드로이드)' : '🔒 15단계 클리어 시 해금 (다이아 안드로이드)';
    ...
    rainbowStatusTag.textContent = isRainbowUnlocked ? '✅ 해금 완료! (무지개 안드로이드)' : '🔒 스페셜 완주 시 해금 (무지개 안드로이드)';
...
// Reset Stage Progress AND All Skin Unlocks!
...
function stageComplete() {
    ...
    if (currentStage !== 'special') {
        if (currentStage > highestStageCleared) {
            ...
            if (currentStage === 20) {
                unlockNoticeBanner.textContent = `🎉 20단계 최종 완파 성공! ⭐ 스페셜 무지개 파쿠르 코스가 해금되었습니다!`;
            } else {
                unlockNoticeBanner.textContent = `🎉 스테이지 ${currentStage} 클리어! 드레스룸에서 새로 해금된 안드로이드 스킨을 확인하세요!`;
            }
        } else {
        ...
        if (!isRainbowUnlocked) {
            ...
            unlockNoticeBanner.classList.remove('hidden');
            unlockNoticeBanner.textContent = '🎉 ⭐ 특별 한정판 레인보우 히어로 안드로이드 해금 성공!';
        } else {
            unlockNoticeBanner.classList.add('hidden');
        }
    }
    ...
    if (currentStage === 'special') {
        ...
        victoryMessage.textContent = '축하합니다! 20개의 모든 파쿠르 코스를 완파하여 ⭐ 스페셜 무지개 코스가 해금되었습니다!';
...
// 13. Main Game Loop with IN-PLACE 3rd PERSON CAMERA SYSTEM (ZERO DISTANCE SHIFT!)
...
function animate() {
    ...
    if (controls.isLocked && gameState === 'playing') {
        ...
        // IN-PLACE 3rd PERSON CAMERA & ANDROID MODEL POSITIONING SYSTEM
        if (cameraMode === '1st') {
            ...
            // Android avatar positioned right at current player standing spot
            if (robotGroup) {
                robotGroup.visible = true;
                robotGroup.position.set(playerPos.x, playerPos.y - targetEyeHeight, playerPos.z);
                robotGroup.rotation.y = Math.atan2(-lookDir.x, -lookDir.z);
            }

            // Tight In-Place View Distance (1.0 unit tight back view - zero backward push!)
            const inPlaceDist = 1.0;
            const eyeLevelY = 0.5;

            camera.position.set(
                playerPos.x - lookDir.x * inPlaceDist,
                playerPos.y + eyeLevelY,
                playerPos.z - lookDir.z * inPlaceDist
            );
