...
function spawnSkinParticles(actionType, count = 2) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (equippedSkin === 'thunder') {
            type = 'fire';
            if (Math.random() > 0.4) {
                geo = new THREE.ConeGeometry(0.2, 0.45, 8);
            } else {
                geo = new THREE.SphereGeometry(0.18, 8, 8);
            }
            ...
            if (Math.random() > 0.3) {
                ...
                geo = new THREE.TorusGeometry(0.35, 0.06, 8, 16);
            }
            mat = new THREE.MeshStandardMaterial({
...
function updateClosetUI() {
    ...
    // Reset status tags for locked skins
    thunderStatusTag.textContent = isThunderUnlocked ? '✅ 해금 완료! (화염 불꽃)' : '🔒 5단계 클리어 시 해금 (화염 불꽃)';
    ...
    emeraldStatusTag.textContent = isEmeraldUnlocked ? '✅ 해금 완료! (3D 나뭇잎)' : '🔒 10단계 클리어 시 해금 (3D 나뭇잎)';
    ...
    diamondStatusTag.textContent = isDiamondUnlocked ? '✅ 해금 완료! (얼음 결정체)' : '🔒 15단계 클리어 시 해금 (얼음 결정체)';
    ...
    rainbowStatusTag.textContent = isRainbowUnlocked ? '✅ 해금 완료! (무지개 폭죽)' : '🔒 스페셜 완주 시 해금 (무지개 폭죽)';
...
// 12. Wall Running & Physics Collision
