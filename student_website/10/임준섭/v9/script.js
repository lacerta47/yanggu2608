...
// 2. Three.js Renderer with Shadow Map
...
// Lighting with 3D Real-time Shadows
...
sunMesh.position.set(50, 90, -380);
scene.add(sunMesh);
...
const biomeThemes = {
    ...
    3: {
        ...
        platBody: 0x1e1b4b, platGrid: 0xa855f7, wallBody: 0x312e81, bldgColor: 0x140c2b
    },
    4: {
        name: "🌲 신비한 에메랄드 숲",
        sky: 0x064e3b, fog: 0x022c22, sun: 0x84cc16,
        platBody: 0x064e3b, platGrid: 0x22c55e, wallBody: 0x047857, bldgColor: 0x022c22
    },
    5: {
        name: "⚡ 골든 황금 산맥",
        sky: 0x78350f, fog: 0x451a03, sun: 0xfde047,
        platBody: 0x451a03, platGrid: 0xeab308, wallBody: 0x92400e, bldgColor: 0x291002
    },
    special: {
        name: "⭐ 스페셜 무지개 은하수",
        sky: 0x2e1065, fog: 0x3b0764, sun: 0xfacc15,
        platBody: 0x4c1d95, platGrid: 0xec4899, wallBody: 0x6b21a8, bldgColor: 0x3b0764
    }
};
...
// 5. Game State & Physics Variables (SUPER FAST AUTO-SPRINT SPEED 220.0!)
let currentStage = 1;
let startTime = 0;
let elapsedTime = 0;
...
// 6. Full 3D City Environment & Level Builder
...
function create3DCityEnvironment(theme) {
    // Spawns tall 3D skyscrapers around the parkour course for realistic spatial depth!
    for (let z = -20; z >= -400; z -= 30) {
        ...
        sides.forEach(x => {
            ...
            const mat = new THREE.MeshLambertMaterial({ color: theme.bldgColor });
            ...
            const wireMat = new THREE.LineBasicMaterial({ color: theme.platGrid, transparent: true, opacity: 0.2 });
...
function applyStageTheme(stageKey) {
    ...
    scene.fog = new THREE.FogExp2(theme.fog, 0.006);
...
// Stage Maps Builder for 20 Long & Harder Stages
function loadStage(stage) {
    ...
    if (stage === 1) {
        ...
        // High-Speed Long & Harder Procedural Stages 3 to 20
        let currentZ = -14;
        let currentY = 1;
        let currentX = 0;

        const numSteps = 8 + Math.floor(stage * 0.75); // Expanded long map length!

        for (let i = 0; i < numSteps; i++) {
            createPlatform(currentX, currentY, currentZ, 7, 2, 7, theme);
            ...
            if (i === Math.floor(numSteps / 3) || i === Math.floor(numSteps * 0.7)) {
                createCheckpointPlatform(currentX, currentY + 1, currentZ, theme);
            }

            currentZ -= 14 + Math.random() * 4;
            currentY += 2.0 + Math.random() * 2.0;
            currentX += (Math.random() > 0.5 ? 1 : -1) * (3 + Math.random() * 4);
        }
...
function loadSpecialStage() {
    ...
    createBoosterPad(0, 60, -184);

    // Section 7: Fourth Booster & Grand Rainbow Victory Tower
    createPlatform(0, 78, -210, 8, 2, 8, theme);
    createBoosterPad(0, 78, -210);

    createPlatform(0, 96, -236, 12, 2, 12, theme);
    createGoal(0, 98, -256);
...
// 7. 5-Skin Character Closet Manager
function updateClosetUI() {
    // Check unlock statuses based on stage progress & special stage completion
    ...
    if (isThunderUnlocked) {
        cardThunder.className = 'char-card unlocked';
    ...
    if (isEmeraldUnlocked) {
        cardEmerald.className = 'char-card unlocked';
    ...
    if (isDiamondUnlocked) {
        cardDiamond.className = 'char-card unlocked';
    ...
    // Card 5: Rainbow Special
    if (isRainbowUnlocked) {
        cardRainbow.className = 'char-card unlocked full-width';
...
function stageComplete() {
    ...
    if (currentStage !== 'special') {
        if (currentStage > highestStageCleared) {
            ...
            unlockNoticeBanner.textContent = `🎉 스테이지 ${currentStage} 클리어! 새로운 캐릭터 스킨이 해금되었는지 확인해보세요!`;
        } else {
...
// 10. Main Game Loop (Super Fast Speed 220.0!)
...
function animate() {
    ...
    if (controls.isLocked && gameState === 'playing') {
        ...
        // SUPER FAST SPEED MULTIPLIER (220.0!)
