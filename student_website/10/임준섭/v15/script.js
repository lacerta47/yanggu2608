...
function togglePerspective() {
    ...
    if (cameraMode === '3rd') {
        ...
        perspectiveBadge.textContent = '🎥 3인칭 클로즈업 로봇 시점!';
    } else {
...
// 🤖 3D SCI-FI ROBOT CHARACTER AVATAR MESH GENERATOR! 🤖
let robotGroup = null;
let robotVisorMat = null;
let robotBodyMat = null;

function create3DRobotAvatar() {
    robotGroup = new THREE.Group();

    const headGeo = new THREE.BoxGeometry(0.8, 0.7, 0.7);
    robotBodyMat = new THREE.MeshStandardMaterial({
        color: 0x334155,
        metalness: 0.9,
        roughness: 0.2
    });
    ...
    robotVisorMat = new THREE.MeshBasicMaterial({ color: 0x38bdf8 });
    const visor = new THREE.Mesh(visorGeo, robotVisorMat);
    visor.position.set(0, 2.3, 0.32);
    robotGroup.add(visor);

    const chestGeo = new THREE.BoxGeometry(1.1, 1.2, 0.7);
    const chest = new THREE.Mesh(chestGeo, robotBodyMat);
    chest.position.set(0, 1.3, 0);
    robotGroup.add(chest);

    const coreGeo = new THREE.CylinderGeometry(0.2, 0.2, 0.1, 16);
    const coreMesh = new THREE.Mesh(coreGeo, robotVisorMat);
    coreMesh.rotation.x = Math.PI / 2;
    coreMesh.position.set(0, 1.4, 0.36);
...
function updateRobotSkinColor() {
    ...
    robotBodyMat.color.setHex(bodyColor);
    robotVisorMat.color.setHex(visorColor);
}
...
function updateClosetUI() {
    ...
    thunderStatusTag.textContent = isThunderUnlocked ? '✅ 해금 완료! (화염 로봇)' : '🔒 5단계 클리어 시 해금 (화염 로봇)';
    ...
    emeraldStatusTag.textContent = isEmeraldUnlocked ? '✅ 해금 완료! (그린 숲 로봇)' : '🔒 10단계 클리어 시 해금 (그린 숲 로봇)';
    ...
    diamondStatusTag.textContent = isDiamondUnlocked ? '✅ 해금 완료! (다이아 로봇)' : '🔒 15단계 클리어 시 해금 (다이아 로봇)';
    ...
    rainbowStatusTag.textContent = isRainbowUnlocked ? '✅ 해금 완료! (무지개 은하수 로봇)' : '🔒 스페셜 완주 시 해금 (무지개 로봇)';
...
// 12. Wall Running & Solid Wall Penetration Barrier System!
function checkWallRun(delta) {
    ...
    if (nearWall && !canJump && (moveForward || moveLeft || moveRight || moveBackward)) {
        if (wallRunStamina > 0) {
            ...
            // Push player firmly onto wall exterior face so body never penetrates wall geometry!
...
// SOLID 100% ZERO-PENETRATION WALL COLLISION ENGINE!
function resolveWallCollisions() {
    const playerRadius = 1.4; // Solid thick radius barrier to prevent any wall clipping!
...
function stageComplete() {
    ...
    if (currentStage !== 'special') {
        if (currentStage > highestStageCleared) {
            ...
            unlockNoticeBanner.textContent = `🎉 스테이지 ${currentStage} 클리어! 드레스룸에서 새로 해금된 로봇 스킨을 확인하세요!`;
        } else {
        ...
        if (!isRainbowUnlocked) {
            ...
            unlockNoticeBanner.classList.remove('hidden');
            unlockNoticeBanner.textContent = '🎉 ⭐ 특별 한정판 레인보우 히어로 로봇 해금 성공!';
        } else {
            unlockNoticeBanner.classList.add('hidden');
        }
    }
...
// 13. Main Game Loop with DOUBLE-PASS SOLID WALL COLLISION BARRIER!
...
function animate() {
    ...
    if (controls.isLocked && gameState === 'playing') {
        ...
        // ENFORCE FIRST WALL COLLISION PASS TO PREVENT ANY PENETRATION
        resolveWallCollisions();

        // Sync back camera X & Z to resolved solid playerPos
        camera.position.x = playerPos.x;
        camera.position.z = playerPos.z;

        // FIXED 3rd Person Camera Calculation (Close-up Over-the-Shoulder View!)
        if (cameraMode === '1st') {
            ...
            const targetCamPos = playerPos.clone().sub(lookDir.clone().multiplyScalar(camDist));
            targetCamPos.y = playerPos.y + camHeight;
        ...
        if (playerPos.y < -25) {
            ...
            platforms.forEach((p) => {
                ...
                if (
                    ...
                    if (cameraMode === '1st') camera.position.y = playerPos.y;
                    canJump = true;
                    wallRunStamina = 2.0;
                    wallRunCooldown = 0;
                }
