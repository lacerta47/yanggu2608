...
class SoundEngine {
    ...
    playLaserZapSound() { this.playSFX(150, 0.25, 'sawtooth', 0.25); }
...
// 🌟 REAL ELEMENT 3D PARTICLE GENERATOR (FIRE, WATER, LEAF, ICE, RAINBOW)! 🌟
let skinParticles = [];
...
function spawnSkinParticles(actionType, count = 2) {
    ...
    for (let i = 0; i < count; i++) {
        ...
        if (equippedSkin === 'thunder') {
            // 🔥 REAL FIRE FLAMES & EMBERS (Fire Cone Mesh + Rising Embers)
            type = 'fire';
            if (Math.random() > 0.4) {
                geo = new THREE.ConeGeometry(0.2, 0.45, 8); // Flame Cone
            } else {
                geo = new THREE.SphereGeometry(0.18, 8, 8); // Fire Ember
            }
            ...
            velY = actionType === 'jump' ? 4.5 + Math.random() * 3.0 : 2.5 + Math.random() * 2.0; // Flames rise up!
        } else if (equippedSkin === 'emerald') {
            // 🍃 REAL FLUTTERING 3D LEAVES & NATURE AURA
            type = 'leaf';
            geo = new THREE.PlaneGeometry(0.35, 0.45); // Leaf shape
            mat = new THREE.MeshBasicMaterial({
            ...
            velY = (Math.random() - 0.5) * 1.0 - 0.5; // Leaves flutter down slowly
            pLife = 0.9 + Math.random() * 0.5;
        } else if (equippedSkin === 'diamond') {
            // ❄️ REAL ICE CRYSTAL PRISMS & FROST
            type = 'ice';
            geo = new THREE.OctahedronGeometry(0.25); // Ice Prism
            ...
            // 👑 REAL RAINBOW FIREWORKS STARS
            type = 'rainbow';
            geo = new THREE.TetrahedronGeometry(0.28); // Star Sparkle
            mat = new THREE.MeshBasicMaterial({
            ...
            // 💦 REAL WATER SPLASH DROPLETS & RIPPLES (Basic Aqua Blue)
            type = 'water';
            if (Math.random() > 0.3) {
                geo = new THREE.SphereGeometry(0.22, 8, 8); // Water Droplet
            } else {
                geo = new THREE.TorusGeometry(0.35, 0.06, 8, 16); // Water Ripple Ring
            }
            mat = new THREE.MeshStandardMaterial({
            ...
            velY = actionType === 'jump' ? 4.0 + Math.random() * 2.5 : -1.0 + Math.random() * 1.5; // Water splashes!
        }
        ...
        if (type === 'fire') {
            mesh.rotation.x = -Math.PI / 2; // Point fire flames upward
        } else {
...
function updateSkinParticles(delta) {
    for (let i = skinParticles.length - 1; i >= 0; i--) {
        ...
        // Custom Physics Motion per Element!
        if (p.type === 'fire') {
            p.vel.y += 2.0 * delta; // Flame heat rises up!
            p.mesh.scale.multiplyScalar(0.96); // Flame shrinks as it burns out
        } else if (p.type === 'leaf') {
            p.mesh.rotation.z += p.rotSpeed * delta; // Fluttering leaf rotation!
            p.vel.x += Math.sin(p.life * 10) * delta * 2.0; // Swaying leaf motion!
        } else if (p.type === 'water') {
            p.vel.y -= gravity * 0.3 * delta; // Water gravity splash drop!
        } else if (p.type === 'ice') {
            p.mesh.rotation.y += p.rotSpeed * delta; // Glittering crystal rotation!
...
// 8. Level Builder & Realistic Sci-Fi Laser Emitter Generator!
...
function createLaser(x, y, z, width = 7, moveRange = 3.5, speed = 2.0) {
    const laserGroup = new THREE.Group();

    const coreGeo = new THREE.CylinderGeometry(0.12, 0.12, width, 16);
    ...
    const glowGeo = new THREE.CylinderGeometry(0.35, 0.35, width, 16);
    ...
    const podGeo = new THREE.CylinderGeometry(0.5, 0.6, 1.2, 16);
    ...
    podLeft.rotation.z = Math.PI / 2;
    podLeft.position.set(-width / 2 - 0.6, 0, 0);
    laserGroup.add(podLeft);

    const podRight = new THREE.Mesh(podGeo, podMat);
    podRight.rotation.z = Math.PI / 2;
    podRight.position.set(width / 2 + 0.6, 0, 0);
    laserGroup.add(podRight);
    ...
    ringLeft.position.set(-width / 2, 0, 0);
    laserGroup.add(ringLeft);

    const ringRight = new THREE.Mesh(ringGeo, ringMat);
    ringRight.rotation.y = Math.PI / 2;
    ringRight.position.set(width / 2, 0, 0);
    laserGroup.add(ringRight);

    laserGroup.position.set(x, y, z);
...
function loadStage(stage) {
    ...
    for (let i = 0; i < numSteps; i++) {
        ...
        if (isCheckpointStep) {
            ...
            if (i % 3 === 2 && i > 1) {
                createLaser(currentX, currentY + 2.5, currentZ, 6.5, 3.0, 2.5);
            }
        }
...
function loadSpecialStage() {
    ...
    createLaser(0, 14.5, -48, 8, 2.0, 3.0);
    ...
    createLaser(0, 30.5, -140, 8, 3.5, 3.5);
    ...
    createLaser(0, 50.5, -252, 8, 4.0, 4.0);
...
function updateClosetUI() {
    ...
    if (isThunderUnlocked) {
        ...
        thunderStatusTag.textContent = '✅ 해금 완료! (솟구치는 이중 화염)';
    ...
    if (isEmeraldUnlocked) {
        ...
        emeraldStatusTag.textContent = '✅ 해금 완료! (3D 바람 나뭇잎)';
    ...
    if (isDiamondUnlocked) {
        ...
        diamondStatusTag.textContent = '✅ 해금 완료! (얼음 결정체)';
    ...
    if (isRainbowUnlocked) {
        ...
        rainbowStatusTag.textContent = '✅ 해금 완료! (무지개 불꽃놀이)';
...
// 12. Wall Running & Realistic Pulsing Laser Animation
...
// Realistic Laser Movement & Pulsing Energy Animation Check!
function updateLasers(time) {
    lasers.forEach(laser => {
        laser.mesh.position.x = laser.originX + Math.sin(time * laser.speed) * laser.moveRange;

        const pulse = 2.5 + Math.sin(time * 15) * 1.0;
        laser.coreMat.emissiveIntensity = pulse;
        laser.glowMat.emissiveIntensity = pulse * 0.8;

        const px = camera.position.x;
        const py = camera.position.y;
        const pz = camera.position.z;

        const lx = laser.mesh.position.x;
        const ly = laser.y;
        const lz = laser.z;

        const dx = Math.abs(px - lx);
        const dy = Math.abs(py - ly);
        const dz = Math.abs(pz - lz);

        if (dx < laser.width / 2 + 0.8 && dy < 1.8 && dz < 1.2) {
            audioEngine.playLaserZapSound();
            laserBadge.classList.remove('hidden');
            setTimeout(() => laserBadge.classList.add('hidden'), 2000);
            respawnPlayer();
        }
    });
}
