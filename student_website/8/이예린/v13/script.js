// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v13 Engine (몬스터 용사 추적 AI 달려오기)
// -------------------------------------------------------------
...
const resultDamageSpan = document.getElementById('result-damage');
...
const AGGRO_RANGE = 12.0; // 🏃 몬스터가 용사를 감지하고 달려오는 사거리
...
// -------------------------------------------------------------
// 🌲 3D 숲속 맵 (Map 3: 🏃 몬스터 용사 추적 AI 속도 설정!)
// -------------------------------------------------------------
...
btnForgeWeapon.addEventListener('click', () => {
    initAudio();
    const cost = getForgeCost();
...
function finishGame() {
    ...
    resultDamageSpan.textContent = `${swordDamage}`;
...
// -------------------------------------------------------------
// 3D 1인칭 메인 애니메이션 & 🏃 몬스터 용사 추적 AI 루프
// -------------------------------------------------------------
function animate() {
        ...
        if (currentMap === 'FOREST') {
            ...
            monsters3D.forEach(m => {
                if (m.alive) {
                    ...
                    // 🏃 1. 몬스터 용사 감지 사거리(12.0) 내로 들어오면 용사를 향해 달려오기!
                    if (dist < AGGRO_RANGE) {
                    ...
                    // ⚔️ 2. 용사와 충돌(1.8 이하) 시 용사의 체력을 깎음
                    if (dist < 1.8) {
                        hp -= (m.touchDmg / 60);
                        hpChanged = true;
                    }
