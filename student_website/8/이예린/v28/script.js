// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v28 Engine
// (언덕 등반 높이 시스템 & 30마리 아기 몬스터 대군단)
// -------------------------------------------------------------
...
// -------------------------------------------------------------
// ⛰️ 3개의 언덕 데이터 및 실시간 등반 높이(Heightmap) 계산기
// -------------------------------------------------------------
const hillsData = [
...
function getForestElevation(x, z) {
    let maxElevation = 0;
    for (let i = 0; i < hillsData.length; i++) {
        ...
        if (dist < h.radius) {
            // 부드러운 코사인 곡면 언덕 높이
            const elevation = Math.cos((dist / h.radius) * (Math.PI / 2)) * h.height;
...
const npcsVillage = [
    { 
        ...
        color: 0xf39c12, 
        hatColor: 0xd35400,
        story: "허허, 어서오게 젊은 용사여!\n3개의 언덕 위로 직접 걸어 올라가면 숲 전체를 시원하게 내려다볼 수 있다네!\n30마리로 늘어난 귀여운 아기 몬스터들과 신나게 놀아보게나!",
        chatLines: ["허허, 언덕 위 전망이 참 좋구려~", "주희 양, 아기 슬라임들이 30마리나 있네!", "민우야, 언덕 오를 때 조심하렴!", "철수 자네 망치 소리가 경쾌하군!"],
    ...
    { 
        ...
        color: 0x2ecc71, 
        hatColor: 0x27ae60,
        story: "안녕하세요 용사님! 언덕 꼭대기와 파란 연못가에 귀여운 아기 몬스터들이 가득해요!\n용사님께서 300코인으로 칼을 단련해서 100그루 숲을 거닐어보세요!",
        chatLines: ["연못 물이 참 맑고 시원해요!", "이장님, 언덕에 산책 다녀오세요~", "영희 씨, 초록 언덕에 꽃이 만발했어요!"],
    ...
    { 
        ...
        color: 0xff7675, 
        hatColor: 0xd63031,
        story: "와! 언덕 위로 올라가니까 30마리 아기 몬스터가 다 보여요!\n형, 파란 연못에서 물장구치고 언덕 꼭대기에서 숨바꼭질해요!",
        chatLines: ["언덕 위에서 야호~ 외쳐봤어요!", "한스 아저씨, 산꼭대기에 올라가 봐요!", "와아~ 몬스터가 30마리나 있어!"],
        quest: { title: "숲속 탐험", desc: "넓어진 100그루 숲과 언덕 위를 탐험하기", targetType: "explore", rewardExp: 30, active: false, completed: false } 
    },
    { 
        id: 'villager4', 
        name: '정원사 영희', 
        ...
        color: 0x9b59b6, 
        hatColor: 0x8e44ad,
        story: "안녕하세요! 연두색 언덕 위에도 아기 여우들과 젤리 슬라임이 놀고 있어요.\n강화된 반짝이는 칼로 30마리 몬스터들과 신나게 모험해 보세요!",
        chatLines: ["언덕 위 꽃들이 바람에 춤추네요~", "주희 씨, 숲 향기가 너무 싱그러워요!", "마을과 숲이 초록빛으로 가득해요~"],
    ...
    { 
        ...
        color: 0x3498db, 
        hatColor: 0x2980b9,
        story: "반갑네 젊은 용사여! 3개의 언덕을 오르내리며 체력을 기르게나!\n대장장이 철수에게서 칼을 강력하게 벼리고 30마리 군단을 마주해보게나!",
        chatLines: ["언덕 능선을 타고 부는 바람이 시원하군...", "민우야, 절벽 쪽은 항상 조심하렴!", "대장장이 철수 칼솜씨는 여전히 최고야."],
...
for (let i = 0; i < 100; i++) {
    ...
    const elev = getForestElevation(tx, tz);
...
// -------------------------------------------------------------
// 👾 총 30마리 아기 몬스터 대군단 (언덕 위, 숲속, 연못가 배치)
// -------------------------------------------------------------
const monsters3D = [
    // 1~6: 서쪽 숲 & 서쪽 언덕
    ...
    // 19~24: 북동쪽 대산맥 주변
    { id: 'mountain_wolf1', baseName: '🦊 북동 산맥 아기여우', originX: 75, originZ: -50, patrolRadius: 12, chaseSpeed: 0.10, patrolSpeed: 0.048, color: 0xff9f43, baseHp: 90, baseDmg: 8, type: 'quad' },
    { id: 'mountain_dino1', baseName: '🦖 대산맥 초록공룡', originX: 85, originZ: -45, patrolRadius: 11, chaseSpeed: 0.085, patrolSpeed: 0.038, color: 0x20bf6b, baseHp: 90, baseDmg: 8, type: 'biped' },
    { id: 'mountain_slime1', baseName: '🍮 산바람 블루 슬라임', originX: 70, originZ: -60, patrolRadius: 10, chaseSpeed: 0.065, patrolSpeed: 0.03, color: 0x0984e3, baseHp: 75, baseDmg: 6, type: 'slime' },
    { id: 'mountain_ghost1', baseName: '👻 산봉우리 별요정', originX: 90, originZ: -55, patrolRadius: 10, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xffffff, baseHp: 90, baseDmg: 8, type: 'biped' },
    { id: 'mountain_spider1', baseName: '🐞 산맥 도트 무당벌레', originX: 65, originZ: -40, patrolRadius: 10, chaseSpeed: 0.09, patrolSpeed: 0.04, color: 0xff4757, baseHp: 85, baseDmg: 7, type: 'spider' },
    { id: 'mountain_slime2', baseName: '🍮 무지개 딸기 슬라임', originX: 80, originZ: -35, patrolRadius: 9, chaseSpeed: 0.06, patrolSpeed: 0.028, color: 0xff7675, baseHp: 70, baseDmg: 6, type: 'slime' },

    // 25~30: 북서쪽 산봉우리 & 100그루 깊은 숲속
    { id: 'nw_wolf1', baseName: '🦊 북서 산봉우리 여우', originX: -80, originZ: 45, patrolRadius: 12, chaseSpeed: 0.10, patrolSpeed: 0.05, color: 0xffbe76, baseHp: 90, baseDmg: 8, type: 'quad' },
    { id: 'nw_dino1', baseName: '🦖 숲속 민트 아기공룡', originX: -70, originZ: 50, patrolRadius: 11, chaseSpeed: 0.085, patrolSpeed: 0.038, color: 0x55efc4, baseHp: 85, baseDmg: 7, type: 'biped' },
    { id: 'nw_slime1', baseName: '🍮 바나나 젤리 슬라임', originX: -85, originZ: 35, patrolRadius: 9, chaseSpeed: 0.06, patrolSpeed: 0.03, color: 0xffd32a, baseHp: 70, baseDmg: 6, type: 'slime' },
    { id: 'nw_ghost1', baseName: '👻 구름 꼬마 유령', originX: -75, originZ: 60, patrolRadius: 10, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xffffff, baseHp: 85, baseDmg: 7, type: 'biped' },
...
monsters3D.forEach(m => {
    ...
    const elev = getForestElevation(m.x, m.z);
...
function updateUI() {
    ...
    if (currentMap === 'VILLAGE') {
        ...
        locationNameSpan.textContent = '🌲 3D 3개 언덕 & 30마리 몬스터 숲';
    }
}
...
function finishGame(deathReason = '') {
    ...
    if (deathReason && deathCauseText) {
        ...
        deathCauseText.textContent = '아기 몬스터들의 앙증맞은 펀치를 맞아 잠시 휴식을 취합니다!';
    }
...
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // ⛰️ [핵심] 언덕 위로 걸어 올라갈 때 시점 높이(Elevation) 실시간 상승!
        const currentElevation = (currentMap === 'FOREST') ? getForestElevation(player3D.x, player3D.z) : 0;
        ...
        if (currentMap === 'VILLAGE') {
            ...
            if (onHillName) {
                proximityNpcText.textContent = `⛰️ [${onHillName} 등반 중!] 높은 곳에서 숲 전체를 조망하고 있어요!`;
                npcProximityPrompt.classList.remove('hidden');
        ...
        // 🌲 30마리 숲속 아기 몬스터 실시간 언덕 오르내리기 & 추격 AI
        if (currentMap === 'FOREST') {
            let hpChanged = false;
            ...
            monsters3D.forEach(m => {
                if (m.alive) {
                    ...
                    if (distToPlayer < AGGRO_RANGE) {
                        ...
                        const mElev = getForestElevation(m.x, m.z);
