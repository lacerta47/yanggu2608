// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v17 Engine (대장간 입구 감지 순서 및 거리 완벽 수정)
// -------------------------------------------------------------
...
// 🚀 [핵심 버그 수정] 대장간 건물 입구 감지를 NPC 대화 감지보다 최우선으로 실행!
function triggerDialogueOrAttack(forceDialogueOnly = false) {
    if (gameState !== 'PLAYING') return;

    // 1. 마을에서 대장간 건물 입구 및 숲속 포탈 체크 (최우선!)
    if (currentMap === 'VILLAGE') {
        const distToBsDoor = Math.hypot(player3D.x - 0, player3D.z - 0.2);
        if (distToBsDoor < 5.2) { // 넉넉한 5.2 사거리로 대장간 입장!
            enterBlacksmithInterior();
            return;
        }
