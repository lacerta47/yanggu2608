// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v27 Engine
// (거대한 산 2개, 3개의 언덕, 100그루 나무숲과 푸른 연못 대탐험)
// -------------------------------------------------------------
...
const gateTag = createTagSprite('🌲 [Space] 100그루 대자연 숲 입장', '#2ed573');
...
const npcsVillage = [
    { 
        ...
        color: 0xf39c12, 
        hatColor: 0xd35400,
        story: "허허, 어서오게 젊은 용사여!\n숲속이 연두색과 초록색 언덕 3개, 100그루 나무숲과 파란 연못으로 넓어졌네!\n300코인으로 칼을 단련하고 웅장한 두 개의 산을 구경해보게나!",
        chatLines: ["허허, 숲속 연못이 참 맑구려~", "주희 양, 숲속에 나무가 100그루나 생겼네!", "민우야, 연못 근처는 조심하렴!", "철수 자네 망치 소리가 경쾌하군!"],
    ...
    { 
        ...
        color: 0x2ecc71, 
        hatColor: 0x27ae60,
        story: "안녕하세요 용사님! 파란 연못 주위에 예쁜 수련꽃과 약초가 듬뿍 피었어요!\n용사님께서 300코인으로 칼을 단련해서 100그루 숲을 거닐어보세요!",
        chatLines: ["연못 물이 참 맑고 시원해요!", "이장님, 건강 챙기세요~", "영희 씨, 초록 언덕에 꽃이 만발했어요!"],
    ...
    { 
        ...
        color: 0xff7675, 
        hatColor: 0xd63031,
        story: "와! 저기 거대한 산 두 개랑 언덕 세 개가 보여요!\n형, 파란 연못에서 물장구치고 100그루 나무숲에서 숨바꼭질해요!",
        chatLines: ["연못에 수련잎이 동동 떠 있어요!", "한스 아저씨, 산꼭대기에 올라가 봐요!", "와아~ 숲속이 엄청 넓어졌다!"],
        quest: { title: "숲속 탐험", desc: "넓어진 100그루 숲과 연못으로 건너가 탐험하기", targetType: "explore", rewardExp: 30, active: false, completed: false } 
    },
    { 
        id: 'villager4', 
        name: '정원사 영희', 
        ...
        color: 0x9b59b6, 
        hatColor: 0x8e44ad,
        story: "안녕하세요! 연두색 언덕과 파란 연못이 동화 속 세상처럼 아름다워요.\n강화된 반짝이는 칼로 아기 몬스터들과 신나게 모험해 보세요!",
        chatLines: ["연못가 수련꽃이 활짝 피었네요~", "주희 씨, 숲 향기가 너무 싱그러워요!", "마을과 숲이 초록빛으로 가득해요~"],
    ...
    { 
        ...
        color: 0x3498db, 
        hatColor: 0x2980b9,
        story: "반갑네 젊은 용사여! 북쪽 산맥 2개와 3개의 언덕이 웅장하게 솟았네!\n대장장이 철수에게서 칼을 강력하게 벼리고 대자연을 누벼보게나!",
        chatLines: ["푸른 산맥에서 불어오는 바람이 상쾌하군...", "민우야, 절벽 쪽은 항상 조심하렴!", "대장장이 철수 칼솜씨는 여전히 최고야."],
...
// -------------------------------------------------------------
// 🌲 3D 대확장 숲 맵 (연두색/초록색 잔디, 언덕 3개, 산 2개, 파란 연못, 100그루 나무)
// -------------------------------------------------------------
// 1. 대확장 연두색 & 초록색 대지 (320m x 200m)
...
// 잔디 패치 디테일
const grassPatch1 = new THREE.Mesh(new THREE.PlaneGeometry(160, 100), new THREE.MeshStandardMaterial({ color: 0x7bed9f, roughness: 0.9 }));
grassPatch1.rotation.x = -Math.PI / 2;
grassPatch1.position.set(-30, 0.02, 20);
forestGroup.add(grassPatch1);

const grassPatch2 = new THREE.Mesh(new THREE.PlaneGeometry(140, 90), new THREE.MeshStandardMaterial({ color: 0x20bf6b, roughness: 0.8 }));
grassPatch2.rotation.x = -Math.PI / 2;
grassPatch2.position.set(40, 0.03, -20);
forestGroup.add(grassPatch2);

// 2. ⛰️ 3개의 크고 작은 언덕들 (3 Hills)
function createGreenHill(x, z, radius, height, colorHex = 0x2ed573) {
    const hillGeo = new THREE.SphereGeometry(radius, 24, 16, 0, Math.PI * 2, 0, Math.PI / 2);
    const hillMat = new THREE.MeshStandardMaterial({ color: colorHex, roughness: 0.8 });
    const hill = new THREE.Mesh(hillGeo, hillMat);
    hill.scale.set(1, height / radius, 1);
    hill.position.set(x, 0, z);
    forestGroup.add(hill);
}
// 언덕 1: 동쪽 큰 연두색 언덕
createGreenHill(45, 25, 26, 7.5, 0x7bed9f);
// 언덕 2: 서쪽 중간 초록색 언덕
createGreenHill(-45, -30, 22, 6.0, 0x2ed573);
// 언덕 3: 남쪽 완만한 아기 에메랄드 언덕
createGreenHill(10, -55, 18, 4.5, 0x20bf6b);

// 3. 🏔️ 2개의 거대한 산 (2 Mountains)
function createGreatMountain(x, z, radius, height, rockColor = 0x57606f, peakColor = 0xf1f2f6) {
    const mountainGroup = new THREE.Group();

    // 산체 본체
    const coneGeo = new THREE.ConeGeometry(radius, height, 16);
    const coneMat = new THREE.MeshStandardMaterial({ color: rockColor, roughness: 0.9 });
    const cone = new THREE.Mesh(coneGeo, coneMat);
    cone.position.y = height / 2;
    mountainGroup.add(cone);

    // 산기슭 초록 삼림 경사
    const skirtGeo = new THREE.ConeGeometry(radius * 1.25, height * 0.45, 16);
    const skirtMat = new THREE.MeshStandardMaterial({ color: 0x10ac84, roughness: 0.85 });
    const skirt = new THREE.Mesh(skirtGeo, skirtMat);
    skirt.position.y = height * 0.22;
    mountainGroup.add(skirt);

    // 하얀 눈/봉우리 정상
    const peakGeo = new THREE.ConeGeometry(radius * 0.35, height * 0.35, 12);
    const peakMat = new THREE.MeshStandardMaterial({ color: peakColor, roughness: 0.4 });
    const peak = new THREE.Mesh(peakGeo, peakMat);
    peak.position.y = height * 0.83;
...
// 산 1: 북동쪽 웅장한 대산맥
createGreatMountain(95, -65, 42, 28, 0x4b6584, 0xffffff);
// 산 2: 북서쪽 험준한 산봉우리
createGreatMountain(-95, 60, 38, 24, 0x57606f, 0xffffff);

// 4. 💧 시원하고 맑은 파란색 연못 (Blue Pond)
...
// 5. 🌲 100그루의 울창한 다채로운 나무들 (100 Trees Generator)
...
// 100그루 나무를 숲 전체에 골고루 배치 (연못 중심 x:0, z:15는 피해서 배치)
const leafColors = [0x7bed9f, 0x2ed573, 0x20bf6b, 0x26de81, 0x10ac84, 0x1dd1a1, 0x55efc4];
const treeList = [];

// 결정론적 의사 난수로 매번 동일하고 예쁘게 100그루 나무 배치
for (let i = 0; i < 100; i++) {
    ...
    // 연못(x: 0, z: 15, 반경 18)이나 입구 게이트(x: -60, z: 0) 근처면 바깥으로 이동
...
// 귀여운 딸기 버섯 16개
function createCuteMushroom(x, z, capColor = 0xff7675) {
    const shroom = new THREE.Group();
    const stem = new THREE.Mesh(new THREE.CylinderGeometry(0.12, 0.15, 0.45, 8), new THREE.MeshStandardMaterial({ color: 0xffffff }));
    stem.position.y = 0.22;
    shroom.add(stem);

    const cap = new THREE.Mesh(new THREE.ConeGeometry(0.4, 0.35, 12), new THREE.MeshStandardMaterial({ color: capColor }));
    cap.position.y = 0.48;
    shroom.add(cap);

    const dot = new THREE.Mesh(new THREE.SphereGeometry(0.06, 6, 6), new THREE.MeshBasicMaterial({ color: 0xffffff }));
    dot.position.set(0.15, 0.52, 0.2);
    shroom.add(dot);
...
const shroomCoords = [
    { x: -50, z: -8, c: 0xff7675 }, { x: -42, z: 6, c: 0xfd79a8 },
    { x: -28, z: -5, c: 0xffd32a }, { x: -15, z: 8, c: 0xff7675 },
    { x: -2,  z: -8, c: 0x7bed9f }, { x: 12,  z: 6, c: 0xffd32a },
    { x: 28,  z: -6, c: 0xff7675 }, { x: 45,  z: 8, c: 0xfd79a8 },
    { x: 58,  z: -4, c: 0xffd32a }, { x: 75,  z: 15, c: 0xff7675 },
    { x: -75, z: -15, c: 0xfd79a8 }, { x: 30, z: 45, c: 0x7bed9f }
];
shroomCoords.forEach(sc => createCuteMushroom(sc.x, sc.z, sc.c));
...
const monsters3D = [
    ...
    { id: 'golem', baseName: '🧸 테디 베어 골렘', originX: 45, originZ: 25, patrolRadius: 10, chaseSpeed: 0.048, patrolSpeed: 0.02, color: 0xcd84f1, baseHp: 100, baseDmg: 9, type: 'biped' },
    { id: 'spider1', baseName: '🐞 아기 무당벌레', originX: 25, originZ: -35, patrolRadius: 12, chaseSpeed: 0.085, patrolSpeed: 0.038, color: 0xff4757, baseHp: 80, baseDmg: 6, type: 'spider' },

    { id: 'skeleton1', baseName: '👻 꼬마 별요정 유령', originX: 35, originZ: -15, patrolRadius: 12, chaseSpeed: 0.075, patrolSpeed: 0.032, color: 0xffffff, baseHp: 90, baseDmg: 7, type: 'biped' },
    { id: 'wolf2', baseName: '🦊 솜사탕 아기 여우', originX: 65, originZ: -20, patrolRadius: 14, chaseSpeed: 0.10, patrolSpeed: 0.05, color: 0xffbe76, baseHp: 85, baseDmg: 8, type: 'quad' },
    { id: 'spider2', baseName: '🐞 핑크 무당벌레', originX: -55, originZ: 35, patrolRadius: 10, chaseSpeed: 0.09, patrolSpeed: 0.04, color: 0xfd79a8, baseHp: 85, baseDmg: 7, type: 'spider' },
    { id: 'skeleton2', baseName: '👻 꼬마 천사 유령', originX: 70, originZ: 35, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xfff200, baseHp: 95, baseDmg: 8, type: 'biped' }
];
...
btnRight.addEventListener('mouseup', () => { keys.right = false; pressTime = Date.now(); });
...
function updateUI() {
    ...
    if (currentMap === 'VILLAGE') {
        ...
        locationNameSpan.textContent = '🌲 3D 100그루 숲 & 푸른 연못';
    }
}
...
function finishGame(deathReason = '') {
    ...
    if (deathReason && deathCauseText) {
        ...
        deathCauseText.textContent = '아기 몬스터들의 통통 펀치를 맞아 휴식을 취합니다!';
    }
...
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        if (currentMap === 'VILLAGE') {
            ...
            // 대확장된 숲속 땅 경계: x: -155 ~ 155, z: -95 ~ 95
            ...
            if (distToPond < 16.5) {
                proximityNpcText.textContent = `💧 [맑은 푸른 연못] 시원한 파란 물이 찰랑이고 수련꽃이 피어있어요!`;
                npcProximityPrompt.classList.remove('hidden');
            } else {
                const distToVillageGate = Math.hypot(player3D.x - (-60), player3D.z - 0);
        ...
        if (currentMap === 'FOREST') {
            ...
            if (hp <= 0) {
                hp = 0;
                finishGame('아기 몬스터들의 앙증맞은 펀치를 맞아 잠시 휴식을 취합니다!');
            }
        }
    }
