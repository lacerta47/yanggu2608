// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v24 Engine
// (다크 네온 & 붉은 악마 몬스터 디자인 대침공 시스템)
// -------------------------------------------------------------
...
const npcsVillage = [
    { 
        ...
        color: 0xf39c12, 
        hatColor: 0xd35400,
        story: "허허, 어서오게 젊은 용사여!\n숲속 몬스터들이 붉은 안광과 뾰족한 가시를 두르고 악마처럼 무서워졌네!\n300코인으로 칼을 단련하고 마을을 꼭 지켜주게나!",
    ...
    { 
        ...
        color: 0x2ecc71, 
        hatColor: 0x27ae60,
        story: "안녕하세요 용사님! 숲속 슬라임들이 붉은 뿔을 달고 이빨을 드러내서 너무 무서워요...\n용사님께서 300코인으로 칼을 단련해서 혼내주세요!",
    ...
    { 
        ...
        color: 0xe74c3c, 
        hatColor: 0xc0392b,
        story: "으악! 숲속 해골이 커다란 사신의 낫을 들고 날개를 펼쳤대요!\n형, 얼른 칼을 강화해서 무시무시한 사신 해골을 물리쳐줘요!",
...
// -------------------------------------------------------------
// 😈 1번(붉은 악마 뿔/가시) + 2번(다크 네온/사신 날개/낫) 3D 모델링
// -------------------------------------------------------------
function buildScaryDemonMonsterMesh(m) {
    ...
    if (m.type === 'slime') {
        ...
        const fang = new THREE.Mesh(new THREE.ConeGeometry(0.04, 0.12, 6), new THREE.MeshBasicMaterial({ color: 0xffffff }));
        fang.rotation.x = Math.PI;
        fang.position.set(0, 0.68, 0.85);
        group.add(fang);
    } 
    // 2. 👹 블러드 고블린 / 💀 사신 해골 / 🗿 옵시디언 골렘
    else if (m.type === 'biped') {
        const isSkeleton = m.id.includes('skeleton');
        const isGolem = m.id.includes('golem');

        if (isSkeleton) {
            // 💀 사신 해골 전사 (보랏빛 망토 + 타오르는 붉은 눈 + 뼈 날개 + 거대한 저승 낫!)
            ...
            // 🗿 카오스 옵시디언 골렘 (흑요석 + 붉은 마그마 균열 + 거대한 가시 주먹)
            ...
            // 어깨의 거대한 흑요석 가시 바위
            for (let s = -1; s <= 1; s += 2) {
                const boulder = new THREE.Mesh(new THREE.DodecahedronGeometry(0.65), new THREE.MeshStandardMaterial({ color: 0x2f3640 }));
                boulder.position.set(s * 1.2, 2.1, 0);
                group.add(boulder);

                const spike = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.6, 6), new THREE.MeshStandardMaterial({ color: 0xff3838 }));
                spike.position.set(s * 1.35, 2.6, 0);
                spike.rotation.z = s * (Math.PI / 4);
                group.add(spike);
            }
        } else {
            // 👹 블러드 고블린 (암적색 바디 + 뾰족한 가시 어깨 + 붉은 가시 곤봉)
            ...
            // 뾰족한 악마 귀 2개
            const leftEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.45, 6), new THREE.MeshStandardMaterial({ color: 0x780016 }));
            leftEar.position.set(-0.55, 1.9, 0);
            leftEar.rotation.z = Math.PI / 3;
            const rightEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.45, 6), new THREE.MeshStandardMaterial({ color: 0x780016 }));
            ...
            const clubSpike = new THREE.Mesh(new THREE.ConeGeometry(0.15, 0.4, 6), new THREE.MeshBasicMaterial({ color: 0xff3838 }));
            clubSpike.position.set(0.65, 1.45, 0.5);
            group.add(clubSpike);
        }
    } 
    // 3. 🐺 지옥의 늑대 (Shadow Hellhound - 칠흑 바디 + 붉은 가시 갈기 + 야생 안광)
    else if (m.type === 'quad') {
        ...
        const wolfHead = new THREE.Mesh(
            ...
            new THREE.MeshStandardMaterial({ color: 0x1e1e24 })
        );
        wolfHead.rotation.x = Math.PI / 2;
        wolfHead.position.set(0, 1.25, 0.95);
        group.add(wolfHead);

        const wolfNose = new THREE.Mesh(new THREE.SphereGeometry(0.09, 8, 8), new THREE.MeshBasicMaterial({ color: 0xff0000 }));
        wolfNose.position.set(0, 1.25, 1.42);
        group.add(wolfNose);

        addDetailedEyes(group, 1.4, 1.0, 0.16, 0.09, 0xff3838);

        // 뾰족한 악마 뿔 귀
        const leftEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.45, 6), new THREE.MeshStandardMaterial({ color: 0xff3838 }));
        leftEar.position.set(-0.25, 1.7, 0.75);
        const rightEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.45, 6), new THREE.MeshStandardMaterial({ color: 0xff3838 }));
        rightEar.position.set(0.25, 1.7, 0.75);
        group.add(leftEar);
        group.add(rightEar);

        for (let lx = -0.32; lx <= 0.32; lx += 0.64) {
            for (let lz = -0.55; lz <= 0.55; lz += 1.1) {
                const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.08, 0.8, 6), new THREE.MeshStandardMaterial({ color: 0x000000 }));
                leg.position.set(lx, 0.4, lz);
                group.add(leg);
            }
        }
    } 
    // 4. 🕷️ 다크 네온 거미 (Dark Neon Spider - 보랏빛 네온 복부 + 4개의 핏빛 겹눈 + 가시 다리)
        ...
        for (let i = -1; i <= 1; i += 2) {
            ...
            eye2.position.set(i * 0.34, 0.65, 0.72);
            group.add(eye1);
            group.add(eye2);
        }
        ...
        for (let i = 0; i < 4; i++) {
            ...
            for (let s = -1; s <= 1; s += 2) {
                const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.02, 1.3, 6), new THREE.MeshStandardMaterial({ color: 0x050505 }));
                leg.position.set(s * 0.75, 0.5, zOff);
                leg.rotation.z = s * 0.85;
                leg.rotation.x = (i - 1.5) * 0.25;
                group.add(leg);
            }
        }
    }
...
const monsters3D = [
    { id: 'slime1', baseName: '😈 지옥 슬라임', originX: -55, originZ: -4, patrolRadius: 10, chaseSpeed: 0.055, patrolSpeed: 0.025, color: 0xeb2f06, baseHp: 50, baseDmg: 5, type: 'slime', scale: 0.85 },
    { id: 'goblin1', baseName: '👹 블러드 고블린', originX: -42, originZ: 6, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0x780016, baseHp: 60, baseDmg: 6, type: 'biped' },
    { id: 'slime2', baseName: '😈 마그마 슬라임', originX: -30, originZ: -6, patrolRadius: 8, chaseSpeed: 0.06, patrolSpeed: 0.03, color: 0xff3838, baseHp: 50, baseDmg: 4, type: 'slime', scale: 0.65 },

    { id: 'wolf1', baseName: '🐺 지옥의 늑대', originX: -15, originZ: 4, patrolRadius: 14, chaseSpeed: 0.095, patrolSpeed: 0.045, color: 0x1e1e24, baseHp: 70, baseDmg: 7, type: 'quad' },
    ...
    { id: 'spider2', baseName: '🕷️ 거대 심연 거미', originX: 50, originZ: 4, patrolRadius: 10, chaseSpeed: 0.09, patrolSpeed: 0.04, color: 0x3c40c6, baseHp: 85, baseDmg: 7, type: 'spider' },
    { id: 'skeleton2', baseName: '💀 정예 저승 사신', originX: 62, originZ: -3, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xffffff, baseHp: 95, baseDmg: 8, type: 'biped' }
];
