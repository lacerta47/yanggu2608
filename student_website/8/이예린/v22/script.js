// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v22 Engine
// (주민 자율 배회 & 상호 대화 시스템 + 대장장이 칼 강화 대화창 완벽 연동)
// -------------------------------------------------------------
...
// 💬 주민들끼리 대화할 때 머리 위에 뜨는 귀여운 실시간 3D 말풍선
...
function updateMonsterHpSprite(m) {
    if (!m.hpSpriteCanvas) {
        ...
        m.hpSprite.position.set(0, 2.2, 0);
    }
...
// 🚶 마을 주민들 (자율 산책 & 상호 대화 & 역할별 흥미진진한 스토리)
const npcsVillage = [
    { 
        id: 'villager0', 
        name: '이장님 성민', 
        ...
        hatColor: 0xd35400,
        talkCooldown: 0,
        story: "허허, 어서오게 젊은 용사여!\n우리 마을은 내가 40년 동안 지켜온 평화로운 곳이라네.\n요즘 동쪽 숲속 몬스터들이 레벨업할 때마다 공격력이 +10씩 매섭게 강해지고 있으니, 대장간에서 칼을 단련하고 마을을 꼭 지켜주게나!",
        chatLines: ["허허, 평화로운 날씨구려~", "주희 양, 약초는 잘 자라는가?", "민우야, 조심해서 놀아라!", "철수 자네 망치 소리가 우렁차군!"],
    ...
    { 
        ...
        hatColor: 0x27ae60,
        talkCooldown: 0,
        story: "안녕하세요 용사님! 전 아픈 이웃들을 돌보는 약초꾼 주희예요.\n숲속 슬라임들이 약초밭을 자꾸 밟아서 상처 치료제 만들기가 너무 힘들어요...\n용사님께서 숲속 몬스터들을 혼내주시면 약초도 구하고 넉넉한 보상을 드릴게요!",
    ...
    { 
        ...
        hatColor: 0xc0392b,
        talkCooldown: 0,
        story: "와! 칼을 든 멋진 용사 형이다!\n나도 얼른 커서 형처럼 멋진 칼을 들고 전설의 보물을 찾으러 갈 거예요!\n동쪽 숲속에는 거대한 골렘이랑 살아있는 늑대가 돌아다닌대요! 조심하세요!",
        chatLines: ["나도 용사처럼 멋진 검을 가질 테야!", "한스 아저씨, 모험 이야기 또 들려주세요!", "와아~ 숲속에 가보고 싶다!"],
    ...
    { 
        ...
        hatColor: 0x8e44ad,
        talkCooldown: 0,
        story: "안녕하세요! 향기로운 장미꽃과 튤립 정원을 가꾸는 영희랍니다.\n숲속 거미들이 꽃밭에 거미줄을 치고 있어서 꽃들이 햇빛을 못 보고 있어요...\n멋진 칼솜씨로 몬스터들을 소탕해주시면 예쁜 꽃들이 다시 활짝 필 거예요!",
    ...
    { 
        ...
        hatColor: 0x2980b9,
        talkCooldown: 0,
        story: "반갑네 젊은 용사여! 전 세계 7대 대륙을 누볐던 전설의 모험가 한스라네!\n몬스터들은 레벨이 오를수록 강력한 +10 공격력을 뿜어내지만, 두려워하지 말고 틈을 노려 공격하게나!\n레벨 3에 도달해 진정한 용사의 자격을 증명해보게!",
        chatLines: ["바람의 냄새가 모험을 부르는군...", "민우야, 용기를 잃지 않는 게 진정한 모험이란다!", "대장장이 철수 칼솜씨는 여전히 최고야."],
...
// 🧔 디테일 대장장이 철수 NPC (칼 강화 전용 대화창 연동)
const blacksmithNpc = {
    id: 'villager2',
    name: '대장장이 철수',
    role: '3D 대장간 장인',
...
// -------------------------------------------------------------
// 👾 살아 숨쉬는 3D 몬스터 모델링 빌더 (눈동자 + 뿔 + 귀)
// -------------------------------------------------------------
function buildDetailedMonsterMesh(m) {
    const group = new THREE.Group();

    if (m.type === 'slime') {
        const slimeBody = new THREE.Mesh(
            new THREE.SphereGeometry(m.scale || 0.85, 16, 16),
            new THREE.MeshStandardMaterial({ color: m.color, roughness: 0.15, transparent: true, opacity: 0.92 })
        );
        slimeBody.position.y = 0.75;
        slimeBody.scale.set(1.15, 0.85, 1.1);
        group.add(slimeBody);

        const nub = new THREE.Mesh(new THREE.ConeGeometry(0.18, 0.35, 8), new THREE.MeshStandardMaterial({ color: m.color }));
        nub.position.set(0, 1.35, 0);
        group.add(nub);

        addDetailedEyes(group, 0.82, 0.72, 0.28, 0.14, 0x111111);

        const smile = new THREE.Mesh(new THREE.TorusGeometry(0.12, 0.03, 8, 12, Math.PI), new THREE.MeshBasicMaterial({ color: 0x111111 }));
        smile.rotation.x = Math.PI;
        smile.position.set(0, 0.65, 0.82);
        group.add(smile);
    } else if (m.type === 'biped') {
        const isSkeleton = m.id.includes('skeleton');
        const isGolem = m.id.includes('golem');

        const bodyGeo = isGolem ? new THREE.BoxGeometry(1.6, 1.8, 1.4) : new THREE.BoxGeometry(0.9, 1.2, 0.7);
        const body = new THREE.Mesh(bodyGeo, new THREE.MeshStandardMaterial({ color: m.color, roughness: 0.6 }));
        body.position.y = isGolem ? 1.4 : 0.9;
        group.add(body);

        const headGeo = isGolem ? new THREE.BoxGeometry(1.1, 0.9, 1.0) : new THREE.SphereGeometry(0.48, 14, 14);
        const head = new THREE.Mesh(headGeo, new THREE.MeshStandardMaterial({ color: isSkeleton ? 0xf5f6fa : m.color }));
        head.position.y = isGolem ? 2.5 : 1.8;
        group.add(head);

        if (isSkeleton) {
            addDetailedEyes(group, 1.85, 0.42, 0.18, 0.12, 0x050505);
            const helm = new THREE.Mesh(new THREE.ConeGeometry(0.55, 0.5, 8), new THREE.MeshStandardMaterial({ color: 0x7f8c8d, metalness: 0.8 }));
            helm.position.y = 2.25;
            group.add(helm);
        } else if (isGolem) {
            const leftGlow = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.2, 0.1), new THREE.MeshBasicMaterial({ color: 0x00f3ff }));
            leftGlow.position.set(-0.32, 2.5, 0.52);
            const rightGlow = new THREE.Mesh(new THREE.BoxGeometry(0.2, 0.2, 0.1), new THREE.MeshBasicMaterial({ color: 0x00f3ff }));
            rightGlow.position.set(0.32, 2.5, 0.52);
            group.add(leftGlow);
            group.add(rightGlow);

            const leftBoulder = new THREE.Mesh(new THREE.DodecahedronGeometry(0.55), new THREE.MeshStandardMaterial({ color: 0x7f8c8d }));
            leftBoulder.position.set(-1.1, 2.1, 0);
            const rightBoulder = new THREE.Mesh(new THREE.DodecahedronGeometry(0.55), new THREE.MeshStandardMaterial({ color: 0x7f8c8d }));
            rightBoulder.position.set(1.1, 2.1, 0);
            group.add(leftBoulder);
            group.add(rightBoulder);
        } else {
            addDetailedEyes(group, 1.85, 0.44, 0.18, 0.1, 0xf1c40f);

            const leftEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.4, 6), new THREE.MeshStandardMaterial({ color: m.color }));
            leftEar.position.set(-0.55, 1.9, 0);
            leftEar.rotation.z = Math.PI / 3;
            const rightEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.4, 6), new THREE.MeshStandardMaterial({ color: m.color }));
            rightEar.position.set(0.55, 1.9, 0);
        ...
        wolfHead.position.set(0, 1.25, 0.9);
        group.add(wolfHead);

        const wolfNose = new THREE.Mesh(new THREE.SphereGeometry(0.08, 8, 8), new THREE.MeshBasicMaterial({ color: 0x111111 }));
        wolfNose.position.set(0, 1.25, 1.35);
        group.add(wolfNose);

        addDetailedEyes(group, 1.38, 0.95, 0.16, 0.08, 0xffd32a);

        const leftEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.35, 6), new THREE.MeshStandardMaterial({ color: m.color }));
        leftEar.position.set(-0.25, 1.65, 0.7);
        const rightEar = new THREE.Mesh(new THREE.ConeGeometry(0.12, 0.35, 6), new THREE.MeshStandardMaterial({ color: m.color }));
        rightEar.position.set(0.25, 1.65, 0.7);
        group.add(leftEar);
        group.add(rightEar);

        for (let lx = -0.32; lx <= 0.32; lx += 0.64) {
            for (let lz = -0.55; lz <= 0.55; lz += 1.1) {
                const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.1, 0.08, 0.8, 6), new THREE.MeshStandardMaterial({ color: 0x2d3436 }));
                leg.position.set(lx, 0.4, lz);
                group.add(leg);
            }
        }
    } else if (m.type === 'spider') {
        const headSphere = new THREE.Mesh(new THREE.SphereGeometry(0.5, 12, 12), new THREE.MeshStandardMaterial({ color: m.color }));
        headSphere.position.set(0, 0.65, 0.35);
        group.add(headSphere);

        const abdomen = new THREE.Mesh(new THREE.SphereGeometry(0.75, 14, 14), new THREE.MeshStandardMaterial({ color: 0x4a148c, roughness: 0.4 }));
        abdomen.position.set(0, 0.8, -0.6);
        group.add(abdomen);
        ...
        for (let i = -1; i <= 1; i += 2) {
            ...
            eye2.position.set(i * 0.32, 0.65, 0.72);
            group.add(eye1);
            group.add(eye2);
        }

        for (let i = 0; i < 4; i++) {
            const zOff = -0.3 + i * 0.25;
            for (let s = -1; s <= 1; s += 2) {
                const leg = new THREE.Mesh(new THREE.CylinderGeometry(0.05, 0.03, 1.2, 6), new THREE.MeshStandardMaterial({ color: 0x1e272e }));
                leg.position.set(s * 0.7, 0.5, zOff);
...
const monsters3D = [
    { id: 'slime1', baseName: '🟢 숲 슬라임', originX: -55, originZ: -4, patrolRadius: 10, chaseSpeed: 0.055, patrolSpeed: 0.025, color: 0x2ecc71, baseHp: 50, baseDmg: 5, type: 'slime', scale: 0.85 },
    { id: 'goblin1', baseName: '🔴 성난 고블린', originX: -42, originZ: 6, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xe74c3c, baseHp: 60, baseDmg: 6, type: 'biped' },
    { id: 'slime2', baseName: '🟢 아기 슬라임', originX: -30, originZ: -6, patrolRadius: 8, chaseSpeed: 0.06, patrolSpeed: 0.03, color: 0x1abc9c, baseHp: 50, baseDmg: 4, type: 'slime', scale: 0.65 },

    { id: 'wolf1', baseName: '🐺 숲 늑대', originX: -15, originZ: 4, patrolRadius: 14, chaseSpeed: 0.095, patrolSpeed: 0.045, color: 0x7f8c8d, baseHp: 70, baseDmg: 7, type: 'quad' },
    { id: 'golem', baseName: '🗿 암석 골렘', originX: 0, originZ: 0, patrolRadius: 10, chaseSpeed: 0.048, patrolSpeed: 0.02, color: 0x95a5a6, baseHp: 100, baseDmg: 9, type: 'biped' },
    { id: 'spider1', baseName: '🕷️ 독 거미', originX: 12, originZ: -5, patrolRadius: 12, chaseSpeed: 0.085, patrolSpeed: 0.038, color: 0x9b59b6, baseHp: 80, baseDmg: 6, type: 'spider' },

    { id: 'skeleton1', baseName: '👻 해골 전사', originX: 25, originZ: 5, patrolRadius: 12, chaseSpeed: 0.075, patrolSpeed: 0.032, color: 0xf1f2f6, baseHp: 90, baseDmg: 7, type: 'biped' },
    { id: 'wolf2', baseName: '🐺 우두머리 늑대', originX: 38, originZ: -6, patrolRadius: 14, chaseSpeed: 0.10, patrolSpeed: 0.05, color: 0x34495e, baseHp: 85, baseDmg: 8, type: 'quad' },
    { id: 'spider2', baseName: '🕷️ 거대 거미', originX: 50, originZ: 4, patrolRadius: 10, chaseSpeed: 0.09, patrolSpeed: 0.04, color: 0x8e44ad, baseHp: 85, baseDmg: 7, type: 'spider' },
    { id: 'skeleton2', baseName: '👻 정예 해골', originX: 62, originZ: -3, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xdcdde1, baseHp: 95, baseDmg: 8, type: 'biped' }
];
...
monsters3D.forEach(m => {
    ...
    const mMesh = buildDetailedMonsterMesh(m);
...
function openNpcDialogue(npc) {
    ...
    // 💬 [핵심 구현] 대화할 때 NPC가 플레이어를 똑바로 마주 보게 회전!
    if (npc.mesh) {
        npc.mesh.lookAt(player3D.x, npc.mesh.position.y, player3D.z);
    }

    // 🔨 대장장이 철수일 경우 칼 강화 패널 표시!
    if (npc.id === 'villager2') {
...
btnForgeWeapon.addEventListener('click', (e) => {
    ...
    if (isSuccess) {
        ...
        updateForgeUI(`🎉 [강화 성공!] 칼 공격력이 +5 상승했습니다! (현재 칼 데미지: ${swordDamage})`);
    } else {
        playCustomSound('hit');
        updateForgeUI(`💥 [강화 실패!] 강철 벼림에 실패했습니다... (다음 도전: ${getForgeCost()}코인)`);
    }
...
function gainExp(amount) {
    ...
    while (exp >= targetExp) {
        ...
        playCustomSound('yap');
        showLevelUpToast(`🎉 LEVEL UP! Lv.${level} 달성! (몬스터 피 +30 & 공격력 +10 상승!)`);
    }
...
// 시작 화면에서 Enter 키를 눌러도 바로 시작되도록 지원
...
// -------------------------------------------------------------
// 3D 1인칭 메인 애니메이션 & 주민 상호 대화 & 몬스터 루프
// -------------------------------------------------------------
function animate() {
