// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v6 Engine (Three.js 1st-Person View)
// -------------------------------------------------------------
...
// 1인칭 화면 오른쪽 아래 칼 배치
fpSwordGroup.position.set(0.4, -0.45, -0.7);
...
createHouse(-18, -5, 0xe67e22);
createHouse(18, -5, 0xd35400);
...
// 마을 NPC 6명
const npcs3D = [
    { id: 'elder', name: '마을 이장님', role: '마을 수호자', avatar: '👴', x: -11, color: 0xffdd59, quest: { title: "이장님의 청부", desc: "숲속 몬스터 2마리 처치하기 (진행: 0/2)", targetType: "kill", targetCount: 2, rewardExp: 50, active: false, completed: false } },
    { id: 'villager1', name: '약초꾼 주희', role: '마을 주민', avatar: '👩‍🌾', x: -6, color: 0x2ecc71, quest: { title: "약초밭 지키기", desc: "숲속 몬스터 4마리 처치하기 (진행: 0/4)", targetType: "kill", targetCount: 4, rewardExp: 60, active: false, completed: false } },
    { id: 'villager2', name: '대장장이 철수', role: '마을 주민', avatar: '🧔', x: -1, color: 0xe67e22, quest: { title: "칼 단련 시험", desc: "Lv.2 도달하기", targetType: "level", targetLevel: 2, rewardExp: 40, active: false, completed: false } },
    { id: 'villager3', name: '꼬마 민우', role: '마을 주민', avatar: '👦', x: 4, color: 0xe74c3c, quest: { title: "숲속 탐험", desc: "숲 맵으로 건너가 탐험해보기", targetType: "explore", rewardExp: 30, active: false, completed: false } },
    ...
    { id: 'villager5', name: '모험가 한스', role: '마을 주민', avatar: '🧙‍♂️', x: 14, color: 0x3498db, quest: { title: "진정한 용사의 길", desc: "Lv.3 도달하기", targetType: "level", targetLevel: 3, rewardExp: 100, active: false, completed: false } }
];
...
npcs3D.forEach(npc => {
    ...
    npcMesh.position.set(npc.x, 0.9, -1.5);
...
// -------------------------------------------------------------
// 3D 숲 (Map 2) 에셋 및 다채로운 6종 몬스터 추가!
// -------------------------------------------------------------
...
for (let i = -24; i <= 24; i += 6) {
    ...
    fTree.position.set(i, 0, -5.5);
    forestGroup.add(fTree);
}

// 👾 숲에 추가된 6종의 다채로운 3D 몬스터들!
...
const monsters3D = [
    { id: 'slime', baseName: '🟢 숲 슬라임', x: -16, geo: new THREE.DodecahedronGeometry(0.8), color: 0x2ecc71, baseHp: 100, baseDmg: 5 },
    { id: 'goblin', baseName: '🔴 분노한 고블린', x: -8, geo: new THREE.BoxGeometry(1.2, 1.4, 1.2), color: 0xe74c3c, baseHp: 110, baseDmg: 6 },
    { id: 'wolf', baseName: '🐺 숲 늑대', x: 0, geo: new THREE.CylinderGeometry(0.6, 0.8, 1.5, 8), color: 0x7f8c8d, baseHp: 120, baseDmg: 7 },
    { id: 'golem', baseName: '🗿 암석 골렘', x: 8, geo: new THREE.BoxGeometry(1.6, 2.0, 1.6), color: 0x95a5a6, baseHp: 150, baseDmg: 8 },
    { id: 'spider', baseName: '🕷️ 독 거미', x: 14, geo: new THREE.SphereGeometry(0.9, 12, 12), color: 0x9b59b6, baseHp: 105, baseDmg: 6 },
    { id: 'skeleton', baseName: '👻 해골 전사', x: 20, geo: new THREE.CylinderGeometry(0.5, 0.5, 2.0, 8), color: 0xf1f2f6, baseHp: 130, baseDmg: 7 }
];
...
function triggerAttackOrInteract() {
    ...
    if (currentMap === 'VILLAGE') {
        ...
        npcs3D.forEach(npc => {
            if (Math.abs(player3D.x - npc.x) < 3.0) {
                openNpcDialogue(npc);
                interacted = true;
            }
        });
    ...
    if (!player3D.isAttacking) {
        ...
        // 1인칭 칼 베기 애니메이션
        fpSwordGroup.rotation.set(-Math.PI / 4, 0, -Math.PI / 3);

        if (currentMap === 'FOREST') {
            monsters3D.forEach(m => {
                if (m.alive && Math.abs(player3D.x - m.x) < 3.0) {
                    damageMonster(m);
                }
            });
        }
...
function startGame() {
    ...
    forestGroup.visible = false;
    player3D.x = -14;
    camera.position.set(player3D.x, 1.6, 0);
...
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // 3D 1인칭 몬스터 접촉 피해 & 리스폰
        if (currentMap === 'FOREST') {
            monsters3D.forEach(m => {
                if (m.alive) {
                    if (Math.abs(player3D.x - m.x) < 1.8) {
                        hp -= (m.touchDmg / 60);
                    }
                } else {
                    m.respawnTimer--;
