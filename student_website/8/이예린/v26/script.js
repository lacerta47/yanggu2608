// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v26 Engine
// (포동포동 아기 동물 & 발그레 볼터치 귀여운 몬스터 대모험)
// -------------------------------------------------------------
...
function updateMonsterHpSprite(m) {
    ...
    ctx.fillStyle = ratio > 0.4 ? '#2bcbba' : '#ff7675';
...
const bladeMat = new THREE.MeshStandardMaterial({ color: 0x2bcbba, metalness: 0.8, roughness: 0.2, emissive: 0x2bcbba, emissiveIntensity: 0.4 });
...
function updateSwordVisuals() {
    if (swordDamage >= 30) {
        ...
        bladeMat.color.setHex(0x0fbcf9);
        bladeMat.emissive.setHex(0x0fbcf9);
...
const gatePillar1 = new THREE.Mesh(new THREE.BoxGeometry(0.8, 5, 0.8), new THREE.MeshStandardMaterial({ color: 0x2bcbba }));
gatePillar1.position.set(-2.5, 2.5, 0);
forestGate.add(gatePillar1);

const gatePillar2 = new THREE.Mesh(new THREE.BoxGeometry(0.8, 5, 0.8), new THREE.MeshStandardMaterial({ color: 0x2bcbba }));
gatePillar2.position.set(2.5, 2.5, 0);
forestGate.add(gatePillar2);

const gateTop = new THREE.Mesh(new THREE.BoxGeometry(5.8, 0.8, 0.8), new THREE.MeshStandardMaterial({ color: 0x70a1ff }));
gateTop.position.set(0, 5.2, 0);
forestGate.add(gateTop);

const gateTag = createTagSprite('🌸 [Space] 아기 몬스터 숲 입장', '#ff7675');
...
const npcsVillage = [
    { 
        ...
        color: 0xf39c12, 
        hatColor: 0xd35400,
        story: "허허, 어서오게 젊은 용사여!\n숲속 몬스터들이 발그레 볼터치를 하고 아기 동물처럼 귀여워졌다네!\n300코인으로 칼을 단련하고 귀여운 몬스터들과 신나게 놀아보게나!",
        chatLines: ["허허, 날씨가 참 화창하구려~", "주희 양, 아기 슬라임이 참 귀엽지?", "민우야, 조심해서 놀아라!", "철수 자네 망치 소리가 경쾌하군!"],
    ...
    { 
        ...
        color: 0x2ecc71, 
        hatColor: 0x27ae60,
        story: "안녕하세요 용사님! 숲속 아기 슬라임들이 핑크 볼터치를 하고 통통 뛰는 게 너무 사랑스러워요!\n용사님께서 300코인으로 칼을 단련해서 슬라임들과 놀아주세요!",
        chatLines: ["오늘 캔 약초 향이 달콤해요!", "이장님, 건강 챙기세요~", "영희 씨, 장미꽃이 참 곱네요!"],
    ...
    { 
        ...
        color: 0xff7675, 
        hatColor: 0xd63031,
        story: "와! 아기 여우 늑대랑 꼬마 유령이 너무 귀여워요!\n형, 얼른 칼을 멋지게 강화해서 아기 몬스터들과 숨바꼭질해요!",
        chatLines: ["나도 귀여운 아기 펫을 키울 테야!", "한스 아저씨, 숲속 이야기 또 들려주세요!", "와아~ 숲속에 가보고 싶다!"],
        quest: { title: "숲속 탐험", desc: "귀여운 아기 몬스터 숲으로 건너가 탐험하기", targetType: "explore", rewardExp: 30, active: false, completed: false } 
    },
    { 
        id: 'villager4', 
        name: '정원사 영희', 
        ...
        color: 0x9b59b6, 
        hatColor: 0x8e44ad,
        story: "안녕하세요! 빨간 땡땡이 아기 무당벌레 거미가 꽃밭에 놀러 왔어요.\n강화된 반짝이는 칼로 함께 놀아주시면 예쁜 꽃들이 다시 활짝 필 거예요!",
    ...
    { 
        ...
        color: 0x3498db, 
        hatColor: 0x2980b9,
        story: "반갑네 젊은 용사여! 아기 몬스터들도 레벨업하면 앙증맞은 +10 펀치를 날린다네!\n대장장이 철수에게서 칼을 강력하게 벼려보게나!",
        chatLines: ["봄바람이 참 포근하군...", "민우야, 낭떠러지는 항상 조심하렴!", "대장장이 철수 칼솜씨는 여전히 최고야."],
...
const forgeCore = new THREE.Mesh(
    ...
    new THREE.MeshStandardMaterial({ color: 0xff7675, emissive: 0xff4757, emissiveIntensity: 0.9 })
);
forgeCore.position.set(0, 1.3, 0.7);
forgeStoneGroup.add(forgeCore);

const forgeLight = new THREE.PointLight(0xff7675, 2.5, 10);
...
// -------------------------------------------------------------
// 🌲 3D 화사한 아기 몬스터 숲 맵 (Map 3)
// -------------------------------------------------------------
const forestFloor = new THREE.Mesh(
    new THREE.PlaneGeometry(140, 80),
    new THREE.MeshStandardMaterial({ color: 0x1dd1a1, roughness: 0.85 })
);
...
function createDetailedForestTree(x, z, typeIndex, scaleRatio = 1.0) {
    ...
    const trunk = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: 0x574b90, roughness: 0.9 })
    );
    trunk.position.y = (1.9 * scaleRatio);
    treeGroup.add(trunk);

    const leafColors = [0x10ac84, 0x1dd1a1, 0x55efc4, 0x00cec9, 0x81ecec];
...
const shroomCoords = [
    ...
    { x: -2,  z: -8, c: 0xa29bfe }, { x: 12,  z: 6, c: 0xffd32a },
...
const vGateLeft = new THREE.Mesh(new THREE.BoxGeometry(0.8, 5.0, 0.8), new THREE.MeshStandardMaterial({ color: 0xff7675 }));
vGateLeft.position.set(-2.5, 2.5, 0);
villageGateInForest.add(vGateLeft);

const vGateRight = new THREE.Mesh(new THREE.BoxGeometry(0.8, 5.0, 0.8), new THREE.MeshStandardMaterial({ color: 0xff7675 }));
...
// -------------------------------------------------------------
// 🐣 [옵션 2] 포동포동 아기 동물 & 발그레 몬스터 3D 모델링 빌더
// -------------------------------------------------------------
function buildCuteBabyMonsterMesh(m) {
    const group = new THREE.Group();

    // 1. 🍮 발그레 젤리 슬라임 (Pudding Slime)
    if (m.type === 'slime') {
        ...
        // 머리 위의 앙증맞은 연두색 새싹 핀 (Sprout Pin)
    ...
    // 2. 🦖 아기 공룡 고블린 / 👻 꼬마 유령 / 🧸 테디 골렘
        ...
        if (isSkeleton) {
            ...
            // 🦖 아기 공룡 고블린 (Baby Dino Goblin - 민트색 몸체 + 아기 공룡 꼬리 + 별 요술봉)
            const body = new THREE.Mesh(
    ...
    // 3. 🦊 포동포동 아기 여우 늑대 (Chubby Baby Fox Wolf)
    else if (m.type === 'quad') {
    ...
    // 4. 🐞 아기 무당벌레 스파이더 (Cute Ladybug Spider)
...
const monsters3D = [
    ...
    { id: 'goblin1', baseName: '🦖 아기 공룡 고블린', originX: -35, originZ: 6, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0x55efc4, baseHp: 60, baseDmg: 6, type: 'biped' },
    { id: 'slime2', baseName: '🍮 민트 젤리 슬라임', originX: -25, originZ: -6, patrolRadius: 8, chaseSpeed: 0.06, patrolSpeed: 0.03, color: 0x81ecec, baseHp: 50, baseDmg: 4, type: 'slime', scale: 0.65 },

    { id: 'wolf1', baseName: '🦊 아기 여우 늑대', originX: -12, originZ: 4, patrolRadius: 14, chaseSpeed: 0.095, patrolSpeed: 0.045, color: 0xffa502, baseHp: 70, baseDmg: 7, type: 'quad' },
    { id: 'golem', baseName: '🧸 테디 베어 골렘', originX: 0, originZ: 0, patrolRadius: 10, chaseSpeed: 0.048, patrolSpeed: 0.02, color: 0xcd84f1, baseHp: 100, baseDmg: 9, type: 'biped' },
    { id: 'spider1', baseName: '🐞 아기 무당벌레', originX: 12, originZ: -5, patrolRadius: 12, chaseSpeed: 0.085, patrolSpeed: 0.038, color: 0xff4757, baseHp: 80, baseDmg: 6, type: 'spider' },

    { id: 'skeleton1', baseName: '👻 꼬마 별요정 유령', originX: 25, originZ: 5, patrolRadius: 12, chaseSpeed: 0.075, patrolSpeed: 0.032, color: 0xffffff, baseHp: 90, baseDmg: 7, type: 'biped' },
    { id: 'wolf2', baseName: '🦊 솜사탕 아기 여우', originX: 38, originZ: -6, patrolRadius: 14, chaseSpeed: 0.10, patrolSpeed: 0.05, color: 0xffbe76, baseHp: 85, baseDmg: 8, type: 'quad' },
    { id: 'spider2', baseName: '🐞 핑크 무당벌레', originX: 45, originZ: 4, patrolRadius: 10, chaseSpeed: 0.09, patrolSpeed: 0.04, color: 0xfd79a8, baseHp: 85, baseDmg: 7, type: 'spider' },
    { id: 'skeleton2', baseName: '👻 꼬마 천사 유령', originX: 52, originZ: -3, patrolRadius: 12, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xfff200, baseHp: 95, baseDmg: 8, type: 'biped' }
];
...
function updateUI() {
    ...
    if (currentMap === 'VILLAGE') {
        ...
        locationNameSpan.textContent = '🌸 3D 귀여운 아기 몬스터 숲';
    }
}
