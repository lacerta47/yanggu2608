// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v14 Engine (NPC 대화 100% 보증 & 전용 대화 버튼)
// -------------------------------------------------------------
...
const monsters3D = [
    { id: 'slime', baseName: '🟢 숲 슬라임', originX: -14, originZ: 0, chaseSpeed: 0.05, geo: new THREE.DodecahedronGeometry(0.8), color: 0x2ecc71, baseHp: 100, baseDmg: 5 },
    { id: 'goblin', baseName: '🔴 분노한 고블린', originX: -6, originZ: -2, chaseSpeed: 0.08, geo: new THREE.BoxGeometry(1.2, 1.4, 1.2), color: 0xe74c3c, baseHp: 110, baseDmg: 6 },
    { id: 'wolf', baseName: '🐺 숲 늑대', originX: 2, originZ: 2, chaseSpeed: 0.09, geo: new THREE.CylinderGeometry(0.6, 0.8, 1.5, 8), color: 0x7f8c8d, baseHp: 120, baseDmg: 7 },
    { id: 'golem', baseName: '🗿 암석 골렘', originX: 8, originZ: -1, chaseSpeed: 0.045, geo: new THREE.BoxGeometry(1.6, 2.0, 1.6), color: 0x95a5a6, baseHp: 150, baseDmg: 8 },
    { id: 'spider', baseName: '🕷️ 독 거미', originX: 14, originZ: 1, chaseSpeed: 0.085, geo: new THREE.SphereGeometry(0.9, 12, 12), color: 0x9b59b6, baseHp: 105, baseDmg: 6 },
    { id: 'skeleton', baseName: '👻 해골 전사', originX: 20, originZ: -2, chaseSpeed: 0.065, geo: new THREE.CylinderGeometry(0.5, 0.5, 2.0, 8), color: 0xf1f2f6, baseHp: 130, baseDmg: 7 }
];
...
['click', 'pointerdown', 'touchstart'].forEach(evt => {
    btnTalk.addEventListener(evt, (e) => {
        e.stopPropagation();
        e.preventDefault();
        initAudio();
...
btnAttack.addEventListener('click', () => { initAudio(); triggerDialogueOrAttack(false); });
...
function triggerDialogueOrAttack(forceDialogueOnly = false) {
    ...
    if (forceDialogueOnly) {
        showLevelUpToast('💡 가까이 주민이 없습니다! 주민 근처로 걸어가서 대화해보세요!');
        return;
    }
...
function gainExp(amount) {
    ...
    while (exp >= targetExp) {
        ...
        swordDamage += 1;
        playCustomSound('yap');
        showLevelUpToast(`🎉 LEVEL UP! Lv.${level} 달성! (최대 체력 +50 상승!)`);
    }
...
['click', 'pointerdown', 'touchstart'].forEach(evtType => {
    btnStartGame.addEventListener(evtType, (e) => {
        e.stopPropagation();
        e.preventDefault();
        startGame();
        try {
...
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // 🚀 실시간 NPC 근접 감지 UI 배너 및 대화 버튼 강조 갱신
        let activeNpcs = [];
        if (currentMap === 'VILLAGE') activeNpcs = npcsVillage;
        else if (currentMap === 'BLACKSMITH_INTERIOR') activeNpcs = [blacksmithNpc];
