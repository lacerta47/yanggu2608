// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v20 Engine
// (레벨업 시 몬스터 공격력 +10씩 강력 상승 시스템)
// -------------------------------------------------------------
...
const npcsVillage = [
    ...
    { 
        ...
        z: 0, 
        color: 0xe74c3c, 
        story: "와! 멋진 용사 형이다!\n몬스터들이 숲속을 살아서 뛰어다니고 레벨업하면 더 세진대요!\n조심해서 다녀오세요!",
...
function getMonsterStats(baseHp, baseDmg, pLevel) {
    ...
    // ⚔️ [핵심 요청 구현] 캐릭터 레벨 상승 시마다 몬스터 공격력 +10 상승!
...
function gainExp(amount) {
    ...
    while (exp >= targetExp) {
        ...
        // ⚔️ [핵심 요청 구현] 레벨업 시 몬스터의 피 +30 & 공격력(touchDmg) +10 상승!
        monsters3D.forEach(m => {
            const stats = getMonsterStats(m.baseHp, m.baseDmg, level);
            m.maxHp = stats.maxHp;
            m.touchDmg = stats.touchDmg; // 공격력 +10씩 상승!
            if (m.alive) {
            ...
            monsters3D.forEach(m => {
                if (m.alive) {
                    ...
                    if (distToPlayer < AGGRO_RANGE) {
                        ...
                        // 용사와 부딪혔을 때 강력해진 touchDmg 지속 적용
                        if (distToPlayer < 1.8) {
                            hp -= (m.touchDmg / 60);
                            hpChanged = true;
                        }
