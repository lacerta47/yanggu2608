// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v9 Engine (방향키 이동 방향 정방향 직관 보정)
// -------------------------------------------------------------
...
document.addEventListener('pointerlockchange', () => {
    if (document.pointerLockElement === container) {
        ...
        isShiftLocked = false;
        shiftLockStatusSpan.textContent = '🔓 OFF (Shift키로 토글)';
    }
});

document.addEventListener('mousemove', (e) => {
    if (isShiftLocked && gameState === 'PLAYING') {
        const sens = 0.003;
...
container.addEventListener('click', (e) => {
    initAudio();
    if (gameState === 'PLAYING' && !isShiftLocked) {
        toggleShiftLock();
    }
});
...
// -------------------------------------------------------------
// 3D 1인칭 메인 애니메이션 루프 (직관적 정방향 이동 조작)
// -------------------------------------------------------------
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // 🧭 내가 바라보는 방향(yaw) 기준 100% 정방향 이동 벡터
        if (keys.up) { // ▲ / W : 전진 (바라보는 방향으로 똑바로!)
        ...
        // 🌲 마을 동쪽 끝 통과 시 숲속 맵 자동 이동
        if (currentMap === 'VILLAGE' && player3D.x > 21) {
            goToForestMap();
        } 
        // 🏡 숲속 서쪽 끝 통과 시 마을 맵 자동 이동
