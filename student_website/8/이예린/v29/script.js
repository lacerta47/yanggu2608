// -------------------------------------------------------------
// 3D 1인칭 용사의 모험 RPG v29 Engine
// (거대한 대산맥 등산 & 구름 위 파노라마 전망대 시스템)
// -------------------------------------------------------------
...
const camera = new THREE.PerspectiveCamera(65, 900 / 500, 0.1, 1200);
...
const gateTag = createTagSprite('🌲 [Space] 대자연 숲 & 산맥 등산', '#2ed573');
...
const npcsVillage = [
    { 
        ...
        color: 0xf39c12, 
        hatColor: 0xd35400,
        story: "허허, 어서오게 젊은 용사여!\n이제 거대한 2개의 산꼭대기 정상까지 직접 등산할 수 있게 되었네!\n구름 위 정상에 올라 숲 전체를 내려다보는 멋진 경치를 즐겨보게나!",
        chatLines: ["허허, 산꼭대기 전망이 끝내주는구려~", "주희 양, 산 위에 구름이 닿아 있네!", "민우야, 산에 오를 때 발밑 조심하렴!", "철수 자네 망치 소리가 산봉우리까지 울리는군!"],
    ...
    { 
        ...
        color: 0x2ecc71, 
        hatColor: 0x27ae60,
        story: "안녕하세요 용사님! 높은 산꼭대기에는 시원한 바람과 함께 신비로운 산 약초가 자라요!\n용사님께서 300코인으로 칼을 단련해서 산 정상까지 정복해 보세요!",
        chatLines: ["산꼭대기 공기가 정말 상쾌해요!", "이장님, 산꼭대기 다녀오셨어요?", "영희 씨, 산 위에서 마을이 다 보여요!"],
    ...
    { 
        ...
        color: 0xff7675, 
        hatColor: 0xd63031,
        story: "와! 저기 거대한 산꼭대기 하얀 눈 위에 올라갈 수 있대요!\n형, 얼른 산꼭대기까지 등산해서 30마리 아기 몬스터들이랑 인사해요!",
        chatLines: ["산꼭대기에서 야호~ 외쳐봤어요!", "한스 아저씨, 저도 산꼭대기 데려가 줘요!", "와아~ 구름이 손에 닿을 것 같아요!"],
        quest: { title: "숲속 탐험", desc: "거대한 산꼭대기 정상과 숲을 탐험하기", targetType: "explore", rewardExp: 30, active: false, completed: false } 
    },
    { 
        id: 'villager4', 
        name: '정원사 영희', 
        ...
        color: 0x9b59b6, 
        hatColor: 0x8e44ad,
        story: "안녕하세요! 산기슭을 따라 올라가면 숲과 파란 연못이 동화처럼 펼쳐져요.\n강화된 반짝이는 칼로 산맥의 아기 몬스터들과 신나게 모험해 보세요!",
        chatLines: ["산꼭대기 꽃들이 구름 속에서 피어나네요~", "주희 씨, 산 위에서 부는 바람이 싱그러워요!", "마을과 산맥이 아름다운 초록빛이에요~"],
    ...
    { 
        ...
        color: 0x3498db, 
        hatColor: 0x2980b9,
        story: "반갑네 젊은 용사여! 대산맥의 정상은 진정한 용사만이 밟을 수 있는 곳이지!\n대장장이 철수에게서 칼을 강력하게 벼리고 산꼭대기를 정복해보게나!",
        chatLines: ["대산맥 꼭대기에서 숲을 굽어보는 기분은 최고지...", "민우야, 가파른 절벽 쪽은 피하렴!", "대장장이 철수 칼솜씨는 여전히 최고야."],
...
const monsters3D = [
    ...
    // 19~24: 🏔️ 북동쪽 거대한 대산맥 정상 & 산기슭
    { id: 'mountain_wolf1', baseName: '🦊 대산맥 아기여우', originX: 85, originZ: -58, patrolRadius: 12, chaseSpeed: 0.10, patrolSpeed: 0.048, color: 0xff9f43, baseHp: 90, baseDmg: 8, type: 'quad' },
    ...
    { id: 'mountain_slime1', baseName: '🍮 산바람 블루 슬라임', originX: 90, originZ: -60, patrolRadius: 9, chaseSpeed: 0.065, patrolSpeed: 0.03, color: 0x0984e3, baseHp: 75, baseDmg: 6, type: 'slime' },
    { id: 'mountain_ghost1', baseName: '👻 산봉우리 별요정', originX: 98, originZ: -68, patrolRadius: 8, chaseSpeed: 0.08, patrolSpeed: 0.035, color: 0xffffff, baseHp: 90, baseDmg: 8, type: 'biped' },
    ...
    { id: 'mountain_slime2', baseName: '🍮 설산 딸기 슬라임', originX: 92, originZ: -72, patrolRadius: 8, chaseSpeed: 0.06, patrolSpeed: 0.028, color: 0xff7675, baseHp: 70, baseDmg: 6, type: 'slime' },
...
function updateUI() {
    ...
    if (currentMap === 'VILLAGE') {
        ...
        locationNameSpan.textContent = '🏔️ 3D 대산맥 등산로 & 숲';
    }
}
...
// -------------------------------------------------------------
// 3D 1인칭 메인 애니메이션 & 거대한 산맥 등산 고도 루프
// -------------------------------------------------------------
function animate() {
    ...
    if (gameState === 'PLAYING') {
        ...
        // 🏔️ [핵심] 언덕 3개 & 거대한 산 2개 등산 높이(Elevation) 실시간 고도 반영!
        ...
        if (currentMap === 'VILLAGE') {
            ...
            if (onMountainName) {
                proximityNpcText.textContent = `🏔️ [${onMountainName} 등반 중!] 구름 위 정상에서 숲 전체가 훤히 보여요! (고도: +${Math.round(currentElevation)}m)`;
                npcProximityPrompt.classList.remove('hidden');

                if (currentElevation > 14 && !hasCelebratedSummit) {
                    hasCelebratedSummit = true;
                    playCustomSound('powerup');
                    showLevelUpToast(`🏔️ [산꼭대기 정복!] 구름 위 파노라마 전망대에 도착했습니다!`);
                }
            } else if (onHillName) {
                proximityNpcText.textContent = `⛰️ [${onHillName} 등반 중!] 높은 곳에서 숲을 조망하고 있어요! (고도: +${Math.round(currentElevation)}m)`;
                npcProximityPrompt.classList.remove('hidden');
        ...
        // 🌲 30마리 숲속 아기 몬스터 실시간 산과 언덕 오르내리기 & 추격 AI
        if (currentMap === 'FOREST') {
            let hpChanged = false;
