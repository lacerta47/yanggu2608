// ==========================================
// 밝은 바다 오드아이 고양이 달리기 로직 (v8)
// - 밝은 수중 파스텔 그라데이션 바다 배경
// - 빠르게 증가하는 속도 엔진 (스릴 넘치는 빠른 반응속도)
// - 돌고래 충돌 판정 (부딪힐 시 즉시 게임 오버)
// ==========================================
...
// ⭐ 빠른 속도 증가 엔진 (INITIAL_BASE_SPEED = 5.0, ACCEL = 0.0028)
const INITIAL_BASE_SPEED = 5.0;
const SPEED_ACCEL = 0.0028;
...
function spawnObstacle() {
  ...
  if (type === 'shell') {
    ...
    // 🐬 돌고래: 위로 떠다님 (y = GROUND_Y - 82px, height = 36px)
    // 서서 달려오면 머리에 부딪혀 죽음! 아래 방향키 슬라이딩(height = 24px)으로 통과!
    width = 65;
    height = 36;
    spawnY = GROUND_Y - 82;
  }
...
function drawBackground() {
  // 상단 2/3: 화사한 밝은 파스텔 바다 그라데이션!
  const seaGradient = ctx.createLinearGradient(0, 0, 0, GROUND_Y);
  seaGradient.addColorStop(0, '#e0f7fa');  // 맨 위: 에메랄드빛 하늘색
  seaGradient.addColorStop(0.35, '#bde0fe'); // 중간: 연한 부드러운 파스텔 하늘
  seaGradient.addColorStop(0.7, '#80deea');  // 아래쪽: 맑은 아쿠아 블루
  seaGradient.addColorStop(1, '#00b4d8');   // 바닥 인근: 비브런트 청록
  ctx.fillStyle = seaGradient;
  ctx.fillRect(0, 0, CANVAS_WIDTH, GROUND_Y);

  // 바닷속 일렁이는 픽셀 햇살 조명선 (햇살 빔)
...
function drawObstacles() {
  obstacles.forEach(obs => {
    ...
    if (obs.type === 'dolphin') {
      ...
      drawPixelRect(x + 16, y + h - 6, w - 32, 6, '#caf0f8');
      ...
      drawPixelRect(x + w - 21, y + 9, 2, 2, '#ffffff');
      drawPixelRect(x + w - 26, y + 14, 4, 3, '#ffb5a7');
...
// ------------------------------------------
// 메인 게임 업데이트 (빠른 속도 증가 & 정밀한 충돌 판정)
// ------------------------------------------
function updateGame() {
  frameCount++;

  // ⭐ 빠르게 속도가 올라가는 속도 엔진 (SPEED_ACCEL = 0.0028)
  ...
  if (nextObstacleTimer <= 0) {
    ...
    if (gameSpeed >= 7.5) {
      const speedFactor = (gameSpeed - 7.5) * 5;
      minGap = 55 + speedFactor;
      maxGap = 95 + speedFactor;
    }
  ...
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    // AABB 충돌 검사 (돌고래 포함 모든 장애물 충돌 시 죽음)
    const padding = 5;
    if (
