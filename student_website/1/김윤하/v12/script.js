// ==========================================
// 오드아이 고양이 달리기 로직 (v12)
// - 슬라이딩 전용 새로운 장애물 '바다 그물' 추가! 🕸️
// - 돌고래(🐬)와 바다 그물(🕸️)은 오직 아래 방향키(▼) 슬라이딩으로만 통과!
// - 조개(🐚), 미역(🌿), 산호초(🪸)는 스페이스바(Space) 점프로 통과!
// ==========================================
...
// ------------------------------------------
// 🐬 장애물 스폰 (점프 전용 3종 + ⭐ 슬라이딩 전용 2종: 돌고래, 그물!)
// ------------------------------------------
function spawnObstacle() {
  ...
  if (rand < 0.28) {
    ...
    type = 'dolphin'; // 🐬 돌고래 (슬라이딩 전용!)
  } else {
    type = 'net';     // 🕸️ 바다 그물 (슬라이딩 전용!)
  }
  ...
  if (type === 'shell') {
    ...
    // 🕸️ ⭐ 슬라이딩 전용 새로운 '바다 그물' 장애물!
    // width = 54px, height = 42px, spawnY = GROUND_Y - 68px
    // 바닥 여유 공간: Exactly 26px -> 서서 가거나 점프하면 그물망에 100% 사망! 오직 슬라이딩(24px)으로만 통과!
    width = 54;
    height = 42;
    spawnY = GROUND_Y - 68;
  }
...
function drawBackground() {
  ...
  sandDecorations.forEach(dec => {
    ...
    if (dec.type === 'crab') {
      ...
      drawPixelRect(dx, dy, 12, 8, '#ff477e');
      drawPixelRect(dx + 2, dy + 2, 2, 2, '#000000');
      drawPixelRect(dx + 8, dy + 2, 2, 2, '#000000');
      ...
      drawPixelRect(dx + 2, dy + 2, 6, 2, '#ffffff');
...
function drawObstacles() {
  obstacles.forEach(obs => {
    ...
    if (obs.type === 'net') {
      // 🕸️ 슬라이딩 전용 새로운 '바다 그물' 장애물! (w: 54px, h: 42px)
      // 상단 그물 부표(주황 코르크 찌 3개)
      drawPixelRect(x + 4, y - 6, 12, 6, '#ff6b35');
      drawPixelRect(x + 21, y - 6, 12, 6, '#ff6b35');
      drawPixelRect(x + 38, y - 6, 12, 6, '#ff6b35');

      // 그물 상단 지지 로프
      drawPixelRect(x, y, w, 4, '#e2e8f0');
      ...
      // 그물망에 걸려있는 픽셀 조약돌/부유물 포인트
      drawPixelRect(x + 12, y + 16, 6, 6, '#64748b');
      drawPixelRect(x + 32, y + 24, 6, 6, '#64748b');

      // 뚝뚝 떨어지는 수중 픽셀 방울
      const drip = Math.sin(frameCount * 0.3 + x) * 3;
      drawPixelRect(x + 10, y + h + 2 + drip, 3, 4, '#90e0ef');
      drawPixelRect(x + 38, y + h + 4 + drip, 3, 4, '#90e0ef');

    } else if (obs.type === 'dolphin') {
      // 🐬 슬라이딩 전용 돌고래
      drawPixelRect(x + 10, y + 4, w - 20, h - 8, '#48cae4');
      drawPixelRect(x + 18, y, w - 32, 4, '#48cae4');
      drawPixelRect(x + 14, y + h - 6, w - 28, 6, '#caf0f8');
