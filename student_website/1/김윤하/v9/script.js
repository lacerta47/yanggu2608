// ==========================================
// 오드아이 고양이 달리기 로직 (v9)
// - 서서 그냥 걷기만 해도 돌고래에 100% 충돌 (슬라이딩 필수!)
// - 쾌적하게 확 줄어든 장애물 등장 빈도
// - 초극세사 디테일의 아기자기하고 귀여운 픽셀 장애물들
// ==========================================
...
function initGame() {
  ...
  frameCount = 0;
  obstacles = [];
  nextObstacleTimer = 85; // 여유로운 시작 타이머
...
// ------------------------------------------
// 🐬 장애물 스폰
// ⭐ 핵심: 돌고래 높이를 낮추어 그냥 걷다간 100% 부딪히게 세팅! (spawnY = GROUND_Y - 58)
// ------------------------------------------
function spawnObstacle() {
  ...
  if (type === 'shell') {
    ...
    // ⭐ 돌고래 높이 조정: spawnY = GROUND_Y - 58px, height = 36px (y: 242 ~ 278)
    // 서서 걷는 고양이(높이 46px, y: 254 ~ 300)는 24px만큼 충돌하여 100% 죽음!
    // 슬라이딩 고양이(높이 24px, y: 276 ~ 300)만 아래로 쏙 무사 통과!
    width = 66;
    height = 36;
    spawnY = GROUND_Y - 58;
  }
...
// 1. 화사하고 밝은 색깔의 수중 그라데이션 바다 배경
function drawBackground() {
  const seaGradient = ctx.createLinearGradient(0, 0, 0, GROUND_Y);
  seaGradient.addColorStop(0, '#e0f7fa');  // 에메랄드 하늘
  seaGradient.addColorStop(0.35, '#bde0fe'); // 연한 부드러운 파스텔
  seaGradient.addColorStop(0.7, '#80deea');  // 맑은 아쿠아
  seaGradient.addColorStop(1, '#00b4d8');   // 청록
...
// 3. ⭐ 초극세사 디테일의 아기자기하고 귀여운 장애물 4종 그리기
function drawObstacles() {
  obstacles.forEach(obs => {
    ...
    if (obs.type === 'dolphin') {
      // 🐬 디테일하고 귀여운 청록/분홍 픽셀 돌고래
      drawPixelRect(x + 12, y + 4, w - 24, h - 8, '#48cae4');
      drawPixelRect(x + 20, y, w - 36, 4, '#48cae4');
      drawPixelRect(x + 16, y + h - 6, w - 32, 6, '#caf0f8'); // 배 (하얀 톤)
      ...
      drawPixelRect(x + w - 21, y + 9, 2, 2, '#ffffff'); // 눈 반사광
      drawPixelRect(x + w - 26, y + 14, 4, 3, '#ffadad'); // 분홍 볼터치
      ...
      drawPixelRect(x + 16, y + 10, 3, 2, '#ff758f'); // 볼터치
      // 진주 & 상단 분홍 리본
      ...
      drawPixelRect(x + 20, y + 24, 2, 2, '#000000'); // 불가사리 눈
    }
...
// ------------------------------------------
// 메인 게임 업데이트 (장애물 간격 확 넓힘 & 돌고래 100% 충돌 죽음 판정)
// ------------------------------------------
function updateGame() {
  frameCount++;
  ...
  // ⭐ 장애물 스폰 타이머: 사용자 요청 반영! 장애물 빈도를 쾌적하게 확 늘려줌 (minGap = 85, maxGap = 140)
  nextObstacleTimer -= 1;
  ...
  // ⭐ 충돌 검사: 그냥 걸어가도(서서 가면) 돌고래와 100% 부딪혀 사망!
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    // AABB Hitbox 충돌 검사 (패딩 4px로 정밀 검사)
    const padding = 4;
    if (
