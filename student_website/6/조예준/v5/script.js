// 🌋 화산시대 공룡 달리기 게임 (v5 - 하늘에서 갑자기 떨어지는 화산 유성!)
...
// ☄️ 하늘에서 갑자기 떨어지는 화산 유성 (Falling Meteor) 생성 함수
function spawnFallingMeteor() {
  // 공룡의 진행 방향 앞쪽(250~650 위치) 중 하나에 경고 표시 후 떨어짐
  const targetX = Math.random() * 400 + 250;

  fallingMeteors.push({
    targetX: targetX,
    currentX: targetX + 120, // 대각선 하늘에서 착하
    currentY: -60,
    targetY: groundY - 32,
    size: 28,
    timer: 0,
    warningDuration: 45, // 약 0.75초 경고
    isFalling: false,
    hasLanded: false
  });
...
function update() {
  ...
  // ☄️ 갑자기 낙하하는 화산 유성 생성 (150점 이후부터 가끔씩 하늘에서 떨어짐!)
  meteorSpawnTimer++;
  if (score > 60 && meteorSpawnTimer > 280) {
    if (Math.random() < 0.65) {
      spawnFallingMeteor();
    }
    meteorSpawnTimer = 0;
  ...
  for (let i = fallingMeteors.length - 1; i >= 0; i--) {
    ...
    if (!m.isFalling) {
      ...
      // 땅에 닿으면 착하 파티클 후 일반 굴러가는 바위로 전환 또는 소멸
      if (m.currentY >= m.targetY) {
        m.currentY = m.targetY;
        addDust(m.targetX, groundY);
        addDust(m.targetX + 10, groundY);

        // 낙하 후 바닥에서 바위 장애물로 전환
        obstacles.push({
          type: 'FIREBALL',
          x: m.targetX,
          y: groundY - 28,
...
function draw() {
  ...
  // 2. ☄️ 낙하 유성 경고 표시 (Warning Indicator)
  for (let m of fallingMeteors) {
    if (!m.isFalling) {
      // 깜빡이는 ⚠️ 경고 표시
      const blink = Math.floor(frameCount / 6) % 2 === 0;
      ...
      if (blink) {
        ...
        ctx.fillText('⚠️ 낙하 주의!', m.targetX - 45, 45);
