// 🌋 화산시대 공룡 달리기 게임 (v19 - 🎵 단계별 다른 멜로디 노래 & 초고속 가속 BGM!)
...
// 4. Level 10+ (무한 마스터): 전설의 초고속 신기록 테마!
...
// ⏩ 레벨이 높아질수록 노래 속도(Interval)가 점점 아주 빨라짐!
function startBgm() {
  stopBgm();
  bgmStep = 0;

  // 레벨이 상승할수록 연주 간격(ms)이 175ms -> 85ms로 점점 초고속 가속!
  const baseInterval = Math.max(85, 175 - (currentLevel * 9));
  const activeInterval = feverTimer > 0 ? Math.max(70, baseInterval - 30) : baseInterval;
...
function triggerFeverTime() {
  ...
  feverTimer = 480;
  playFeverStartSound();
  if (bgmEnabled) startBgm(); // 피버 타임 전용 템포 BGM 전환!
...
function startNewGame() {
  ...
  if (bgmEnabled) startBgm(); // 🎵 게임 시작 시 BGM 연주 시작!
}
...
function update() {
  ...
  // 🚩 10단계 무한 모드 (레벨 상승 시 BGM 멜로디 변경 & 노래 속도 가속!)
  ...
  if (targetLevel > currentLevel) {
    ...
    // ⏩ 레벨이 상승하면 새로운 레벨 음악과 가속 템포로 BGM 즉시 변경!
    if (bgmEnabled) {
      startBgm();
    }
  }
  ...
  // 🌟 희귀 파워 아이템 스폰
  powerItemSpawnTimer++;
  if (powerItemSpawnTimer > 2500) {
    if (powerItems.length < 1 && Math.random() < 0.15) {
      spawnPowerItem();
    }
    powerItemSpawnTimer = 0;
  }
