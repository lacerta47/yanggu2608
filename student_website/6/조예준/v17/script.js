// 🌋 화산시대 공룡 달리기 게임 (v17 - 🔥 피버타임 무적 거대 공룡 변신 모드!)
...
let feverGauge = 0; // 🔥 0 ~ 100 피버 게이지
let feverTimer = 0; // 🔥 피버 타임 지속 타이머 (프레임)
...
function addFeverGauge(amount) {
  if (feverTimer > 0) return; // 이미 피버타임 중에는 무시
...
function triggerFeverTime() {
  ...
  feverTimer = 480; // 8초간 무적 거대 공룡 피버 타임!
  playFeverStartSound();
    ...
    if (dino.hammerCharges > 0) {
      ...
      for (let j = obstacles.length - 1; j >= 0; j--) {
        ...
        if (obs.x >= dino.x - 20 && obs.x <= dino.x + 160) {
          ...
          score += 40;
          addFeverGauge(20); // 🔨 망치로 바위 깰 때마다 피버게이지 +20%!
...
// 🌟 파워 아이템 및 무지개 피버 별(STAR) 아이템 스폰
function spawnPowerItem() {
  const rand = Math.random();
  let type = 'WING';
  if (rand < 0.3) type = 'WING';       // 🪽 날개
  else if (rand < 0.55) type = 'HAMMER'; // 🔨 망치
  else if (rand < 0.8) type = 'SPRING';  // 🦘 스프링
  else type = 'STAR';                   // 🌟 무지개 피버 별 아이템!
...
function update() {
  ...
  score += (feverTimer > 0 ? 0.45 : 0.15); // 🔥 피버타임 시 점수 3배속 부스트!
  if (scoreText) scoreText.textContent = Math.floor(score);
  ...
  for (let i = powerItems.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      if (item.type === 'WING') {
        ...
        addFeverGauge(40); // 🌟 무지개 별 먹으면 피버 게이지 +40% 대폭 상승!
      }
  ...
  // 장애물 이동 및 충돌 검사 (🔥 피버 타임 시 모든 장애물 무차별 콰광 파괴!)
  for (let i = obstacles.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      if (feverTimer > 0) {
        ...
        score += 50; // 🔥 피버 무적 통과 콤보 점수 50점 폭발!
        if (scoreText) scoreText.textContent = Math.floor(score);
...
function draw() {
  ...
  // 7. 공룡 그리기 (🔥 피버 타임 시 거대 화염 무적 공룡 변신!)
  const dx = dino.x;
  const dy = dino.y;
