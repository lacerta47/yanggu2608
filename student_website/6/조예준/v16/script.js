// 🌋 화산시대 공룡 달리기 게임 (v16 - 3대 슈퍼 파워: 🪽 마법날개, 🔨 거대한 망치, 🦘 대왕 스프링!)
...
const dino = {
  ...
  // 3대 슈퍼 능력
  flyTimer: 0,       // 🪽 마법 날개 유효 시간 (프레임 단위)
  hammerCharges: 0,  // 🔨 망치 남은 사용 횟수
  springCharges: 0,  // 🦘 대왕 스프링 남은 점프 횟수
  hammerSwingTimer: 0 // 망치 휘두르는 애니메이션 타이머
};

// 배열
let obstacles = [];
let fallingMeteors = [];
let powerItems = []; // 🪽, 🔨, 🦘 슈퍼 파워 아이템 목록
...
function usePowerAction() {
  ...
  if (gameState === 'PLAYING') {
    if (dino.hammerCharges > 0) {
      dino.hammerCharges--;
      dino.hammerSwingTimer = 15; // 망치 휘두르기 애니메이션
      playHammerSmashSound();

      // 공룡 바로 앞 근처의 장애물 분쇄 파괴!
...
// 🌟 3가지 슈퍼 파워 아이템 무작위 스폰 (🪽, 🔨, 🦘)
function spawnPowerItem() {
  const rand = Math.random();
  let type = 'WING';
  if (rand < 0.35) type = 'WING';      // 🪽 마법 날개
  else if (rand < 0.7) type = 'HAMMER'; // 🔨 거대 망치
  else type = 'SPRING';                 // 🦘 대왕 스프링
...
function update() {
  ...
  // 공룡 운동 (🪽 날개 활성화 시 중력 대폭 감소!)
  if (dino.flyTimer > 0) {
    dino.vy += 0.25; // 부드러운 하강 중력
    dino.y += dino.vy;

    if (dino.y < 50) dino.y = 50; // 천장 최고 높이 제한
  ...
  // 🌟 슈퍼 파워 아이템 스폰 (약 170프레임마다)
  powerItemSpawnTimer++;
  if (powerItemSpawnTimer > 170) {
    if (Math.random() < 0.75) {
      spawnPowerItem();
    }
    powerItemSpawnTimer = 0;
  ...
  for (let i = powerItems.length - 1; i >= 0; i--) {
    ...
    if (
      ...
      if (item.type === 'WING') {
        dino.flyTimer = 360; // 🪽 6초간 하늘 날기!
      } else if (item.type === 'HAMMER') {
        dino.hammerCharges += 5; // 🔨 망치 5회 분쇄 충전!
      } else if (item.type === 'SPRING') {
        dino.springCharges += 3; // 🦘 대왕 점프 3회 충전!
      }
