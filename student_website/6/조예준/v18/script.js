// 🌋 화산시대 공룡 달리기 게임 (v18 - 🎵 신나는 원시 탐험 BGM 배경음악!)
...
function playBgmStep() {
  ...
  try {
    ...
    // 1. 메인 멜로디 음
    const freq = bgmMelody[bgmStep % bgmMelody.length] * (feverTimer > 0 ? 1.25 : 1.0);
    ...
    // 2. 묵직한 탐험 베이스 음
    const bassFreq = bgmBass[bgmStep % bgmBass.length] * (feverTimer > 0 ? 1.25 : 1.0);
...
function startBgm() {
  ...
  bgmTimerId = setInterval(() => {
    playBgmStep();
  }, feverTimer > 0 ? 110 : 170); // 🔥 피버 타임 시 BGM 템포 대폭 빨라짐!
}
...
function update() {
  ...
  // 🌟 파워 아이템 스폰 (45초~50초마다 1개 나올까 말까 한 전설의 울트라 희귀 스폰!)
  powerItemSpawnTimer++;
