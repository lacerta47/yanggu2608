// 🌋 화산시대 공룡 달리기 게임 (v11 - 🚨 유성 낙하 슝~ 효과음 추가!)
...
function update() {
  ...
  // 🚨 유성 생성
  meteorSpawnTimer++;
  if (score > 35 && meteorSpawnTimer > 220) {
    if (Math.random() < 0.7) {
...
function showGameOverModal() {
  ...
  if (modalMessage) modalMessage.textContent = `화산 장애물이나 낙하한 유성에 부딪혔어요!\n\n⭐ 획득 점수: ${Math.floor(score)}점\n🏆 최고 점수: ${Math.floor(highScore)}점`;
