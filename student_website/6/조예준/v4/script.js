// 🌋 화산시대 공룡 달리기 게임 (v4 - 획기적으로 다양해진 장애물과 익룡!)
...
function spawnObstacle() {
  ...
  if (isGround) {
    // 1. 지상 장애물종류 3가지 (싱글 돌, 연속 돌, 굴러오는 운석 마그마 구체)
    ...
    // 2. 공중 익룡종류 3가지 (붉은 화염 익룡, 노란 익룡, 푸른 번개 익룡)
    const birdTypeRand = Math.random();

    if (birdTypeRand < 0.45) {
      // 🚨 붉은 화염 익룡 (낮게 비행 -> 엎드려 숙이기 필수!)
      obstacles.push({
        type: 'BIRD_RED',
        x: canvas.width + 20,
        y: groundY - 42,
      ...
      // ⚡ 푸른 번개 익룡 (높은 공중에서 빠른 속도로 민첩하게 날아감!)
      obstacles.push({
        type: 'BIRD_BLUE',
        x: canvas.width + 20,
        y: groundY - 110,
        width: 42,
        height: 26,
        speedBonus: 2.5, // 일반 속도보다 빠르게 날아옴
        wingState: 0
      });
    }
  }
