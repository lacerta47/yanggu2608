// 🌋 화산시대 공룡 달리기 게임 (v8 - 🍖고기, 🥚황금알, 💎다이아방패, 🍬별사탕 아이템 총집합!)

// 안전한 캔버스 라운드 사각형 호환 함수 (모든 브라우저 100% 작동)
...
// 🎁 다채로워진 4가지 파워 아이템 (🍖 고기, 🥚 황금 알, 💎 다이아 방패, 🍬 별사탕)
function spawnItem() {
  const rand = Math.random();
  let type = 'MEAT';
  if (rand < 0.3) type = 'MEAT';       // 🍖 뼈다귀 고기 (+50점)
  else if (rand < 0.55) type = 'EGG';  // 🥚 황금 공룡 알 (+100점 대박 점수)
  else if (rand < 0.8) type = 'DIAMOND'; // 💎 다이아 방패 (보호막)
  else type = 'CANDY';                 // 🍬 별사탕 (보호막 + 점수)
...
function draw() {
  ...
  for (let item of items) {
    ...
    if (item.type === 'MEAT') {
      ...
      ctx.fillRect(item.x + 2, itemY + 14, 6, 4);
      ctx.fillRect(item.x + 24, itemY + 14, 6, 4);
      ...
      ctx.fillStyle = '#ffdd59';
      ctx.beginPath();
      ctx.ellipse(item.x + 16, itemY + 16, 10, 14, 0, 0, Math.PI * 2);
      ctx.fill();

      // 알에 무늬 반점
      ctx.fillStyle = '#ffa502';
      ctx.beginPath();
      ctx.arc(item.x + 13, itemY + 12, 3, 0, Math.PI * 2);
      ctx.arc(item.x + 20, itemY + 20, 2.5, 0, Math.PI * 2);
      ctx.fill();

      ctx.shadowBlur = 0;

    } else if (item.type === 'DIAMOND') {
      // 💎 빛나는 다이아몬드 방패 아이템 (무적 보호막!)
      ctx.shadowBlur = 16;
      ...
      ctx.beginPath();
      ctx.moveTo(item.x + 16, itemY + 2);
      ctx.lineTo(item.x + 30, itemY + 16);
      ctx.lineTo(item.x + 16, itemY + 30);
      ctx.lineTo(item.x + 2, itemY + 16);
      ctx.closePath();
      ctx.fill();
      ...
      ctx.fillStyle = '#ffffff';
      ctx.beginPath();
      ctx.arc(item.x + 16, itemY + 16, 5, 0, Math.PI * 2);
      ctx.fill();
