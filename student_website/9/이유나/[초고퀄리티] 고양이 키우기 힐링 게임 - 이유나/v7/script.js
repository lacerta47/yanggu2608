/**
 * [초고퀄리티] 고양이 키우기 힐링 게임 (v7)
 * All Class/Function/Variable names in English.
 * All UI labels, dialogue, thoughts, and notifications in Korean.
 * - Down Arrow 3x Combo: 3-Second Spiral Whirlpool Eyes!
 * - Kitten Full Body Rendering & 1-Year Max Growth Cap!
 * - Mother Cat Outfits Perfectly Fitted for Both Standing & Lying Down Poses!
 */
...
class CatHealingGame {
  constructor() {
    ...
    // Down Arrow Key 3-Tap Combo Tracker (눈이 3초간 회오리 모양으로 빙글빙글)
    ...
    // Feature unlocks (0s, 30s, 60s, 90s, 120s)
    this.unlockedFeatures = [false, false, false, false, false];
    this.unlockTimes = [0, 30, 60, 90, 120];
  ...
  initEvents() {
    ...
    // Keyboard Control with 3x Down Arrow Combo Detection
    window.addEventListener('keydown', (e) => {
      ...
      if (e.code === 'ArrowDown' || e.code === 'KeyS') {
        if (!this.keys.down) {
          ...
          // Check if pressed 3 times consecutively!
  ...
  // Down 3x Combo: 3-Second Spiral Whirlpool Eyes
  triggerSpiralEyes() {
    this.isSpiralEyes = true;
    this.spiralEyesTimer = 180; // 3 seconds (60fps * 3)
    this.soundManager.playMeowSound();
  ...
  updateTimeAndDecay() {
    ...
    // Mother cat age increases up to 5 years old
    const newAge = Math.min(5, Math.floor(this.gameTimeSeconds / 24));
    if (newAge !== this.age) {
      this.age = newAge;
      this.updateAgeDisplay();
    }

    // 2 Minutes (120s) -> 5-year-old Mother Cat gives birth to kittens!
    if (this.gameTimeSeconds >= 120 && !this.hasBred) {
    ...
    // Decay rate
    const decayRate = 0.5 + (this.age * 0.1);
  ...
  // Breeding with Complete Kittens (Bodies & 1-Year Max Growth)
  triggerBreedingSuccess() {
    ...
    document.getElementById('victory-summary').textContent = 
      `어미 고양이가 5살이 되어 귀여운 아기 고양이 ${kittenCount}마리를 낳았어요! 아기 고양이도 함께 무럭무럭 키워보세요! 🎉`;
  ...
  updatePhysics() {
    ...
    // Kitten Age calculation: Caps at 1 Year Old (1살이 되면 더 안 자람!)
    this.kittens.forEach(k => {
      k.x += (Math.random() * 2 - 1);
      k.x = Math.max(120, Math.min(750, k.x));

      const livedSeconds = this.gameTimeSeconds - k.spawnTime;
      k.age = Math.min(1, Math.floor(livedSeconds / 30)); // Reaches 1 year old and stops growing!
    });
  }
  ...
  // Natural Lying Down Pose with Fitted Outfits & Spiral Swirl Eyes
  drawLyingDownCat(theme) {
    ...
    // Outfit fitted to horizontal lying body
    ...
    // Eyes: Check if Spiral Eyes active!
    if (this.isSpiralEyes) {
  ...
  // Normal Standing Cat with Fitted Outfits & Spiral Swirl Eyes
  drawStandingCat(theme) {
    ...
    // Outfit fitted to standing body
  ...
  // Draw Dynamic Spinning Spiral Whirlpool Swirls
  ...
  drawOutfitOnLyingBody(bx, by) {
    ...
    if (this.bodyOutfitIndex === 1) {
      ...
      // 👗 Pink Dress on lying body
      this.ctx.fillStyle = '#ec4899';
      this.ctx.beginPath();
      this.ctx.ellipse(bx, by, 36, 19, 0, 0, Math.PI * 2);
      this.ctx.fill();
    } else if (this.bodyOutfitIndex === 3) {
      // 🧥 Hero Cape draped over lying body
      this.ctx.fillStyle = '#ef4444';
      this.ctx.fillRect(bx - 28, by - 14, 56, 26);
    } else if (this.bodyOutfitIndex === 4) {
      // 🦺 Overalls on lying body
      this.ctx.fillStyle = '#eab308';
  ...
  // Newborn Kitten with Complete Body (Head, Ears, Full Body, Paws, Tail) & Max 1-Year Growth
  drawKitten(kitten) {
    this.ctx.save();
    this.ctx.translate(kitten.x, kitten.y);

    // Kitten scale increases from 0.45x (0살) up to 0.65x (1살), and stays capped at 1 year old!
    const kittenScale = 0.45 + kitten.age * 0.2;
