...
class CatHealingGame {
  constructor() {
    ...
    // Feature unlocks (Accelerated schedule for 2-minute game loop)
    // 0s -> Dressroom, 30s -> Walk, 60s -> Bath, 90s -> Brush, 120s -> Tease
  ...
  updateTimeAndDecay() {
    ...
    // Age increases 1 year every 24 seconds (so at 120s = 5 years old!)
    ...
    // 2 Minutes (120 seconds) breeding trigger!
  ...
  triggerBreedingSuccess() {
    ...
    document.getElementById('victory-summary').textContent = 
      `당신의 헌신적인 사랑 덕분에 2분이 지나 귀여운 새끼 고양이 ${kittenCount}마리를 낳았어요! 🎉`;
