/**
 * [초고퀄리티] 고양이 키우기 힐링 게임 (v10)
 * - Baby kitten lies down on its side and floats hearts for 3 seconds when clicked!
 * - Dedicated "🔄 다시하기" (Restart) button added directly below the cat name profile.
 ...
 * - Mother cat outfits perfectly fitted for both standing and lying down poses.
 */
...
class CatHealingGame {
  ...
  initEvents() {
    ...
    // Dedicated Restart Button below Cat Name
    ...
    // Canvas Mouse Click: Kitten Side-Lying & Floating Hearts or Mother Cat Petting
    this.canvas.addEventListener('click', (e) => {
      ...
      this.kittens.forEach(k => {
        ...
        if (distKitten < 40) {
          ...
          // Spawn floating hearts immediately!
          this.particles.push(new FloatingParticle(k.x, k.y - 45, '🐾 옆으로 누워 골골~ (3초)', '#ff4081'));
          ...
          // Spawn continuous hearts over the 3 seconds
          for (let i = 1; i <= 6; i++) {
            setTimeout(() => {
              if (k.isCuddling) {
  ...
  triggerBreedingSuccess() {
    ...
    document.getElementById('victory-summary').textContent = 
      `어미 고양이가 5살이 되어 귀여운 새끼 고양이 ${kittenCount}마리를 낳았어요! 아기 고양이를 클릭하면 옆으로 누워서 하트를 띄우며 애교를 부려요! 🎉`;
  ...
  // --- Mother Cat Rendering (Grows Every 2 Minutes / 120s up to 5 Years Old) ---
  drawMainCat() {
    this.ctx.save();
  ...
  // Draw Kitten (Lying down on side with floating hearts vs playful standing)
  drawKitten(kitten) {
    this.ctx.save();
    ...
    if (kitten.isCuddling) {
      // 3-Second Horizontal Side-Lying Pose with Sweet Closed Eyes (^ ^)
      this.ctx.fillStyle = 'rgba(0, 0, 0, 0.15)';
      this.ctx.beginPath();
      this.ctx.ellipse(0, 10, 36, 10, 0, 0, Math.PI * 2);
      this.ctx.fill();

      // Body lying horizontally on floor
