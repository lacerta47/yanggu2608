/**
 * [초고퀄리티] 고양이 키우기 힐링 게임 (v11)
 * - Kittens sleep together with mother cat at night!
 * - Kittens accompany mother cat to Fountain Park walks & Bathroom baths!
 * - Clicking a kitten plays a cute "냐옹~" baby meow and floats hearts while lying down!
 * - Dedicated "🔄 다시하기" (Restart) button below cat name profile.
 * - Mother cat ages 1 year every 2 minutes (120s), growing larger until 5 years old (growth stops).
...
class CatHealingGame {
  constructor() {
    ...
    this.age = 0; // Ages 1 year every 2 minutes (120 seconds), capped at 5 years!
  ...
  initEvents() {
    ...
    // Canvas Mouse Click: Kitten Click -> "냐옹~ 💕" + Floating Hearts
    this.canvas.addEventListener('click', (e) => {
      ...
      this.kittens.forEach(k => {
        ...
        if (distKitten < 40) {
          ...
          // High pitched baby meow sound!
          this.soundManager.playBabyMeowSound();

          // Cute "냐옹~ 💕" text and floating hearts!
          this.particles.push(new FloatingParticle(k.x, k.y - 45, '냐옹~ 💕', '#ff4081'));
          this.particles.push(new FloatingParticle(k.x - 12, k.y - 65, '❤️', '#ff4081'));
          this.particles.push(new FloatingParticle(k.x + 12, k.y - 75, '❤️', '#ff4081'));

          for (let i = 1; i <= 6; i++) {
            setTimeout(() => {
              if (k.isCuddling) {
                this.particles.push(new FloatingParticle(
                  k.x + (Math.random() * 30 - 15),
                  k.y - 45 - (i * 8),
                  '❤️',
                  '#ff4081'
                ));
  ...
  updateTimeAndDecay() {
    ...
    // Mother cat gains 1 year every 2 minutes (120 seconds), capped at 5 years old!
    const newAge = Math.min(5, Math.floor(this.gameTimeSeconds / 120));
    if (newAge !== this.age) {
      this.age = newAge;
  ...
  updateAgeDisplay() {
    ...
    else if (this.age >= 5) ageStage = '완전체 어른 (더이상 안 커져요!)';
  ...
  // Day & Night Toggle: Mother Cat AND Baby Kittens Sleep Together on Bed!
  toggleDayNight() {
    ...
    if (this.isNight) {
      ...
      // Kittens sleep next to mother cat on/around the bed!
      this.kittens.forEach((k, idx) => {
        k.targetX = 560 + idx * 45;
        k.targetY = 350;
        this.particles.push(new FloatingParticle(k.targetX, k.targetY - 35, '💤 zzz...', '#a5d8ff'));
      });
  ...
  // Kittens accompany mother cat to Fountain Park!
  goToPark() {
    ...
    // Position kittens around the fountain in Park
    this.kittens.forEach((k, idx) => {
      k.x = 240 + idx * 90;
      k.y = 380;
    });
  ...
  // Kittens accompany mother cat to Bathroom bath!
  goToBathroom() {
    ...
    // Position kittens enjoying bath next to mother cat!
    this.kittens.forEach((k, idx) => {
      k.x = 320 + idx * 60;
      k.y = 370;
    });
    ...
    this.soundManager.playMeowSound();
    this.soundManager.playBabyMeowSound();
    this.particles.push(new FloatingParticle(this.catX, this.catY - 60, '🛁 냥~ 다함께 즐거운 목욕!', '#339af0'));
  ...
  triggerBreedingSuccess() {
    ...
    document.getElementById('victory-summary').textContent = 
      `어미 고양이가 5살이 되어 귀여운 새끼 고양이 ${kittenCount}마리를 낳았어요! 함께 잠을 자고, 공원 산책과 목욕도 같이 즐겨보세요! 🎉`;
  ...
  updatePhysics() {
    ...
    // Kitten Physics & Sleep / Walk Position Updates
    this.kittens.forEach(k => {
      if (this.isNight) {
        // Night sleep mode -> Move towards bed
        if (k.targetX) {
          k.x += (k.targetX - k.x) * 0.05;
          k.y = k.targetY;
        }
      } else if (k.isCuddling) {
        k.cuddleTimer--;
        if (k.cuddleTimer <= 0) {
          k.isCuddling = false;
        }
  ...
  drawScene() {
    ...
    // Render Kittens in Bedroom, Park, and Bathroom scenes!
    if (this.currentScene === 'BEDROOM' || this.currentScene === 'PARK' || this.currentScene === 'BATHROOM') {
  ...
  // Draw Kitten (Sleeping with mother cat at night vs Cuddling on click vs Standing)
  drawKitten(kitten) {
    this.ctx.save();
    ...
    if (this.isNight || this.catAction === 'SLEEP') {
      // 🌙 Night Sleeping Pose (Mother & Kittens Sleep Together!)
      this.ctx.fillStyle = 'rgba(0, 0, 0, 0.15)';
      this.ctx.beginPath();
      this.ctx.ellipse(0, 8, 30, 8, 0, 0, Math.PI * 2);
      this.ctx.fill();
      ...
      this.ctx.beginPath();
      this.ctx.ellipse(0, -6, 24, 14, 0, 0, Math.PI * 2);
      this.ctx.fill(); this.ctx.stroke();

      // Head nestled
      this.ctx.beginPath();
      this.ctx.arc(-16, -8, 13, 0, Math.PI * 2);
      this.ctx.fill(); this.ctx.stroke();

      // Sleeping Ears
      this.ctx.fillStyle = kitten.earColor;
      this.ctx.beginPath();
      this.ctx.moveTo(-25, -16); this.ctx.lineTo(-32, -24); this.ctx.lineTo(-20, -20);
      this.ctx.fill(); this.ctx.stroke();
      ...
      this.ctx.lineWidth = 2;
      this.ctx.beginPath();
      this.ctx.arc(-20, -8, 3, Math.PI, 0);
      this.ctx.arc(-12, -8, 3, Math.PI, 0);
      this.ctx.stroke();

    } else if (kitten.isCuddling) {
      // 3-Second Horizontal Side-Lying Pose with Sweet Closed Eyes & Floating Hearts
      this.ctx.fillStyle = 'rgba(0, 0, 0, 0.15)';
      this.ctx.beginPath();
      this.ctx.ellipse(0, 10, 36, 10, 0, 0, Math.PI * 2);
      this.ctx.fill();
      ...
      this.ctx.lineWidth = 1.8;
      this.ctx.beginPath();
      this.ctx.ellipse(0, -6, 28, 14, 0, 0, Math.PI * 2);
      this.ctx.fill(); this.ctx.stroke();

      this.ctx.beginPath();
      this.ctx.arc(-22, -10, 14, 0, Math.PI * 2);
      this.ctx.fill(); this.ctx.stroke();

      this.ctx.fillStyle = kitten.earColor;
      this.ctx.beginPath();
      this.ctx.moveTo(-32, -18); this.ctx.lineTo(-40, -28); this.ctx.lineTo(-26, -23);
      this.ctx.fill(); this.ctx.stroke();

      this.ctx.beginPath();
      this.ctx.moveTo(-16, -20); this.ctx.lineTo(-18, -30); this.ctx.lineTo(-8, -23);
      this.ctx.fill(); this.ctx.stroke();

      this.ctx.strokeStyle = '#e11d48';
      this.ctx.lineWidth = 2;
      this.ctx.beginPath();
      this.ctx.arc(-26, -11, 3.5, Math.PI, 0);
      this.ctx.arc(-17, -11, 3.5, Math.PI, 0);
      this.ctx.stroke();

      this.ctx.fillStyle = '#ff85a2';
      this.ctx.beginPath();
      this.ctx.arc(-21, -6, 1.8, 0, Math.PI * 2);
      this.ctx.fill();

      this.ctx.fillStyle = kitten.color;
      this.ctx.beginPath();
      this.ctx.arc(-10, 4, 4.5, 0, Math.PI * 2);
      this.ctx.arc(10, 4, 4.5, 0, Math.PI * 2);
      this.ctx.fill(); this.ctx.stroke();
      ...
      this.ctx.quadraticCurveTo(40, -14, 35, -25);
      this.ctx.stroke();

    } else {
      // Standing / Walking / Bathing Kitten
      this.ctx.fillStyle = 'rgba(0, 0, 0, 0.15)';
      this.ctx.beginPath();
